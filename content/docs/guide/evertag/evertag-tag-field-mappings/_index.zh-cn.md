...
title: "标签字段映射"
date: 2020-02-01
description: "探索 Evertag 应用支持的完整音频标签字段列表，包括内部字段名称及其在主要元数据格式中的映射关系。"
keywords: ["音频元数据", "音频标签", "ID3 标签", "Vorbis Comments", "MP4 标签", "ASF 元数据", "Evertag 标签支持", "音乐文件标签", "音频标签格式"]
tags: ["Evertag", "音频标签", "元数据", "ID3", "Vorbis", "MP4", "ASF"]
cascade:
  type: docs
---


Evertag 支持多种音频标签字段，适用于 ID3、MP4、Vorbis Comments 和 ASF 等流行格式。  

本参考指南概述了每个标签字段、其在 Evertag 中的内部名称以及它如何映射到不同的元数据标准。   

使用它可以更好地了解 Evertag 如何为您的音乐库组织和编辑标签数据。
...
- **MP4 (3g2, m4a, m4p, m4r, m4v, mp4)**: shwm
