...
title: "標籤欄位對應"
date: 2020-02-01
description: "探索 Evertag 應用程式支援的完整音訊標籤欄位列表，包括內部欄位名稱及其在主要元資料格式中的對應關係。"
keywords: ["音訊元資料", "音訊標籤", "ID3 標籤", "Vorbis Comments", "MP4 標籤", "ASF 元資料", "Evertag 標籤支援", "音樂檔案標籤", "音訊標籤格式"]
tags: ["Evertag", "音訊標籤", "元資料", "ID3", "Vorbis", "MP4", "ASF"]
cascade:
  type: docs
---


Evertag 支援多種音訊標籤欄位，適用於 ID3、MP4、Vorbis Comments 和 ASF 等流行格式。  

本參考指南概述了每個標籤欄位、其在 Evertag 中的內部名稱以及它如何對應到不同的元資料標準。   

使用它可以更好地了解 Evertag 如何為您的音樂庫組織和編輯標籤資料。
