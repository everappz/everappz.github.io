...
- **Navegador** (HTTP)
- **Computador** (WebDAV)
- **Computador (avançado)** (SMB) - uma unidade de rede para Mac, Windows e Linux; num Mac aparece sozinha na barra lateral do Finder. A única ligação que pode ser encriptada.
...
- **Exigir encriptação SMB** - cifra cada transferência SMB com **encriptação SMB3 (AES)** para que mais ninguém na rede consiga ler os seus ficheiros. Desativado por predefinição. Requer um **início de sessão e palavra-passe** definidos acima (as ligações encriptadas não podem ser anónimas) e um cliente que suporte SMB3, como o Finder num Mac moderno ou o Windows 10 e posterior. As alterações entram em vigor da próxima vez que iniciar a partilha. *(Premium.)*
...
- **Encriptação SMB** - proteja as transferências SMB com encriptação SMB3 (AES).
