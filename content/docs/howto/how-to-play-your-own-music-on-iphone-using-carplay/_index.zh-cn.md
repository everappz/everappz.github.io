---
title: "如何使用 CarPlay 在 iPhone 上播放自己的音乐"
date: 2025-07-16
description: "了解如何使用 Apple CarPlay 在 iPhone 上播放自己的本地或离线音乐。包含使用 Evermusic 和 Flacbox 等应用的分步指南和技巧。"
keywords: ["carplay 音乐", "在 iphone 上播放自己的音乐", "iphone 本地音乐", "carplay 离线音乐", "evermusic", "flacbox", "iphone mp3 播放器"]
tags: ["carplay", "iphone", "本地音乐", "离线播放", "evermusic", "flacbox"]
readingTime: 5
---
...
如果遇到任何问题，请检查您的互联网连接和用户名/密码。
在应用的 Premium 版本中，您可以添加无限数量的服务。

### 支持的云存储服务

目前，该应用支持最流行的云存储服务：iCloud Drive、Dropbox、OneDrive、Google Drive、MEGA、Synology Drive、SMB、WebDAV、DLNA、Yandex.Disk、Box、阿里云盘、百度网盘、pCloud、WD My Cloud Home、InfiniCLOUD、MediaFire、OpenDrive、HiDrive、Cloud Mail.ru、Put.io、MyDrive。此列表在应用的免费版和高级版之间以及不同应用之间可能有所不同。

### 从云存储下载文件

您可以通过从已连接的云存储服务下载音频文件来添加到应用中，如[此处](/docs/howto/how-to-upload-my-files-to-the-cloud-storage-and-connect-them-to-evermusic-flacbox-evertag)所述。

### 离线模式

您可以自动同步文件以进行离线播放，如[此处](/docs/howto/play-offline-music-in-evermusic-flacbox-download-sync-from-cloud-to-local-files/)所述。

### 从设备导入文件

如[此处](/docs/howto/how-to-play-local-music-stored-on-your-iphone-or-mac)所述，轻松从设备导入文件，或如[此处](/docs/howto/how-to-connect-a-usb-flashcard-to-the-iphone-and-listen-to-music-or-manage-files-located-on-it)所述从连接的 USB 驱动器导入。

### iTunes 文件共享

如[此处](/docs/howto/how-to-transfer-files-from-my-mac-to-iphone-or-ipad-using-finder)所述，使用有线连接传输文件。

### Wi-Fi Drive

如[此处](/docs/howto/how-to-transfer-files-wirelessly-from-a-computer-to-an-iphone-using-wifi-drive)所述，无线传输文件。

## 在 iPhone 上启用 CarPlay

### 要求
- 运行 iOS 13.0 或更高版本的 iPhone
- 支持 **Apple CarPlay** 的汽车（USB 或无线）
- iPhone 上必须启用 **Siri**

### 通过 USB 设置 CarPlay

1. 启动汽车并确保 iPhone 上已启用 **Siri**。
2. 使用 **Lightning 转 USB 线缆**将 iPhone 连接到汽车。
3. 当 iPhone 上出现提示时，点击**允许**以启用 CarPlay。
4. CarPlay 应自动在汽车的信息娱乐屏幕上启动。

### 设置无线 CarPlay

1. 启动汽车并确保已开启 **Bluetooth** 和 **Wi-Fi**。
2. 在 iPhone 上，前往：
   - **设置 > 一般 > CarPlay**
3. 从可用车辆列表中选择您的汽车。
4. 按照汽车显示屏上的说明完成配对。
...
Evermusic 中的**资料库**标签页是所有音乐的组织中心。将曲目添加到资料库后——无论是来自云存储、本地文件还是 iTunes——应用会自动读取元数据并整齐地分组到**专辑**、**艺术家**、**流派**等类别中。
...
- **作曲家** – 按作曲家元数据整理
- **此应用中的文件** – 存储在应用中的离线文件
- **此 iPhone 上的文件** – 从"文件"应用手动导入的曲目
- **iTunes 音乐** – 来自 iPod 资料库的音乐（不包括 Apple Music）
- **在线文件** – 直接从云服务流媒体播放的音乐
...
- **收藏文件** – 快速访问您标记为收藏的文件夹或文件，便于快速导航，在 CarPlay 中尤其有用。
- **最近打开的文件** – 查看您从已连接服务访问的最新文件。
- **云存储** – 管理所有已连接的云服务和本地网络驱动器。

您可以在连接主屏幕上使用**"连接到云存储"**选项来连接新的云服务。连接后，您可以浏览文件夹、查看音乐资料库，甚至可以即时流播或下载曲目。如果文件夹嵌套很深，您可以**将其标记为收藏**以便日后轻松访问——这对需要快速导航的 CarPlay 用户特别有帮助。

您还可以使用 **Wi-Fi Drive** 或 **iTunes 文件共享**从电脑无线或通过 USB 导入音乐。支持连接 NAS 设备、外部驱动器和本地网络，让您完全掌控音乐的存储和访问方式。

要了解有关连接和管理云端及网络资料库的所有方式的更多信息，请阅读完整的[连接指南](/docs/guide/evermusic/evermusic-guide-connections/)。
...
- **收藏文件** – 您标记为收藏的文件夹或文件的快捷方式。对 CarPlay 或快速访问特别有用。
- **最近打开的文件** – 您最近从本地存储打开的曲目和文件夹。
- **文档** – 显示应用内部文档目录中的所有文件和文件夹。包括通过云下载、Wi-Fi Drive、iTunes 文件共享或 USB 传输添加的项目。
- **离线文件夹** – 显示您标记为离线使用的所有文件夹（例如，为离线播放下载的云文件夹、专辑、播放列表或艺术家合集）。
- **iCloud** – 显示通过 iCloud Drive 同步的内容，确保跨设备访问。
- **设备文件夹** – 位于 iPhone/iPad 上应用外部的文件夹。通过系统文件选择器或文件夹连接手动添加。
...
- **播放全部** – 将当前文件夹（和子文件夹）中的所有音频文件添加到播放器队列。
- **随机播放全部** – 以随机顺序添加相同文件。
- **继续播放** – 如果在应用设置中启用，则从上次保存的位置继续播放。
...
这是由于 CarPlay 系统的限制，它限制了可以显示的文件夹层级数量。
...
- **将深度嵌套的文件夹添加到收藏夹**
  这使它们可以直接从 CarPlay 中的**收藏夹**菜单轻松访问。

- **简化文件夹结构**
  如果可能，请尝试扁平化文件夹层次结构，以获得更顺畅的 CarPlay 导航。

此解决方法确保在车内浏览音乐时获得无缝体验。
...
要查看和控制播放，只需点击 CarPlay 屏幕右上角的**正在播放**图标。

**正在播放**屏幕包括：

- 完整的播放控制（播放、暂停、下一首、上一首）
- 曲目标题、艺术家和专辑封面
- 播放模式，如**重复**、**随机播放**和**播放速度**调整

此屏幕让您在驾驶时安全高效地控制聆听体验。
...
- **刷新屏幕** – 手动重新加载 CarPlay 界面并更新所有内容元素。
- **一般** – 配置基本音频播放设置：
  - 重复模式
  - 随机播放模式
  - 音频输出模式
...
- **显示图片** – 开启/关闭专辑封面和图标。禁用图片可能会提升 CarPlay 性能。
- **连接时暂停播放** – 连接 CarPlay 时自动暂停音频，以避免突然大声播放。
- **文件加载** – 指定加载云文件时使用的网络类型（仅 Wi-Fi 或 Wi-Fi + 蜂窝网络）。
...
这些设置也可以在移动界面的**设置 → CarPlay** 中找到，您可以在将 iPhone 连接到车辆之前微调播放和界面行为。

要了解更多信息，请阅读完整的[设置指南](/docs/guide/evermusic/evermusic-guide-settings/)。

> **请注意：** 本指南中提到的某些设置和功能可能仅适用于 **Evermusic** 或 **Flacbox**。功能可用性在每个应用的免费版和高级版之间也可能有所不同。
...
- 从云端、USB 或 iTunes 将音乐添加到 iPhone
- 将 CarPlay 与您的车辆连接和设置
- 从资料库、连接或本地文件导航和管理音乐
- 使用高级音频设置自定义 CarPlay 播放

现在轮到您握住方向盘，享受您最喜爱的音乐——按您的方式——每次驾驶时。

在我们完整的 [Evermusic 用户指南](/docs/guide/evermusic/)中探索更多功能、设置和教程。
