---
title: "Cara Memainkan Muzik Anda Sendiri di iPhone Menggunakan CarPlay"
date: 2025-07-16
description: "Ketahui cara memainkan muzik tempatan atau luar talian anda di iPhone menggunakan Apple CarPlay. Panduan langkah demi langkah dengan petua untuk menggunakan aplikasi seperti Evermusic dan Flacbox."
keywords: ["muzik carplay", "mainkan muzik sendiri di iphone", "muzik tempatan iphone", "muzik luar talian carplay", "evermusic", "flacbox", "pemain mp3 iphone"]
tags: ["carplay", "iphone", "muzik tempatan", "main balik luar talian", "evermusic", "flacbox"]
readingTime: 5
---

{{< author-byline >}}


**Ringkasan:** Gunakan [Evermusic](https://apps.apple.com/app/apple-store/id885367198?pt=95781850&ct=everappzcom&mt=8) atau [Flacbox](https://apps.apple.com/app/apple-store/id1097564256?pt=95781850&ct=everappzcom&mt=8) untuk memainkan fail MP3, FLAC, atau audio lain di iPhone melalui Apple CarPlay. Tambah muzik daripada storan awan, USB, atau pemindahan Wi-Fi, kemudian layari pustaka, senarai main dan folder anda terus di skrin kereta anda.

## Pengenalan

Ingin memainkan muzik anda sendiri di kereta menggunakan Apple CarPlay? Sama ada lagu anda disimpan di iPhone, dalam storan awan, atau luar talian, aplikasi seperti **Evermusic** dan **Flacbox** memudahkan anda mendengar koleksi muzik peribadi semasa memandu.

{{< cards cols="1">}}
{{< card title="" subtitle="Baris Giliran CarPlay Seterusnya" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/carplay-nowplaying-5.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

Dalam panduan ini, kami akan menunjukkan cara menyediakan fail muzik anda untuk CarPlay, menyusunnya dengan kulit album dan maklumat trek yang betul, serta memainkannya dengan selamat dari iPhone anda. Dengan Evermusic atau Flacbox, anda boleh mencipta senarai main, dan menstrim atau memuat turun lagu daripada perkhidmatan seperti **Google Drive**, **Dropbox**, **OneDrive**, **NAS**, atau komputer rumah anda.

Aplikasi ini sesuai untuk sesiapa yang mahukan kawalan penuh ke atas pustaka muzik mereka.

{{< cards cols="2">}}
{{< card link="https://apps.apple.com/app/apple-store/id885367198?pt=95781850&ct=everappzcom&mt=8" title="Muat Turun Evermusic" icon="download" tag="Free" >}}
{{< card link="https://apps.apple.com/app/apple-store/id1097564256?pt=95781850&ct=everappzcom&mt=8" title="Muat Turun Flacbox" icon="download" tag="Free" >}}
{{< /cards >}}

## Masukkan Fail ke dalam Aplikasi

Pertama, anda perlu menambah fail ke dalam aplikasi. Terdapat beberapa cara untuk melakukannya:

### Sambung ke storan awan

- Buka tab 'Sambungan'
- Pilih 'Sambung ke storan awan' daripada menu
- Pilih perkhidmatan storan awan daripada senarai
- Masukkan kelayakan anda, dan ketik 'Selesai.'

Jika anda menghadapi sebarang masalah, pastikan anda menyemak sambungan internet dan log masuk/kata laluan anda.  
Dalam versi Premium aplikasi, anda boleh menambah bilangan perkhidmatan yang tidak terhad.

### Perkhidmatan storan awan yang disokong     

Pada masa ini, aplikasi menyokong perkhidmatan storan awan yang paling popular: iCloud Drive, Dropbox, OneDrive, Google Drive, MEGA, Synology Drive, SMB, WebDAV, DLNA, Yandex.Disk, Box, 阿里云盘, 百度网盘, pCloud, WD My Cloud Home, InfiniCLOUD, MediaFire, OpenDrive, HiDrive, Cloud Mail.ru, Put.io, MyDrive. Senarai ini mungkin berbeza antara versi percuma dan premium aplikasi, dan juga antara aplikasi yang berbeza.

### Muat turun fail daripada storan awan

Anda boleh menambah fail audio ke dalam aplikasi dengan memuat turunnya daripada perkhidmatan storan awan yang disambungkan seperti yang diterangkan [di sini](/docs/howto/how-to-upload-my-files-to-the-cloud-storage-and-connect-them-to-evermusic-flacbox-evertag).

### Mod luar talian

Anda boleh menyegerakkan fail secara automatik untuk main balik luar talian seperti yang diterangkan [di sini](/docs/howto/play-offline-music-in-evermusic-flacbox-download-sync-from-cloud-to-local-files/).

### Import fail daripada peranti anda

Import fail dengan mudah daripada peranti anda seperti yang diterangkan [di sini](/docs/howto/how-to-play-local-music-stored-on-your-iphone-or-mac) atau pemacu USB yang disambungkan seperti yang diterangkan [di sini](/docs/howto/how-to-connect-a-usb-flashcard-to-the-iphone-and-listen-to-music-or-manage-files-located-on-it).

### Perkongsian Fail iTunes

Pindahkan fail menggunakan sambungan kabel seperti yang diterangkan [di sini](/docs/howto/how-to-transfer-files-from-my-mac-to-iphone-or-ipad-using-finder).

### Wi-Fi Drive

Pindahkan fail secara wayarles seperti yang diterangkan [di sini](/docs/howto/how-to-transfer-files-wirelessly-from-a-computer-to-an-iphone-using-wifi-drive).

## Aktifkan CarPlay di iPhone

### Keperluan
- iPhone dengan iOS 13.0 atau lebih baharu  
- Kereta dengan sokongan **Apple CarPlay** (USB atau wayarles)  
- **Siri** mesti diaktifkan di iPhone anda

### Sediakan CarPlay melalui USB

1. Hidupkan kereta anda dan pastikan **Siri** diaktifkan di iPhone anda.
2. Sambungkan iPhone anda ke kereta menggunakan kabel **Lightning ke USB**.
3. Apabila diminta di iPhone anda, ketik **Benarkan** untuk mengaktifkan CarPlay.
4. CarPlay akan dilancarkan secara automatik di skrin infotainment kereta anda.

### Sediakan CarPlay Wayarles

1. Hidupkan kereta anda dan pastikan **Bluetooth** dan **Wi-Fi** dihidupkan.
2. Di iPhone anda, pergi ke:
   - **Tetapan > Umum > CarPlay**
3. Pilih kereta anda daripada senarai kenderaan yang tersedia.
4. Ikut arahan yang ditunjukkan di paparan kereta anda untuk melengkapkan pemasangan.

### Pilihan: Sesuaikan Susun Atur CarPlay Anda

- Pergi ke **Tetapan > Umum > CarPlay**
- Pilih kereta anda daripada senarai
- Ketik **Sesuaikan** untuk menyusun semula atau menyembunyikan aplikasi

### Petua Penyelesaian Masalah

- Pastikan kereta anda dalam **mod pemasangan** (rujuk manual kereta anda)
- Mulakan semula iPhone dan sistem kereta anda jika sambungan gagal
- Pastikan perisian tegar kereta anda dikemas kini

## Antara Muka Aplikasi dalam Mod CarPlay

Setelah anda melancarkan aplikasi Evermusic atau Flacbox dalam mod CarPlay, anda akan melihat antara muka utama yang dibahagikan kepada 4 tab utama: Pustaka, Sambungan, Fail Tempatan, Tetapan. 

{{< cards cols="1">}}
{{< card title="" subtitle="Menu Utama CarPlay" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/library-with-images.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

## Pustaka

Tab **Pustaka** dalam Evermusic ialah hab pusat di mana semua muzik anda disusun. Setelah anda menambah trek ke pustaka anda—sama ada daripada storan awan, fail tempatan, atau iTunes—aplikasi secara automatik membaca metadata mereka dan mengumpulkannya dengan kemas ke dalam kategori seperti **Album**, **Artis**, **Genre**, dan lain-lain.

Skrin ini memberi anda akses pantas ke kegemaran, terkini, senarai main, penanda buku dan semua trek yang ditambah. Anda juga boleh meneruskan main balik daripada sesi terakhir anda, melihat lagu yang belum dimainkan, dan meneroka muzik mengikut tag atau jenis sumber.

{{< cards cols="1">}}
{{< card title="" subtitle="Pustaka" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/library-with-images-3.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

Bahagian **Pustaka** mengandungi kategori berikut:

- **Teruskan Main Balik** – Sambung semula senarai main atau baris giliran lagu terakhir anda  
- **Kegemaran** – Trek yang anda tandakan dengan bintang  
- **Terkini** – Lagu yang baru dimainkan  
- **Lagu Belum Dimainkan** – Trek yang telah ditambah tetapi belum dimainkan  
- **Penanda Buku** – Kedudukan yang disimpan dalam fail audio panjang  
- **Senarai Main** – Koleksi tersuai dan dijana automatik anda  
- **Lagu** – Semua lagu dikumpulkan mengikut tajuk  
- **Album** – Dikumpulkan mengikut nama album  
- **Artis Album** – Dikumpulkan mengikut metadata artis album  
- **Artis** – Dikumpulkan mengikut tag artis  
- **Genre** – Berdasarkan maklumat genre terbenam  
- **Komposer** – Disusun mengikut metadata komposer  
- **Fail dalam Aplikasi Ini** – Fail luar talian yang disimpan dalam aplikasi  
- **Fail di iPhone Ini** – Trek yang diimport secara manual daripada aplikasi Fail  
- **Muzik iTunes** – Muzik daripada pustaka iPod anda (tidak termasuk Apple Music)  
- **Fail Dalam Talian** – Muzik yang distrim terus daripada perkhidmatan awan

{{< cards cols="1">}}
{{< card title="" subtitle="Paparan Album" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/albums.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

Anda boleh membuka mana-mana submenu dan mengetik trek untuk memulakan main balik serta-merta. Untuk butiran lanjut, lihat [Panduan Pustaka Muzik](/docs/guide/evermusic/evermusic-guide-music-library/) penuh.


## Sambungan

Tab **Sambungan** ialah hab pusat anda untuk mengakses dan mengurus semua perkhidmatan storan awan dan peranti rangkaian tempatan yang disambungkan.

{{< cards cols="1">}}
{{< card title="" subtitle="Sambungan" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/connections-with-images-3.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

Dari sini, anda boleh menyambung ke platform awan popular seperti Dropbox, Google Drive, OneDrive, MEGA, iCloud Drive, dan juga pemacu rangkaian seperti SMB, DLNA, dan WebDAV. Setelah disambungkan, anda boleh melayari, menstrim, memuat turun, dan mengurus fail terus dari dalam aplikasi.

Skrin **Sambungan** merangkumi bahagian berikut:

- **Fail Kegemaran** – Akses pantas ke folder atau fail yang anda tandakan sebagai kegemaran untuk navigasi pantas, terutamanya berguna dalam CarPlay.
- **Fail yang Baru Dibuka** – Lihat fail terkini yang anda akses daripada perkhidmatan yang disambungkan.
- **Storan Awan** – Urus semua perkhidmatan awan dan pemacu rangkaian tempatan yang disambungkan.

Anda boleh menyambungkan perkhidmatan awan baharu menggunakan pilihan **"Sambung ke Storan Awan"** di skrin Sambungan utama. Setelah disambungkan, anda boleh menavigasi ke dalam folder, melayari pustaka muzik, dan juga menstrim atau memuat turun trek semasa dalam perjalanan. Jika folder terletak terlalu dalam, anda boleh **menandakannya sebagai Kegemaran** untuk akses mudah kemudian — terutamanya membantu untuk pengguna CarPlay yang mahukan navigasi pantas.

Anda juga boleh menggunakan **Wi-Fi Drive** atau **Perkongsian Fail iTunes** untuk mengimport muzik daripada komputer anda secara wayarles atau melalui USB. Sokongan untuk menyambung ke peranti NAS, pemacu luaran, dan rangkaian tempatan memberi anda kawalan penuh ke atas cara anda menyimpan dan mengakses muzik anda.

Untuk mengetahui lebih lanjut tentang semua cara untuk menyambung dan mengurus pustaka awan dan rangkaian anda, baca [Panduan Sambungan](/docs/guide/evermusic/evermusic-guide-connections/) penuh.

## Fail Tempatan

Bahagian **Fail Tempatan** ialah hab pusat anda untuk mengurus fail audio yang disimpan terus pada peranti anda atau di dalam direktori **Dokumen** aplikasi Evermusic. Ia juga termasuk fail luar talian yang dimuat turun daripada storan awan, fail cache pemain audio, dan folder yang telah anda sediakan untuk main balik luar talian. Bahagian ini memastikan anda boleh menikmati pustaka muzik anda walaupun tanpa sambungan internet.

{{< cards cols="1">}}
{{< card title="" subtitle="Fail Tempatan" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/local-files-with-images.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

Skrin **Fail Tempatan** disusun ke dalam bahagian utama berikut:

- **Fail Kegemaran** – Pintasan ke folder atau fail yang anda tandakan sebagai kegemaran. Terutamanya berguna untuk CarPlay atau akses pantas.
- **Fail yang Baru Dibuka** – Trek dan folder yang anda buka baru-baru ini daripada storan tempatan.
- **Dokumen** – Memaparkan semua fail dan folder yang terletak di dalam direktori Dokumen dalaman aplikasi. Ini termasuk item yang ditambah melalui muat turun awan, Wi-Fi Drive, Perkongsian Fail iTunes, atau pemindahan USB.
- **Folder Luar Talian** – Menunjukkan semua folder yang anda tandakan untuk kegunaan luar talian (cth., folder awan, album, senarai main, atau koleksi artis yang dimuat turun untuk main balik luar talian).
- **iCloud** – Memaparkan kandungan yang disegerakkan melalui iCloud Drive, memastikan akses merentas semua peranti.
- **Folder Peranti** – Folder yang terletak di luar aplikasi pada iPhone/iPad anda. Ini ditambah secara manual melalui pemilih fail sistem atau sambungan folder.
- **Pemain Audio** – Folder cache yang digunakan untuk pengoptimuman crossfade dan prestasi. Boleh dilumpuhkan atau dikosongkan dalam tetapan.

{{< cards cols="1">}}
{{< card title="" subtitle="Folder Peranti dalam Fail Tempatan" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/local-files-device-folders.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

Anda boleh mengetahui lebih lanjut dalam [Panduan Fail Tempatan](/docs/guide/evermusic/evermusic-guide-local-files/) penuh.

## Paparan Folder

{{< cards cols="1">}}
{{< card title="" subtitle="Folder Tempatan dengan Kulit" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/local-folder-with-images-2.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

Apabila anda membuka folder, anda akan menemui satu set tindakan berguna di bahagian atas:

- **Main Semua** – Menambah semua fail audio dalam folder semasa (dan subfolder) ke baris giliran pemain.
- **Kocok Semua** – Menambah fail yang sama dalam susunan rawak.
- **Teruskan Main Balik** – Sambung semula main balik dari kedudukan terakhir yang disimpan jika diaktifkan dalam tetapan aplikasi.

## Had Kedalaman Kandungan

{{< cards cols="1">}}
{{< card title="" subtitle="Had Kedalaman Kandungan" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/content-depth-limit.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

Apabila menggunakan CarPlay, anda mungkin menemui ralat **"Had Kedalaman Kandungan"** — terutamanya jika pustaka muzik anda mempunyai banyak folder bersarang yang dalam.  
Ini berlaku kerana batasan dalam sistem CarPlay, yang menyekat berapa banyak tahap folder yang boleh dipaparkan.

**Cara Mengatasinya**

Untuk mengelakkan masalah ini dan mengakses muzik anda dengan lebih pantas:

- **Tambah folder bersarang yang dalam ke Kegemaran**  
  Ini menjadikannya mudah diakses terus daripada menu **Kegemaran** dalam CarPlay.

- **Mudahkan struktur folder anda**  
  Jika boleh, cuba ratakan hierarki folder anda untuk navigasi CarPlay yang lebih lancar.

Penyelesaian ini memastikan pengalaman yang lancar apabila melayari muzik anda di dalam kereta.

## Skrin Sedang Dimainkan

{{< cards cols="1">}}
{{< card title="" subtitle="Entri CarPlay Sedang Dimainkan" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/carplay-nowplaying.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

Selepas mengetik mana-mana fail audio, ia akan ditambah secara automatik ke **baris giliran pemain**.  
Untuk melihat dan mengawal main balik, cukup ketik ikon **Sedang Dimainkan** yang terletak di sudut kanan atas skrin CarPlay.

Skrin **Sedang Dimainkan** termasuk:

- Kawalan main balik penuh (main, jeda, seterusnya, sebelumnya)  
- Tajuk trek, artis, dan karya seni album  
- Mod main balik seperti **Ulang**, **Kocok**, dan pelarasan **Kelajuan Main Balik**

Skrin ini membolehkan anda kekal mengawal pengalaman mendengar anda semasa memandu — dengan selamat dan cekap.

## Tetapan

{{< cards cols="1">}}
{{< card title="" subtitle="Menu Tetapan" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/settings-2.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

Bahagian **Tetapan** dalam antara muka CarPlay membolehkan anda menyesuaikan cara aplikasi berfungsi semasa anda memandu. Tetapan ini membantu meningkatkan prestasi, mengurangkan gangguan, dan memberikan pengalaman mendengar yang lebih lancar.

- **Muat Semula Skrin** – Muat semula UI CarPlay secara manual dan kemas kini semua elemen kandungan.
- **Umum** – Konfigurasikan tetapan main balik audio penting:
  - Mod Ulang
  - Mod Kocok
  - Mod Output Audio
  - Simpan Kedudukan Main Balik
  - Simpan Keadaan Pemain Audio

- **Isih** – Laraskan cara kandungan diisih dalam menu CarPlay seperti fail, pustaka muzik, dan sambungan.

{{< cards cols="1">}}
{{< card title="" subtitle="Menu Pilihan Isih" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/settings-sort.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

- **Had Pemuatan Kandungan** – Tetapkan berapa banyak item yang perlu muncul setiap skrin. Had yang lebih rendah meningkatkan kelajuan pemuatan dan mengurangkan usaha menatal.
- **Tunjukkan Imej** – Togol karya seni album dan ikon. Melumpuhkan imej mungkin meningkatkan prestasi CarPlay.
- **Jeda Main Balik Apabila Disambungkan** – Secara automatik menjeda audio apabila CarPlay disambungkan untuk mengelakkan main balik kuat secara tiba-tiba.
- **Pemuatan Fail** – Tentukan jenis rangkaian mana (Wi-Fi sahaja atau Wi-Fi + Selular) yang perlu digunakan apabila memuatkan fail awan.

- **Penyama Audio**

{{< cards cols="1">}}
{{< card title="" subtitle="Panel Konfigurasi Penyama" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/settings-eq-configuration.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

Aktifkan penyama audio terbina dalam, laraskan jalur frekuensi, dan pilih daripada praset yang dikonfigurasikan untuk pengalaman bunyi yang diperibadikan.

{{< cards cols="1">}}
{{< card title="" subtitle="Senarai Praset Penyama" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/settings-equalizer.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

- **Main Balik Crossfade**

{{< cards cols="1">}}
{{< card title="" subtitle="Main Balik Crossfade" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/settings-crossfade-playback.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

Cipta peralihan yang lancar antara lagu dengan bertindih pengakhiran satu trek dengan permulaan trek seterusnya. Tempoh crossfade boleh disesuaikan.

- **Main Balik Tanpa Jeda**

{{< cards cols="1">}}
{{< card title="" subtitle="Main Balik Tanpa Jeda" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/settings-gapless-playback.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

Mainkan trek dengan lancar tanpa jeda — sesuai untuk rakaman langsung, campuran DJ, dan album konsep.

Tetapan ini juga tersedia pada antara muka mudah alih di bawah **Tetapan → CarPlay**, di mana anda boleh melaraskan main balik dan tingkah laku antara muka sebelum menyambungkan iPhone anda ke kenderaan anda.

Untuk mengetahui lebih lanjut, baca [Panduan Tetapan](/docs/guide/evermusic/evermusic-guide-settings/) penuh.

> **Sila ambil perhatian:** Sesetengah tetapan dan ciri yang disebutkan dalam panduan ini mungkin eksklusif untuk **Evermusic** atau **Flacbox** sahaja. Ketersediaan ciri juga boleh berbeza antara versi percuma dan premium setiap aplikasi.

## Kesimpulan

Dengan **Evermusic** dan **Flacbox**, memainkan muzik anda sendiri di kereta menggunakan Apple CarPlay menjadi mudah, fleksibel, dan boleh dipercayai. Sama ada anda menstrim daripada storan awan, mengakses fail tempatan, atau memainkan trek yang dimuat turun secara luar talian — aplikasi ini dibina untuk memberi anda kawalan penuh ke atas pengalaman mendengar anda semasa memandu.

{{< cards cols="1">}}
{{< card title="" subtitle="Skrin CarPlay Sedang Dimainkan" image="/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/carplay-nowplaying-2.webp" imageStyle="border-radius: clamp(14px, 2vw, 28px);" >}}
{{< /cards >}}

Daripada integrasi awan yang lancar hingga penyegerakan folder luar talian, daripada organisasi pustaka muzik yang mendalam hingga main balik yang boleh disesuaikan dengan penyama dan crossfade — ciri-ciri ini menjadikan Evermusic dan Flacbox lebih daripada sekadar pemain audio. Mereka adalah teman CarPlay pintar yang direka untuk audiofil, pengguna harian, dan penumpang tetap.

Dengan mengikuti panduan ini, anda telah mempelajari cara untuk:

- Menambah muzik daripada awan, USB, atau iTunes ke iPhone anda  
- Menyambung dan menyediakan CarPlay dengan kenderaan anda  
- Menavigasi dan mengurus muzik daripada Pustaka, Sambungan, atau Fail Tempatan  
- Menyesuaikan main balik CarPlay dengan tetapan audio lanjutan    

Kini giliran anda untuk memegang stereng dan menikmati muzik kegemaran anda — dengan cara anda — setiap kali anda memandu.

Terokai lebih banyak ciri, tetapan, dan panduan cara dalam [Panduan Pengguna Evermusic](/docs/guide/evermusic/) penuh kami.

## Soalan Lazim

{{% details title="Apakah format fail muzik yang berfungsi dengan CarPlay dalam Evermusic dan Flacbox?" closed="true" %}}
Evermusic dan Flacbox menyokong pelbagai format audio termasuk MP3, FLAC, AAC, WAV, AIFF, OGG, WMA, dan banyak lagi. Semua format yang disokong berfungsi melalui CarPlay tanpa sebarang penukaran diperlukan.
{{% /details %}}

{{% details title="Bolehkah saya memainkan muzik dari Google Drive atau Dropbox di CarPlay?" closed="true" %}}
Ya. Kedua-dua Evermusic dan Flacbox membolehkan anda menyambung ke perkhidmatan storan awan seperti Google Drive, Dropbox, OneDrive, MEGA, dan lain-lain. Anda boleh menstrim muzik secara langsung atau memuat turunnya untuk main balik CarPlay luar talian.
{{% /details %}}

{{% details title="Adakah saya memerlukan sambungan internet untuk memainkan muzik di CarPlay?" closed="true" %}}
Tidak. Anda boleh memuat turun muzik daripada storan awan untuk main balik luar talian. Setelah fail disimpan secara tempatan pada iPhone anda, ia boleh dimainkan melalui CarPlay tanpa sebarang sambungan internet.
{{% /details %}}

{{% details title="Mengapa saya melihat ralat Had Kedalaman Kandungan di CarPlay?" closed="true" %}}
CarPlay menyekat berapa banyak tahap folder yang boleh dipaparkan. Jika muzik anda berada dalam folder bersarang yang dalam, tambah folder tersebut ke Kegemaran supaya anda boleh mengaksesnya terus daripada menu Kegemaran dalam CarPlay.
{{% /details %}}

{{% details title="Adakah Evermusic atau Flacbox percuma untuk digunakan dengan CarPlay?" closed="true" %}}
Kedua-dua aplikasi percuma untuk dimuat turun dengan sokongan CarPlay penuh, penyama, dan ciri main balik. Versi percuma mempunyai had pada sambungan awan (3), senarai main (10), dan folder luar talian (1). Premium menghapuskan semua had.
{{% /details %}}
