...
keywords: [
  "שאלות נפוצות Evermusic", "תמיכה Flacbox", "עזרה Evervideo", "שאלות Evertag",
  "איך להשתמש ב-Evermusic", "פתרון בעיות נגן מוזיקה בענן", "מדריך הפעלה לא מקוון",
  "תמיכה בעורך תגי שמע", "בעיות סטרימינג וידאו", "מדריך העברת קבצים"
]
tags: [
  "שאלות נפוצות", "עזרה", "תמיכה", "evermusic", "flacbox", "evervideo", "evertag",
  "הגדרת ענן", "בעיות הפעלה", "ניהול קבצים", "עריכת מטא-נתונים",
  "פתרון בעיות", "מצב לא מקוון", "נגן מוזיקה", "נגן וידאו"
]
aliases:
...
מחפש תשובות או עזרה בשימוש באחת מהאפליקציות שלנו? הגעת למקום הנכון.

דפי השאלות הנפוצות שלנו יעזרו לך לחבר אחסון ענן, לנהל קבצי מוזיקה וסרטונים, להגדיר השמעה לא מקוונת, לכוון הגדרות אקולייזר, לתקן מטא-נתונים ועוד הרבה יותר.

עיין בשאלות הנפוצות של האפליקציה שלך להלן כדי להתחיל, או עיין בשאלות ותשובות נפוצות שקיבלנו מהודעות דוא"ל של משתמשים.
...
**הגדרת WebDAV:**<br>
- **שרתי ארה"ב:** `https://webdav.pcloud.com`  
- **שרתים אירופאיים:** `https://ewebdav.pcloud.com`  
- **שם משתמש:** כתובת הדוא"ל שלך ב-pCloud  
- **סיסמה:** סיסמת חשבון pCloud שלך  
...
1. עבור ל**פינה הימנית העליונה** של המסך ופתח את **מרכז הבקרה** (ליד השעון).  
2. לחץ על סמל **קול/עוצמת קול**.  
3. במסך הבא, לחץ על **AirPlay** לצפייה ברשימת כל מכשירי AirPlay הזמינים.  
...
- ודא שגרסת האפליקציה העדכנית ביותר מותקנת בשני המכשירים<br>
- הפעל **iCloud** בשני המכשירים<br>
- הפעל את האפליקציה במכשיר iOS שלך והמתן דקה אחת להעלאת פרטי הרכישה<br>
- ב-Mac, התקן את האפליקציה מה-App Store באמצעות **אותו Apple ID**<br>
- הפעל את האפליקציה והמתן כמה שניות לסנכרון המידע<br>
...
- **גיבוי ושחזור** מהגדרות האפליקציה [כיצד להעביר את ספריית המוזיקה בין מכשירים ב-Evermusic: מדריך שלב אחר שלב](/docs/howto/how-to-transfer-your-music-library-between-devices-in-evermusic-step-by-step-guide/)<br>
- **ייצוא רשימת השמעה ל-M3U**, ולאחר מכן ייבוא במכשיר אחר:<br>
  - [כיצד לייצא רשימות השמעה](/docs/howto/export-tracks-collection-from-evermusic-flacbox-to-m3u-csv-txt/)<br>
...
- פרטי כניסה מאוחסנים בבטחה ב-**iOS Keychain**<br>
- אנו אוספים רק דוחות קריסה ושימוש אנונימיים<br>
- תוכל לבטל הסכמה ב**הגדרות האפליקציה ← אנליטיקה**<br><br>

מידע נוסף:<br>
- [מדיניות פרטיות](/legal/privacy-policy/)<br>
- [אבטחת Apple Keychain](https://support.apple.com/guide/security/keychain-data-protection-secb0694df1a/web)<br><br>
...
- **הגדרות ← ספריית מוזיקה ← סנכרון מוזיקה מקוון ← סנכרון ברקע**<br><br>
...
עם זאת, אם התחברת בהצלחה ל-**WD MyCloud EX2 Ultra**, **WD MyCloud Mirror** או דגם אחסון **WD MyCloud** אחר, עדיין תוכל להשתמש בו עם הפתרון החלופי הבא:<br><br>

1. פתח **חיבורים ← התחבר לאחסון ענן ← My Cloud Home**<br>
2. צור תיקיה באמצעות מנהל הקבצים המובנה<br>
3. העלה קבצי מוזיקה לתיקיה זו<br>
4. אלה יאוחסנו ב"sandbox" הנגיש רק לאפליקציה<br>
...
**Evermusic**<br>
- iOS (סמל כחול): `lsevermusic://`<br>
- macOS: `lsevermusicmac://`<br>
- iOS (סמל אדום): `lsevermusicpro://`<br><br>
...
- הפעל **SMB1** בשרת שלך ובהגדרות האפליקציה<br>
- השתמש **בבוחר הקבצים של המערכת**:<br>
  - עבור אל **קבצים מקומיים > קבצים במכשיר זה > פתח קבצים...**<br>
...
– **כיצד מנהלים אחסון iCloud?**  <br>
יש לך שתי אפשרויות:  <br>
1. השתמש ב-[https://www.icloud.com](https://www.icloud.com) בדפדפן שלך כדי לארגן, להעלות או למחוק קבצים.<br>  
2. באפליקציה, לאחר חיבור ל-iCloud דרך **חיבורים ← התחבר לאחסון ענן ← iCloud Drive**, השתמש במנהל הקבצים המובנה להעלאה, הורדה, שינוי שם תיקיות או מחיקת קבצים ישירות באחסון iCloud שלך — מבלי לשמור אותם במכשיר שלך.<br>

⚠️ היזהר בעת מחיקת קבצים. האפליקציה מוחקת אותם לצמיתות (הם לא יעברו לאשפה ואי אפשר לשחזר אותם).<br>
...
1. **שימוש בחיבור כבל (מומלץ לספריות גדולות)**  <br>
   השתמש באפליקציית **Apple Devices** ב-Windows 11 להעברת קבצים ישירות ל-iPhone דרך USB.  
   עקוב אחר המדריך של Apple כאן:  
   https://support.apple.com/en-ph/120402 <br>

2. **אלחוטית דרך Wi-Fi Drive**  <br>
   הפעל את תכונת **WiFi Drive** באפליקציה והעלה את המוזיקה שלך דרך דפדפן במחשב.  
   הוראות שלב אחר שלב כאן:  
   [כיצד להעביר קבצי מוזיקה ממחשב ל-iPhone ללא iTunes באמצעות WiFi-Drive](/docs/howto/how-to-transfer-music-from-computer-to-iphone-without-itunes/) <br>

3. **אלחוטית באמצעות שרת DLNA**  <br>
   הפעל את שרת המדיה DLNA במחשב Windows שלך והזרם או העבר את הספרייה שלך ישירות לאפליקציה.  
   מדריך:  
   [כיצד להפעיל שרת מדיה DLNA ב-Windows 10 ולנגן מוזיקה ב-iPhone](/docs/howto/how-to-enable-dlna-media-server-on-windows-10-and-play-your-music-on-iphone/) <br>

4. **אלחוטית באמצעות שיתוף קבצים SMB**  <br>
   הפעל **שיתוף קבצים SMB** במחשב שלך והתחבר אליו באפליקציה לגלישה, הורדה או העברת קבצים תיקיה אחר תיקיה.  
   הוראות:  
   [העברת קבצים ממחשב ל-iPhone באמצעות פרוטוקול SMB](/docs/howto/transfer-your-files-from-the-computer-to-iphone-using-smb-protocol/) <br>
