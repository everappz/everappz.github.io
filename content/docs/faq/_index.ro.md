...
keywords: [
  "Evermusic FAQ", "suport Flacbox", "ajutor Evervideo", "întrebări Evertag",
  "cum să utilizezi Evermusic", "depanare player muzică cloud", "ghid redare offline",
  "suport editor etichete audio", "probleme streaming video", "tutorial transfer fișiere"
]
tags: [
  "FAQ", "ajutor", "suport", "evermusic", "flacbox", "evervideo", "evertag",
...
Cauți răspunsuri sau ajutor pentru a folosi una dintre aplicațiile noastre? Ești în locul potrivit.

Paginile noastre de întrebări frecvente te vor ajuta să conectezi spațiul de stocare în cloud, să gestionezi fișierele muzicale și video, să configurezi redarea offline, să ajustezi setările egalizatorului, să corectezi metadatele și multe altele.

Explorează întrebările frecvente pentru aplicația ta de mai jos pentru a începe, sau răsfoiește întrebările și răspunsurile comune pe care le-am primit prin e-mailurile utilizatorilor.
...
**Configurare WebDAV:**<br>
- **Servere SUA:** `https://webdav.pcloud.com`  
- **Servere europene:** `https://ewebdav.pcloud.com`  
- **Nume utilizator:** Adresa ta de e-mail pCloud  
- **Parolă:** Parola contului tău pCloud  
...
1. Mergi în **colțul din dreapta sus** al ecranului și deschide **Centrul de control** (lângă ceas).  
2. Fă clic pe pictograma **Sunet/Volum**.  
3. Pe ecranul următor, fă clic pe **AirPlay** pentru a vedea o listă cu toate dispozitivele AirPlay disponibile.  
...
- Asigură-te că ai cea mai recentă versiune a aplicației instalată pe ambele dispozitive<br>
- Activează **iCloud** pe ambele dispozitive<br>
- Pornește aplicația pe dispozitivul iOS și așteaptă 1 minut pentru ca informațiile de achiziție să fie încărcate<br>
- Pe Mac, instalează aplicația din App Store folosind același **Apple ID**<br>
- Pornește aplicația și așteaptă câteva secunde pentru sincronizarea informațiilor<br>
...
- **Backup și restaurare** din Setările aplicației [Cum să transferi biblioteca de muzică între dispozitive în Evermusic: ghid pas cu pas](/docs/howto/how-to-transfer-your-music-library-between-devices-in-evermusic-step-by-step-guide/)<br>
- **Exportați lista de redare în M3U**, apoi importați pe alt dispozitiv:<br>
  - [Cum să exportați listele de redare](/docs/howto/export-tracks-collection-from-evermusic-flacbox-to-m3u-csv-txt/)<br>
...
- Datele de autentificare sunt stocate în siguranță în **iOS Keychain**<br>
- Colectăm doar rapoarte anonime de defecțiuni și utilizare<br>
- Poți renunța în **Setările aplicației → Analiză**<br><br>

Mai multe informații:<br>
- [Politica de confidențialitate](/legal/privacy-policy/)<br>
- [Securitatea Apple Keychain](https://support.apple.com/guide/security/keychain-data-protection-secb0694df1a/web)<br><br>
...
- **Setări → Biblioteca muzicală → Sincronizare muzică online → Sincronizare în fundal**<br><br>
...
Cu toate acestea, dacă te-ai conectat cu succes la un **WD MyCloud EX2 Ultra**, **WD MyCloud Mirror** sau alt model de stocare **WD MyCloud**, îl poți folosi în continuare cu următoarea soluție alternativă:<br><br>

1. Deschide **Conexiuni → Conectare la stocare cloud → My Cloud Home**<br>
2. Creează un dosar folosind managerul de fișiere integrat<br>
3. Încarcă fișiere muzicale în acest dosar<br>
4. Acestea vor fi stocate într-un „sandbox" accesibil doar de aplicație<br>
...
**Evermusic**<br>
- iOS (pictogramă albastră): `lsevermusic://`<br>
- macOS: `lsevermusicmac://`<br>
- iOS (pictogramă roșie): `lsevermusicpro://`<br><br>
...
- Activează **SMB1** pe serverul tău și în setările aplicației<br>
- Folosește **selectorul de fișiere al sistemului**:<br>
  - Mergi la **Fișiere locale > Fișiere pe acest dispozitiv > Deschide fișiere...**<br>
...
– **Cum gestionez spațiul de stocare iCloud?**  <br>
Ai două opțiuni:  <br>
1. Folosește [https://www.icloud.com](https://www.icloud.com) în browser pentru a organiza, încărca sau șterge fișiere.<br>  
2. În aplicație, după conectarea la iCloud prin **Conexiuni → Conectare la stocare cloud → iCloud Drive**, folosește managerul de fișiere integrat pentru a încărca, descărca, redenumi dosare sau șterge fișiere direct în spațiul de stocare iCloud — fără a le salva pe dispozitivul tău.<br>

⚠️ Fii atent când ștergi fișiere. Aplicația le șterge permanent (nu vor ajunge în coșul de gunoi și nu pot fi recuperate).<br>
...
1. **Folosind o conexiune prin cablu (recomandat pentru biblioteci mari)**  <br>
   Folosește aplicația **Apple Devices** pe Windows 11 pentru a transfera fișiere direct pe iPhone prin USB.  
   Urmează ghidul Apple aici:  
   https://support.apple.com/en-ph/120402 <br>

2. **Wireless prin Wi-Fi Drive**  <br>
   Activează funcția **WiFi Drive** din aplicație și încarcă muzica printr-un browser pe computer.  
   Instrucțiuni pas cu pas aici:  
   [Cum să transferi fișiere muzicale de pe computer pe iPhone fără iTunes folosind WiFi-Drive](/docs/howto/how-to-transfer-music-from-computer-to-iphone-without-itunes/) <br>

3. **Wireless folosind serverul DLNA**  <br>
   Pornește serverul media DLNA pe computerul Windows și transmite sau transferă biblioteca direct în aplicație.  
   Ghid:  
...
4. **Wireless folosind partajarea fișierelor SMB**  <br>
   Activează **partajarea fișierelor SMB** pe PC și conectează-te la el în aplicație pentru a naviga, descărca sau transfera fișierele dosar cu dosar.  
   Instrucțiuni:  
   [Transferă fișiere de pe computer pe iPhone folosind protocolul SMB](/docs/howto/transfer-your-files-from-the-computer-to-iphone-using-smb-protocol/) <br>
