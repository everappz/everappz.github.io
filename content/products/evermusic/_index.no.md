---
date: '2025-06-12T17:00:00+00:00'
draft: false
title: 'Evermusic - Frakoblet musikkspiller for iPhone og Mac'
layout: hextra-home
description: "Last ned den beste gratis frakoblede musikkspilleren for din iPhone og Mac og lytt til musikken din overalt."
keywords: ["Evermusic", "frakoblet musikkspiller", "iOS lydspiller", "skymusikkapp", "last ned musikk iPhone", "Mac musikkspiller", "strømme musikk frakoblet", "FLAC-spiller iPhone", "multi-sky musikkspiller", "Evermusic app funksjoner"]
tags: ["Evermusic", "frakoblet avspilling", "skymusikk", "MP3-spiller", "iPhone-apper", "lydstrømming", "musikkbehandling", "Mac-apper", "mediaspiller"]
aliases:
  - /evermusic/
appStoreUrl: "https://apps.apple.com/app/apple-store/id885367198?pt=95781850&ct=everappzcom&mt=8"
appStoreId: "885367198"
features: "Frakoblet musikkavspilling, Skystrømming (iCloud, Google Drive, Dropbox, OneDrive, MEGA, Box), Lydequalizer med forhåndsinnstillinger, Crossfade og sømløs avspilling, M3U spilleliste import/eksport, Sangtekstvisning, Lydbokmerker, Apple CarPlay, AirPlay og Chromecast, SMB/WebDAV/DLNA-støtte, USB-minnepinne avspilling, Last.fm scrobbling"
screenshots:
  - "https://everappz.com/products/evermusic/screenshots/2048x2732/3.png"
  - "https://everappz.com/products/evermusic/screenshots/2048x2732/4.png"
  - "https://everappz.com/products/evermusic/screenshots/2048x2732/5.png"
  - "https://everappz.com/products/evermusic/screenshots/2048x2732/6.png"
---

<!-- <body class="hx:bg-transparent" style="background: radial-gradient(ellipse at 50% 80%, rgba(59,130,246,0.05), hsla(0,0%,100%,0));"> -->

{{< force-dark >}}

{{< hextra/hero-container
  image="/products/evermusic/heroimage/hero_1200.webp"
  imageWidth="600"
  imageCard="true"
>}}

<div class="hx:flex hx:flex-col hx:items-center hx:justify-center hx:sm:block">

<div class="hx:mt-4 hx:mr-4">
{{< hextra/hero-badge link="../../blog/evermusic-celebrates-11-million-worldwide-downloads-a-musical-milestone" >}}
  <div class="hx:w-2 hx:h-2 hx:rounded-full hx:bg-primary-400"></div>
  <span>11 millioner nedlastinger</span>
    {{< icon name="arrow-circle-right" attributes="height=14" >}}
{{< /hextra/hero-badge >}}
</div>

<div class="hx:mt-6 hx:mb-6">
<div class="hx:flex hx:gap-4 hx:items-center">
  <img src="/images/app_icons/webp/Evermusic_Icon-App-1024x1024.webp" alt="Evermusic Icon" loading="lazy" class="hero-headline-icon" />
  {{< hextra/hero-headline >}}
  Evermusic
  {{< /hextra/hero-headline >}}
</div>
</div>

<div class="hx:mb-6">
{{< hextra/hero-centered-subtitle >}}
<a href="https://www.chip.de/downloads/Evermusic-Pro-iPhone-_-iPad-App_91614216.html" target="_blank" rel="noopener">
  Er den perfekte løsningen for å organisere og spille&nbsp;<br class="hx:sm:block hx:hidden" />din egen musikk fra skyen&nbsp;<br class="hx:sm:block hx:hidden" /><strong>chip.de</strong>
  </span>
{{< /hextra/hero-centered-subtitle >}}
</div>

<div class="hx:mb-6">
{{< hextra/hero-paragraph >}}
• Spill musikk med crossfade, sømløs avspilling og equalizer  
• Importer M3U-spillelister og last ned sanger for frakoblet lytting  
• Strøm musikk fra skytjenester, NAS, datamaskin eller USB-minnepinner  
• Se sangtekster mens du lytter og legg til lydbokmerker for å fortsette når som helst  
{{< /hextra/hero-paragraph >}}
</div>

{{< hextra/hero-button 
  text="Last ned for mobil"
  link="https://apps.apple.com/app/apple-store/id885367198?pt=95781850&ct=everappzcom&mt=8"
  icon="device-mobile" 
  style="margin-top:1rem;margin-right:1rem;"
>}}

{{< hextra/hero-button 
  text="Last ned for skrivebord"
  link="https://apps.apple.com/app/apple-store/id1564384601?pt=95781850&ct=everappzcom&mt=8"
  icon="desktop-computer" 
  style="margin-top:1rem;margin-right:1rem;"
>}}

</div>

{{< /hextra/hero-container >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

{{< hextra/feature-grid >}}

  {{< hextra/feature-card
    title="Spar plass på enheten"
    subtitle="Har du 100 GB med musikk samlet gjennom årene, men bare 32 GB lagringsplass på iPhone? Bare flytt alle sporene dine til skyen og frigjør verdifull plass til bilder, videoer og apper."
    icon="folder"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(59,130,246,0.15), hsla(0,0%,100%,0));"
  >}}

  {{< hextra/feature-card
    title="Umiddelbar tilgang til spor"
    subtitle="Med støtte for de mest populære skytjenestene lar Evermusic deg velge én eller flere tjenester og strømme musikken din fra skyen. Ingen mer iTunes-synkronisering, lydfilene dine lastes opp til skyen fra PC eller Mac akkurat som andre filer."
    icon="collection"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(192,38,211,0.15), hsla(0,0%,100%,0));"
  >}}

  {{< hextra/feature-card
    title="Frakoblet modus"
    subtitle="Frakoblet modus lar deg laste ned favorittalbum, spor, sjangre og spillelister for frakoblet avspilling og lytte til musikk når du ikke er koblet til internett."
    icon="download"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(234,88,12,0.15), hsla(0,0%,100%,0));"
  >}}

  {{< hextra/feature-card
    title="Musikken din i skyen"
    subtitle="Lag din egen avanserte musikkstrømmetjeneste gratis! Administrer ditt massive musikkbibliotek, spar lagringsplass på enheten, og strøm favorittspor direkte fra skyen med smart buffering og sømløs avspilling. Nyt støtte for populære skytjenester som iCloud, Google Drive, Dropbox, MEGA.nz, Box, OneDrive og mer."
    icon="cloud"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(34,197,94,0.15), hsla(0,0%,100%,0));"
  >}}

  {{< hextra/feature-card
    title="Overfør filer enkelt"
    subtitle="Koble til din Mac eller PC og strøm musikk direkte fra hjemmedatamaskinen. Overfør lydfiler sømløst mellom datamaskinen og iOS-enheten din ved hjelp av Wi-Fi Drive eller iTunes fildeling. Du kan også koble til din NAS eller en USB-minnepinne og få tilgang til biblioteket ditt fra hvor som helst."
    icon="duplicate"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(251,191,36,0.15), hsla(0,0%,100%,0));"
  >}}

  {{< hextra/feature-card
    title="Automatisk synkronisering"
    subtitle="Synkroniseringsbehandleren skanner automatisk skylagringen din etter lydfiler og oppdaterer musikkbiblioteket ditt. Metadatalasteren organiserer spor etter album, artister og sjangre ved hjelp av innebygde lydtagger."
    icon="refresh"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(6,182,212,0.15), hsla(0,0%,100%,0));"
  >}}

{{< /hextra/feature-grid >}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Rent og enkelt design
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

{{< cards cols="4">}}

  {{< card 
    icon="adjustments"
    image="/products/evermusic/screenshots/2048x2732/3.png" 
    title="Lydequalizer" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Finjuster lyden din med en iPod-lignende lydequalizer, tilpassbare forhåndsinnstillinger og forforsterkerforsterkning for den beste lytteopplevelsen." 
  >}}

  {{< card 
    icon="annotation"
    image="/products/evermusic/screenshots/2048x2732/4.png"  
    title="Sangtekstvisning" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Les innebygde sangtekster og sporkommentarer mens du lytter. Nyt synkroniserte sangtekster for en mer engasjerende musikkopplevelse." 
  >}}

  {{< card 
    icon="collection"
    image="/products/evermusic/screenshots/2048x2732/5.png"  
    title="Spillelistebehandler" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Opprett og organiser egendefinerte spillelister, endre rekkefølge på sanger, eksporter til M3U, eller arkiver dem som ZIP-filer for enkel deling eller sikkerhetskopiering." 
  >}}

  {{< card 
    icon="cloud"
    image="/products/evermusic/screenshots/2048x2732/6.png"  
    title="Skymusikkstrømming" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Koble til de beste skylagringsplattformene som Google Drive, Dropbox og OneDrive for å strømme musikksamlingen din når som helst, hvor som helst." 
  >}}

  {{< card 
    icon="duplicate"
    image="/products/evermusic/screenshots/2048x2732/9.png"  
    title="Filbehandler" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Administrer lydfilene dine enkelt – gi nytt navn til spor, organiser mapper og overfør musikk mellom enheter ved hjelp av innebygde verktøy." 
  >}} 

  {{< card 
    icon="sun"
    image="/products/evermusic/screenshots/2048x2732/10.png"  
    title="Apptilpasning" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Tilpass spilleren til din stil: bytt temaer, juster oppsett og konfigurer funksjoner for en virkelig personlig musikkappopplevelse." 
  >}}

{{< /cards >}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Komplett funksjonssett
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

{{< cards >}}

  {{< card title="Lydformater" subtitle="Støtter: mp3, m4a, flac, wav, aac, mp4, m3u, m3u8, m4b, m4p, aiff, ogg, mp2, ac3, wma og mer." icon="music-note" >}}

  {{< card title="Lydbøker" subtitle="Inkluderer lydbokmerker, avspillingshastighetskontroll og gjenopptakelsesposisjon." icon="book-open" width="50">}}

  {{< card title="Automatisk synkronisering" subtitle="Synkroniserer musikkbiblioteket ditt med skyen, organisert etter artist, album og sjanger." icon="refresh" >}}

  {{< card title="Filbehandler" subtitle="Administrer filer med operasjoner som kopier, flytt, gi nytt navn og slett." icon="folder" >}}

  {{< card title="Skytjenester" subtitle="Støtter iCloud, Google Drive, Dropbox, OneDrive, Box, MEGA, WD MyCloud Home, pCloud og mer." icon="cloud" >}}

  {{< card title="Crossfade-avspilling" subtitle="Neste sang begynner før den nåværende slutter for sømløse overganger." icon="switch-horizontal" >}}

  {{< card title="Sømløs avspilling" subtitle="Ingen pauser mellom sanger – ideelt for live, klassisk eller konseptalbum." icon="volume-up" >}}

  {{< card title="Lydequalizer" subtitle="Tilpassbar equalizer med forhåndsinnstillinger, manuell kontroll og forforsterkerforsterkning." icon="adjustments" >}}

  {{< card title="Avansert søk" subtitle="Finn favorittalbumene, artistene og sporene dine raskt og enkelt." icon="search" >}}

  {{< card title="Datatilgang" subtitle="Koble til via SMB, WebDAV eller DLNA. Overfør via iTunes fildeling og Wi-Fi." icon="desktop-computer" >}}

  {{< card title="AirPlay og Chromecast" subtitle="Strøm til Apple TV og Google Chromecast-enheter trådløst." icon="device-mobile" >}}

  {{< card title="Spillelister" subtitle="Opprett, rediger, endre rekkefølge og eksporter/importer M3U-spillelister med frakoblet modus." icon="collection" >}}

  {{< card title="USB-minnekort" subtitle="Spill musikk direkte fra SD-kort ved hjelp av eksterne lesere som SanDisk iXpand." icon="inbox" >}}

  {{< card title="Frakoblet modus" subtitle="Last ned album og spillelister eller hurtigbufr nylig spilte spor automatisk for frakoblet lytting." icon="download" >}}

  {{< card title="Apple CarPlay" subtitle="Kjør trygt med et dedikert CarPlay-grensesnitt for musikkavspilling." icon="truck" >}}

  {{< card title="ID3-tagredigering" subtitle="Rediger ødelagte eller manglende metadata med den innebygde tagredigereren." icon="pencil-alt" >}}

  {{< card title="Sangtekster og kommentarer" subtitle="Se innebygde sangtekster og kommentarer mens du lytter til musikken din." icon="annotation" >}}

  {{< card title="Last.fm" subtitle="Koble til Last.fm-kontoen din for å få personlig lyttestatistikk og anbefalinger." icon="chart-bar" >}}
  
{{< /cards >}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  App Store-anmeldelser
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">
{{< appstore-reviews apps="885367198,1564384601" stars="5,4" >}}
</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Prisplaner
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">
{{< hextra/feature-grid cols="4">}}

  {{< hextra/feature-card
    title="Gratis"
    class="pricing-card pricing-free"
    subtitle=`<span class='pricing-details'>• Inneholder annonser<br />
• Spillelister (10)<br />
• Skytjenester (3)<br />
• Arkiver mediesamlinger (3)<br />
• Favoritter (1000)<br />
• Sanger i spilleliste (1000)<br />
• Sanger i kø (750)<br />
• Frakoblede mapper (1)<br />
• Frakoblede album (1)<br />
• Automatisk taggsøk (20)<br />
• Eksporter til M3U (5)<br />
• Last ned filer<br />
• Eksporter iTunes-sanger</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    <strong>Gratis</strong>
  </span>
</span>`
  >}}

  {{< hextra/feature-card
    title="Premium månedlig"
    class="pricing-card pricing-subscription"
    subtitle=`<span class='pricing-details'>• Annonsefri opplevelse<br />
• Ubegrensede spillelister<br />
• Ubegrensede skytjenester<br />
• Ubegrenset mediearkivering<br />
• Ubegrensede favoritter<br />
• Ubegrensede sanger i spilleliste<br />
• Ubegrensede sanger i kø<br />
• Ubegrensede frakoblede mapper<br />
• Ubegrensede frakoblede album<br />
• Ubegrenset taggsøk<br />
• Ubegrenset eksport til M3U<br />
• Last ned filer<br />
• Eksporter iTunes-sanger<br />
• Eksporter iTunes-samlinger<br />
• Last ned mapper<br />
• Last ned samlinger<br />
• Full tilpasning</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    <strong>$1.99</strong>/måned
  </span>
</span>`
  >}}

  {{< hextra/feature-card
    title="Premium årlig"
    class="pricing-card pricing-subscription"
    subtitle=`<span class='pricing-details'>• Annonsefri opplevelse<br />
• Ubegrensede spillelister<br />
• Ubegrensede skytjenester<br />
• Ubegrenset mediearkivering<br />
• Ubegrensede favoritter<br />
• Ubegrensede sanger i spilleliste<br />
• Ubegrensede sanger i kø<br />
• Ubegrensede frakoblede mapper<br />
• Ubegrensede frakoblede album<br />
• Ubegrenset taggsøk<br />
• Ubegrenset eksport til M3U<br />
• Last ned filer<br />
• Eksporter iTunes-sanger<br />
• Eksporter iTunes-samlinger<br />
• Last ned mapper<br />
• Last ned samlinger<br />
• Full tilpasning</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    7 dager gratis, deretter <strong>$12.99</strong>/år
  </span>
</span>`
  >}}

  {{< hextra/feature-card
    title="Premium livstid"
    class="pricing-card pricing-lifetime"
    subtitle=`<span class='pricing-details'>• Annonsefri opplevelse<br />
• Ubegrensede spillelister<br />
• Ubegrensede skytjenester<br />
• Ubegrenset mediearkivering<br />
• Ubegrensede favoritter<br />
• Ubegrensede sanger i spilleliste<br />
• Ubegrensede sanger i kø<br />
• Ubegrensede frakoblede mapper<br />
• Ubegrensede frakoblede album<br />
• Ubegrenset taggsøk<br />
• Ubegrenset eksport til M3U<br />
• Last ned filer<br />
• Eksporter iTunes-sanger<br />
• Eksporter iTunes-samlinger<br />
• Last ned mapper<br />
• Last ned samlinger<br />
• Full tilpasning</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    <strong>$59.99</strong>/livstid
  </span>
</span>`
  >}}

{{< /hextra/feature-grid >}}

</div>

<div class="hx:mt-6"></div>

<div class="hx:w-full hx:mx-auto hx:flex hx:flex-col hx:items-center">
  {{< hextra/info-paragraph border="true" >}}
   <strong>Familiedeling</strong>: Alle kjøp og abonnementer støtter Familiedeling, slik at du kan dele Premium-tilgang med familien din.<br><strong>Universell tilgang</strong>: Livstids- og abonnementsplaner deles mellom iOS- og Mac-enheter ved hjelp av iCloud-synkronisering.<br><strong>Priser</strong>: Prisene vises i amerikanske dollar for USA. Endelig pris kan variere basert på din region.  
  {{< /hextra/info-paragraph >}}
</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Ofte stilte spørsmål
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">
{{% details title="Hva er Evermusic?" closed="true" %}}
Evermusic er en musikkspillerapp som hjelper deg å lytte til favorittsangene dine fra forskjellige skylagringstjenester.<br>
Du kan enkelt laste ned musikk for å spille frakoblet, opprette og administrere spillelister, og bruke en innebygd equalizer for å forbedre lytteopplevelsen din.<br>
Den fungerer med tjenester som Google Drive, Dropbox, OneDrive og mer, slik at du kan ha all musikken din på ett sted og få tilgang til den fra hvilken som helst enhet.<br><br>
Appen støtter også ulike lydformater og lar deg organisere musikkbiblioteket ditt etter artist, album, sjanger og komponist.<br>
Du kan synkronisere biblioteket ditt mellom skylagringen og enheten din, slik at du alltid har favorittmelodiene dine tilgjengelig.<br>
I tillegg, med funksjoner som sømløs avspilling, crossfade og muligheten til å strømme musikk til Chromecast- og AirPlay-enheter, tilbyr Evermusic en komplett løsning for alle dine musikkbehov.
{{% /details %}}

{{% details title="Hvordan fungerer Evermusic?" closed="true" %}}
Evermusic fungerer ved å koble til ulike skylagringstjenester, som Google Drive, Dropbox, OneDrive og andre, slik at du kan få tilgang til musikkbiblioteket ditt fra hvilken som helst enhet.<br>
Når du er tilkoblet, kan du bla gjennom og strømme musikken din direkte fra skyen, eller laste ned favorittsangene, albumene og spillelistene dine for frakoblet avspilling.<br>
Appen støtter flere lydformater, noe som gjør det enkelt å spille alle musikkfiler du har lagret.<br><br>
I tillegg til skyintegrering tilbyr Evermusic kraftige funksjoner for å forbedre lytteopplevelsen din.<br>
Du kan opprette og administrere spillelister, bruke den innebygde equalizeren for å justere lydkvaliteten, og nyte sømløs avspilling med funksjoner som sømløs avspilling og crossfade.<br>
Appen støtter også strømming til enheter som Chromecast og AirPlay, slik at du kan spille musikken din på alle kompatible enheter.<br>
Med Evermusic kan du organisere musikkbiblioteket ditt etter artist, album, sjanger og komponist, og holde det synkronisert på tvers av skylagringen og enhetene dine, slik at du alltid har tilgang til favorittmelodiene dine.<br><br>
Dessuten lar Evermusic deg spille lokale filer lagret direkte på din iPhone, iPad eller Mac.<br>
Du kan enkelt importere musikk fra enhetens lagring eller fra iTunes-musikkbiblioteket.<br>
Dette betyr at du kan nyte hele musikksamlingen din, enten den er lagret i skyen eller lokalt på enheten din.<br>
Med Evermusic er alle favorittsporene dine tilgjengelige på ett praktisk sted.<br><br>

Utforsk våre veiledninger for mer informasjon:<br>
- [Hvordan spille musikk fra iCloud Drive på min iPhone eller Mac.](/docs/howto/how-to-listen-to-music-from-icloud-drive-on-your-iphone-or-mac)<br>
- [Hvordan aktivere DLNA-medieserver på Windows 10 og spille musikken din på iPhone.](/docs/howto/how-to-enable-dlna-media-server-on-windows-10-and-play-your-music-on-iphone)<br>
- [Hvordan laste opp filene mine til skylagring og koble dem til Evermusic, Flacbox, Evertag.](/docs/howto/how-to-upload-my-files-to-the-cloud-storage-and-connect-them-to-evermusic-flacbox-evertag)<br>
- [Hvordan overføre filer trådløst fra en datamaskin til en iPhone ved hjelp av WiFi-Drive.](/docs/howto/how-to-transfer-files-wirelessly-from-a-computer-to-an-iphone-using-wifi-drive)<br>
- [Hvordan koble en USB-minnepinne til iPhone og lytte til musikk eller administrere filer på den.](/docs/howto/how-to-connect-a-usb-flashcard-to-the-iphone-and-listen-to-music-or-manage-files-located-on-it)<br>
- [Hvordan spille musikk på iPhone fra WD My Cloud Home.](/docs/howto/how-to-play-music-on-iphone-from-wd-my-cloud-home)
{{% /details %}}

{{% details title="Er Evermusic gratis?" closed="true" %}}
Evermusic er en gratis applikasjon med noen begrensninger som du kan fjerne etter oppgradering til Premium-versjonen. Applikasjonen tilbyr et engangskjøp for livstid og to abonnementsalternativer (1 måned og 1 år) for å fjerne alle restriksjoner, slik at du kan velge den beste og mest optimale prisen for deg. Prisene kan variere avhengig av land eller territorium. Vær også oppmerksom på at Familiedeling er aktivert for alle kjøp og planer, slik at du kan dele Premium-versjonen med familiemedlemmer.<br><br>
Livstidskjøp og abonnementer deles mellom iOS og Mac, ved bruk av iCloud for å synkronisere denne informasjonen. Hvis du har Premium-versjonen på iOS-enheten din, sørg for at du har den nyeste versjonen installert og at iCloud er aktivert. Start appen på iOS og vent ett minutt til kjøpsinformasjonen din lastes opp til iCloud.<br><br>
[Les mer](/docs/faq/evermusic/what-is-the-difference-between-evermusic-and-evermusic-premium/)
{{% /details %}}

{{% details title="Hva er forskjellen mellom Evermusic gratis og Evermusic Premium?" closed="true" %}}
**Evermusic gratis**<br>
• Inneholder annonser: Gratisversjonen viser annonser for å generere inntekter, noe som av og til kan forstyrre musikklyttingen din.<br>
• Begrensede spillelister: Du kan opprette opptil (10) spillelister i gratisversjonen.<br>
• Begrensede skytjenester: Gratisversjonen støtter opptil (3) skytjenester.<br>
• Begrensede favoritter: Du kan legge til maksimalt (100) sanger i favorittene dine i gratisversjonen.<br>
• Spilleliste- og købegrensninger: Gratisversjonen har begrensninger på antall sanger du kan legge til i en enkelt spilleliste (1 000) og maksimalt antall sanger i spillerkøen (750).<br>
• Begrensede frakoblede funksjoner: Gratisversjonen har restriksjoner på frakoblet tilgang, inkludert kun (1) frakoblet mappe og muligheten til å laste ned album, artister, sjangre, spillelister og komponister frakoblet.<br>
• Ingen eksport av iTunes-musikksamlinger: Du kan ikke eksportere samlinger fra iTunes-musikk til lokale filer i gratisversjonen.<br>
• Begrensede taggsøkfunksjoner: Tilbyr (20) automatiske taggsøkforespørsler og (20) albumomslag-søkforespørsler per dag.<br>
• Begrenset eksport av mediesamlinger: Tillater (5) eksporter av mediesamlinger (spillelister, album, artister, sjangre) til M3U/CSV/TXT per dag.<br>
• Begrensede tilpasningsfunksjoner: Du kan bare bruke begrensede tilpasningsalternativer, og endring av appikonet er også begrenset.<br><br>

**Evermusic Premium**<br>
• Annonsefri opplevelse: Evermusic Premium fjerner annonser og gir en sømløs og uavbrutt musikklytteopplevelse.<br>
• Ubegrensede spillelister: Du kan opprette et ubegrenset antall spillelister med Evermusic Premium.<br>
• Ubegrensede skytjenester: Evermusic Premium lar deg koble til et ubegrenset antall skytjenester, som utvider tilgangen til musikkbiblioteket ditt.<br>
• Ubegrensede favoritter: Du kan legge til et ubegrenset antall sanger i favorittene dine i Evermusic Premium.<br>
• Spilleliste- og køfleksibilitet: Evermusic Premium fjerner begrensninger på spilleliste- og køstørrelse, og gir deg mer kontroll over musikkorganiseringen din.<br>
• Omfattende frakoblede funksjoner: Evermusic Premium tilbyr avanserte frakoblede funksjoner, inkludert muligheten til å laste ned mapper, album, artister, sjangre, spillelister og komponister frakoblet.<br>
• Eksport av iTunes-musikksamlinger: Du kan eksportere samlinger fra iTunes-musikk til lokale filer.<br>
• Ubegrenset taggsøk: Tilbyr ubegrensede automatiske taggsøkforespørsler.<br>
• Ubegrenset albumomslag-søk: Tilbyr ubegrensede albumomslag-søkforespørsler.<br>
• Ubegrenset eksport av mediesamlinger: Tillater ubegrensede eksporter av mediesamlinger (spillelister, album, artister, sjangre) til M3U/CSV/TXT.<br>
• Full tilpasning: Gir fulle tilpasningsalternativer, inkludert muligheten til å endre appikonet.<br><br>

[Les mer](/docs/faq/evermusic/what-is-the-difference-between-evermusic-and-evermusic-premium/)
{{% /details %}}


{{% details title="Er Evermusic trygt?" closed="true" %}}
Evermusic bruker kun offisiell SDK og sikre tilkoblinger for å samhandle med tilkoblede skytjenester. Brukernavn og passord er ikke tilgjengelig for applikasjonen. Alle forespørsler fra applikasjonen til skytjenesten er kryptert.<br>
Når du skriver inn brukernavn og passord, viser applikasjonen den offisielle autorisasjonssiden som leveres av skytjenesteleverandøren, og hele autorisasjonsprosessen skjer utenfor applikasjonen. Skytjenesteleverandøren sender et auth-token til applikasjonen etter vellykket autorisasjon, og dette tokenet brukes til å gjøre API-kall.<br><br>

Auth-token er en digital nøkkel som lar tredjepartsapplikasjoner samhandle med skylagring. Auth-token lagres på enheten din i sikker systemlagring kalt nøkkelring. Du kan laste ned filene dine fra den tilkoblede skytjenesten til enheten, og disse filene plasseres i appens «Dokumenter»-katalog. Du kan fjerne disse filene når som helst ved å bruke den innebygde filbehandleren.<br>
Applikasjonen deler ingen informasjon fra den tilkoblede skykontoen. Du kan tilbakekalle tilgang til skykontoen din når som helst ved å åpne kontoinnstillingssiden i nettleseren din.<br><br>

For å tilbakekalle auth-token, logg inn på kontoen din i nettleseren og naviger til innstillingssiden. Der finner du alle tredjepartsapper som er koblet til skykontoen din, og du kan fjerne hvilken som helst av dem hvis du ikke lenger vil bruke den applikasjonen.<br><br>

Du kan også koble fra de tilkoblede skykontoene i applikasjonen, og auth-token vil også bli fjernet fra enheten din. Hvis du fjerner applikasjonen fra enheten din, vil alle nedlastede data og tilgangstokener også bli fjernet.<br><br>

[Les mer](/docs/guide/evermusic/evermusic-guide-connections/)
{{% /details %}}

{{% details title="Hvordan lager jeg en spilleliste i Evermusic?" closed="true" %}}
- Åpne Spillelister-seksjonen.<br>
- Trykk på «+»-knappen eller «...»-knappen øverst til høyre og velg «Ny spilleliste».<br>
- Skriv inn et navn for spillelisten og trykk «Lagre». Dialogboksen «Legg til sanger» vises.<br>
- Velg sporene du vil legge til i spillelisten.<br><br>

[Les mer](/docs/guide/evermusic/evermusic-guide-playlists/)
{{% /details %}}

{{% details title="Hvilke skytjenester støtter Evermusic?" closed="true" %}}
For øyeblikket støtter applikasjonen de mest populære skytjenestene: iCloud Drive, Google Drive, Dropbox, OneDrive, Box, MEGA, Yandex.Disk, WD MyCloud Home, DLNA, MediaFire, WebDAV, SMB, pCloud, HiDrive, 百度网盘, My Cloud Home, InfiniCLOUD, Cloud Mail.ru, Put.io, MyDrive.<br><br>

[Les mer](/docs/guide/evermusic/evermusic-guide-connections/)
{{% /details %}}

{{% details title="Hvordan kan jeg bruke equalizeren?" closed="true" %}}
- Åpne lydspillerskjermen.<br>
- Trykk på «Equalizer»-ikonet nederst på skjermen.<br>
- Slå på bryterkontrollen øverst til høyre på equalizerskjermen for å aktivere equalizeren.<br>
- Velg en av de tilgjengelige forhåndsinnstillingene (rock, pop, dance, disco..) eller endre equalizerskyverne for å bygge din egen forhåndsinnstilling.<br><br>

Full veiledning tilgjengelig her:<br>
[Hvordan bruke lydequalizer på iPhone, iPad, Mac med Evermusic og Flacbox](/docs/howto/how-to-use-the-audio-equalizer-on-your-iphone-ipad-mac-with-evermusic-and-flacbox)
{{% /details %}}

{{% details title="Hvordan aktiverer jeg frakoblet modus i Evermusic?" closed="true" %}}
- Koble til en skytjeneste:<br>
 • Gå til «Tilkoblinger»-fanen.<br>
 • Velg «Koble til en skylagring» og følg instruksjonene for å koble til ønsket tjeneste.<br><br>

- Naviger til musikkmappen:<br>
 • Åpne den tilkoblede skylagringen og finn musikkmappen din.<br><br>

- Aktiver frakoblet modus:<br>
 • Trykk på «Flere handlinger»-knappen ved siden av mappenavnet.<br>
 • Velg «Aktiver frakoblet modus».<br><br>
 • Den valgte mappen og alt innholdet lastes ned til «Lokale filer» > «Frakoblede mapper».<br><br>

- Automatiske oppdateringer:<br>
 • Appen vil kontinuerlig skanne etter endringer. Nye filer lagt til i nettmappen lastes ned automatisk.<br><br>

- Konfigurer skanneintervall:<br>
 • Gå til «Innstillinger» > «Filbehandler» > «Frakoblede mapper» > «Tidsintervall» for å angi skannefrekvensen.<br><br>

- Manuell synkronisering:<br>
 • For å synkronisere manuelt, gå til «Innstillinger» > «Filbehandler» > «Frakoblede mapper» > «Synkroniserte frakoblede mapper».<br>
 • Trykk på «Flere handlinger» og velg «Start synkronisering».<br><br>

[Les mer](/docs/howto/play-offline-music-in-evermusic-flacbox-download-sync-from-cloud-to-local-files/)
{{% /details %}}

{{% details title="Hvordan spille lokalt nedlastet musikk på iPhone?" closed="true" %}}
Når du har installert applikasjonen, åpne «Lokale filer»-skjermen og bla ned til «Filer på denne iPhone»-seksjonen.<br>
Derfra velger du «Åpne filer...» hvis du trenger å velge flere lydfiler, eller «Åpne mappe...» hvis du vil velge en musikkmappe.<br>
Appen vil skanne mappens innhold, og alle funne lydfiler vil bli valgt.<br>
Naviger til musikkmappen din, trykk «Åpne» for å bekrefte valget ditt, og filene legges til i spillerkøen.<br>
Disse filene spilles direkte fra den valgte plasseringen uten å bli kopiert til applikasjonspakken.<br><br>

**Legge til en mappe i Favoritter for rask tilgang**<br>
Forenkle prosessen ved å legge til en mappe på enheten din i favorittene dine.<br>
På denne måten trenger du ikke å gjenta trinnene hver gang du vil spille musikk.<br>
Åpne «Lokale filer»-skjermen, bla til «Hurtigtilgang»-seksjonen og trykk «Favoritter» for å få tilgang til «Favorittfiler»-skjermen.<br>
Trykk på flere handlinger-knappen (tre prikker) øverst til høyre og velg «Legg til mappe».<br>
Naviger til ønsket mappe og trykk «Åpne» for å bekrefte.<br>
Mappen din legges til i «Favorittfiler», og gir rask tilgang til musikken din.<br><br>

**Importere lokale filer til musikkbiblioteket**<br>
Hvis du foretrekker å organisere dine lokale filer i musikkbiblioteket, åpne «Musikkbibliotek»-skjermen, trykk på tre prikker-knappen øverst til høyre og velg «+» (Legg til musikk).<br>
Velg «Lokale filer»-menyelementet, bla ned til «Filer på denne iPhone»-seksjonen og trykk «Åpne filer...».<br>
Velg filene du vil legge til og trykk «Åpne» for å bekrefte.<br>
Appen vil skanne de valgte filene og legge dem til i musikkbiblioteket ditt, organisert etter metadata som artist, album og sjanger.<br><br>

**Legge til lokale filer i en spilleliste**<br>
For å legge til lokale filer i en spilleliste, åpne «Spillelister»-skjermen og trykk på mer-knappen øverst til høyre.<br>
Velg «+» (Ny spilleliste), skriv inn et navn for den nye spillelisten, og på neste skjerm velger du «Lokale filer»-alternativet.<br>
Bla ned til «Filer på denne iPhone»-seksjonen og trykk «Åpne filer...».<br>
Velg lydfilene du vil legge til og trykk «Åpne» for å bekrefte.<br>
Filene legges til i spillelisten din, der du kan endre sangrekkefølgen og utføre andre handlinger ved hjelp av mer-knappen.<br><br>

Med disse enkle trinnene kan du låse opp det fulle potensialet til iPhone og Mac som de ultimate plattformene for å nyte din kjære lokale musikksamling.<br><br>

[Les mer](/docs/howto/how-to-play-local-music-stored-on-your-iphone-or-mac)
{{% /details %}}

{{% details title="Hvordan kan jeg gjenoppta en spilleliste fra der jeg slapp?" closed="true" %}}
Først, sørg for at «Lagre lydspillertilstand» er aktivert i Innstillinger > Lydspiller > Generelt.<br>
Når du bytter til en annen spilleliste og kommer tilbake, vil du se fire handlinger på toppverktøylinjen under albumomslaget: «Søk», «Fortsett avspilling», «Spill alle» og «Bland alle».<br>
Trykk «Fortsett avspilling» for å gjenoppta spillelisten fra sist lagrede tilstand og medieposisjon.<br>
{{% /details %}}

{{% details title="Hvordan vise sangtekster i Evermusic?" closed="true" %}}
Du kan vise innebygde sangtekster for spor i Evermusic-appen ved å følge disse trinnene:<br>
1. Start avspilling av en lydfil ved å trykke på den.<br>
2. Åpne fullskjerms lydspiller.<br>
3. Trykk på «Flere handlinger»-knappen øverst til høyre for å se menyen for flere handlinger.<br>
4. Velg «Vis kommentarer»-menyelementet.<br>
5. På sporkommentarskjermen har du tre moduser som du kan bytte mellom ved å sveipe til høyre: «Kommentarer», «Innebygde sangtekster» og «LRC-fil».<br><br>

**Moduser**:<br>
1. «Kommentarmodus»: Viser innholdet fra KOMMENTAR-feltet i lydfilen din.<br>
2. «Sangtekstmodus»: Viser de innebygde sangtekstene fra lydfilen din. Hvis lydfilen din har sangtekster i taggene, vises de her. Denne modusen støtter også tidsstyrt tekst i LRC-format. For å redigere sangteksttaggen, bruk en ekstern lydtagredigerer som Evertag. Du kan finne sangtekster i LRC-format gratis på nettsteder som Lyricsify. Kopier de nødvendige sangtekstene til taggene dine og lagre dem for å vise dem senere på denne skjermen.<br>
3. «LRC-filmodus»: I stedet for å redigere lydfiler kan du plassere en LRC-fil i samme mappe som den originale lydfilen. Begge filene bør ha samme navn, men forskjellige filendelser. Når du sveiper til tredje side på Kommentarer-skjermen, søker appen etter LRC-filen i samme katalog og viser innholdet.<br><br>

[Les mer](/docs/howto/how-to-add-and-view-comments-to-your-audio-tracks-on-iphone-ipad-and-mac-with-evermusic-and-flacbox)
{{% /details %}}

{{% details title="Hvordan overfører jeg musikk til Evermusic fra datamaskinen min?" closed="true" %}}
Du kan koble til datamaskinen eller personlig NAS ved hjelp av SMB-, WebDAV- eller DLNA-protokoller. Alternativt kan du bruke iTunes fildeling for å overføre musikk.<br><br>

For å koble til en datamaskin ved hjelp av **SMB**-protokollen, trykk «Koble til en skytjeneste» → SMB.<br>
Skriv inn datamaskinens IP-adresse og delt mappenavn i URL-feltet med formatet smb://datamaskin-ip-adresse/delt-mappenavn,<br>
skriv inn brukernavn og passord og trykk «Ferdig».<br>
Hvis tilkoblingen er vellykket, vil du se tilkoblet lagring i «Skytjenester»-seksjonen.<br><br>

En full veiledning er tilgjengelig her:<br>
[Overfør filene dine fra datamaskinen til iPhone ved hjelp av SMB-protokollen](/docs/howto/transfer-your-files-from-the-computer-to-iphone-using-smb-protocol/)<br><br>

For **WebDAV**-protokollen er alle trinnene de samme bortsett fra URL-feltet.<br>
URL-en skal være i formatet http://server-navn eller https://server-navn hvis serveren støtter SSL.<br><br>

En full veiledning er tilgjengelig her:<br>
[Hvordan koble til NAS-lagring med WebDAV og lytte til musikk på iPhone eller Mac](/docs/howto/how-to-connect-nas-storage-using-webdav-and-listen-to-music-on-your-iphone-or-mac)<br><br>

**Wi-Fi Drive** er en populær teknologi som lar deg overføre filer fra datamaskinen til en iOS-enhet trådløst ved hjelp av en nettleser på skrivebordet.<br>
For å bruke denne funksjonen må enheten og datamaskinen være koblet til det samme Wi-Fi-nettverket.<br>
Åpne «Tilkoblinger» → «Datamaskin» → «Koble til med Wi-Fi» og aktiver serveren.<br>
Åpne deretter en nettleser på skrivebordet og skriv inn URL-en fra applikasjonen.<br>
Du kan dra og slippe filer fra datamaskinen til den åpnede nettsiden, og de vil vises på enheten.<br><br>

Mer detaljert veiledning tilgjengelig her:<br>
[Hvordan overføre filer trådløst fra en datamaskin til en iPhone ved hjelp av WiFi-Drive](/docs/howto/how-to-transfer-files-wirelessly-from-a-computer-to-an-iphone-using-wifi-drive)<br><br>

**iTunes fildeling** er en annen teknologi som lar deg overføre filer fra datamaskinen til enheten ved hjelp av iTunes og en lightning-kabel.<br>
Bare koble enheten til datamaskinen med en kabel og start iTunes.<br>
Åpne iTunes → «Applikasjoner-seksjonen» → og finn Evermusic.<br>
Trykk på appikonet for å se en delt mappe.<br>
Kopier filer fra datamaskinen til den delte mappen på enheten.<br><br>

Detaljert instruksjon tilgjengelig her:<br>
[Hvordan spille lokale filer (iTunes-filer) på min iPhone](/docs/howto/how-to-play-local-itunes-files-on-my-iphone)<br><br>

Med **DLNA** kan du også sette opp en DLNA-medieserver og strømme musikken din fra Windows-PC som beskrevet her:<br>
[Hvordan aktivere DLNA-medieserver på Windows 10 og spille musikken din på iPhone](/docs/howto/how-to-enable-dlna-media-server-on-windows-10-and-play-your-music-on-iphone)
{{% /details %}}

{{% details title="Hvordan laste ned musikk?" closed="true" %}}
Før du kan laste ned musikk og lytte til musikken din frakoblet, må du koble til en nettverkskonto.<br>
Bare åpne «Tilkoblinger»-skjermen og legg til kontoen din.<br>
Når du har lagt til en nettverkskonto, kan du laste ned musikken din fra skyen.<br><br>

**For å laste ned musikk fra skyen**<br>
– Åpne den tilkoblede skytjenesten.<br>
– Naviger inn i mappen du vil laste ned.<br>
– Trykk på flere handlinger-knappen «...» øverst til høyre og velg «Velg»-menyelementet.<br>
– Velg filene du vil laste ned og trykk på «Last ned»-knappen.<br><br>

**For å aktivere frakoblet modus for artist/spilleliste/album/sjanger**<br>
– Åpne artister/album/spillelister/sjanger-skjermen<br>
– Trykk på «Frakoblet modus»-avmerkingsboksen<br>
– Frakoblet artist/album/spilleliste/sjanger vises i «Frakoblet musikk»-seksjonen i musikkbiblioteket<br><br>

Mer detaljert instruksjon om frakoblet modus tilgjengelig her:<br>
[Spill frakoblet musikk i Evermusic og Flacbox: Last ned og synkroniser fra sky til lokale filer](/docs/howto/play-offline-music-in-evermusic-flacbox-download-sync-from-cloud-to-local-files/)<br><br>

Et annet tilgjengelig alternativ er å laste ned musikk fra YouTube og importere den i Evermusic som beskrevet her:<br>
[Hvordan laste ned musikk fra YouTube og lytte til frakoblet musikk på iPhone](/docs/howto/how-to-download-music-from-youtube-and-listen-to-offline-music-on-iphone)
{{% /details %}}

{{% details title="Støtter Evermusic Apple CarPlay?" closed="true" %}}
Ja, **Evermusic støtter Apple CarPlay fullt ut**. Du kan bla gjennom musikkbiblioteket ditt, spille lokale eller frakoblede filer, koble til skylagring og kontrollere avspilling direkte fra bilens infotainment-skjerm.

CarPlay-grensesnittet inkluderer dedikerte faner for **Bibliotek**, **Tilkoblinger**, **Lokale filer** og **Innstillinger**, som gir deg full kontroll over musikken din på veien. Avspillingskontroller, tilfeldig rekkefølge, gjenta og købehandling er også tilgjengelig.

For å bruke CarPlay, sørg for at Siri er aktivert og at iPhone er koblet til via USB eller trådløst.

[Les hele guiden](/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/)
{{% /details %}}

{{% details title="Hvilke lydformater støtter Evermusic?" closed="true" %}}
Her er den fullstendige listen over støttede lydformater og tilhørende filendelser:<br><br>

**Støttede lydformater:**<br>
• AAC (MPEG-4 Advanced Audio Coding)<br>
• ALAC (Apple Lossless)<br>
• AMR (Adaptive Multi-Rate)<br>
• HE-AAC (MPEG-4 High-Efficiency AAC)<br>
• iLBC (Internet Low Bitrate Codec)<br>
• Linear PCM (Uncompressed, Linear Pulse Code Modulation)<br>
• MP3 (MPEG-1 Audio Layer 3)<br>
• μ-law and A-law<br>
• IMA4 (IMA/ADPCM)<br><br>

**Støttede lydfilendelser:**<br>
mpeg, aifc, 3gp, avi, aif, latm, 3gpp, m4a, loas, cdda, aac, m4p, m4b, ac3, pls, mp4v, m3u, m4r, aiff, xhe, mp1, snd, mp2, wav, qt, wave, m3u8, m4v, mp3, 3g2, caf, mp4, flac, au, w64, ec3, adts, amr, vtt, mpa, aa<br><br>

Med dette brede utvalget av støttede formater og filendelser kan du nyte musikken din i formatet du foretrekker.
{{% /details %}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Brukerveiledning
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

<div class="hx:w-full hx:mx-auto hx:flex hx:flex-col hx:items-center hx:text-center">
  {{< hextra/info-paragraph border="false" >}}
  Denne guiden hjelper deg å få mest mulig ut av Evermusic på din iPhone, iPad eller Mac. Lær hvordan du strømmer musikk fra skyen, administrerer lydbøkene dine og flytter musikk mellom enheter. Evermusic gir deg full kontroll over musikksamlingen din i én enkel app.
  {{< /hextra/info-paragraph >}}
</div>

{{< cards >}}
  {{< card icon="location-marker" title="Navigasjon" subtitle="Lær hvordan du navigerer i Evermusic ved hjelp av fanelinjen på iPhone eller venstremenyen på iPad og Mac." link="/docs/guide/evermusic/evermusic-guide-navigation" >}}

  {{< card icon="cloud" title="Tilkoblinger" subtitle="Koble til skykontoene dine og administrer nettfiler ved hjelp av den innebygde filbehandleren." link="/docs/guide/evermusic/evermusic-guide-connections" >}}

  {{< card icon="library" title="Musikkbibliotek" subtitle="Organiser og utforsk sporene, albumene og artistene dine i musikkbiblioteket." link="/docs/guide/evermusic/evermusic-guide-music-library" >}}

  {{< card icon="collection" title="Spillelister" subtitle="Opprett og ordne spillelister som passer til humøret eller anledningen din." link="/docs/guide/evermusic/evermusic-guide-playlists" >}}

  {{< card icon="folder" title="Lokale filer" subtitle="Få tilgang til og administrer frakoblet musikk gjennom Lokale filer-seksjonen." link="/docs/guide/evermusic/evermusic-guide-local-files" >}}

  {{< card icon="play" title="Lydspiller" subtitle="Kontroller avspilling, kø og lydinnstillinger som equalizer og innsovningstimer." link="/docs/guide/evermusic/evermusic-guide-player" >}}

  {{< card icon="adjustments" title="Innstillinger" subtitle="Tilpass Evermusics utseende, funksjoner og ytelsesinnstillinger." link="/docs/guide/evermusic/evermusic-guide-settings" >}}

{{< /cards >}}

</div>

<div class="hx:mt-6"></div>

<!-- </body> -->
