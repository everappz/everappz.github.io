---
date: '2025-06-12T17:00:00+00:00'
draft: false
title: 'Evermusic - Offline musikafspiller til iPhone og Mac'
layout: hextra-home
description: "Download den bedste gratis offline musikafspiller til din iPhone og Mac og lyt til din musik overalt."
keywords: ["Evermusic", "offline musikafspiller", "iOS lydafspiller", "cloud musik app", "download musik iPhone", "Mac musikafspiller", "streaming musik offline", "FLAC afspiller iPhone", "multi-cloud musikafspiller", "Evermusic app funktioner"]
tags: ["Evermusic", "offline afspilning", "cloud musik", "MP3 afspiller", "iPhone apps", "lyd streaming", "musikstyring", "Mac apps", "medieafspiller"]
aliases:
  - /evermusic/
appStoreUrl: "https://apps.apple.com/app/apple-store/id885367198?pt=95781850&ct=everappzcom&mt=8"
appStoreId: "885367198"
features: "Offline musikafspilning, Cloud streaming (iCloud, Google Drive, Dropbox, OneDrive, MEGA, Box), Lydequalizer med forudindstillinger, Crossfade og pauseløs afspilning, M3U playliste import/eksport, Sangtekstvisning, Lydbogmærker, Apple CarPlay, AirPlay og Chromecast, SMB/WebDAV/DLNA support, USB-flashdrev afspilning, Last.fm scrobbling"
screenshots:
  - "https://everappz.com/products/evermusic/screenshots/2048x2732/3.png"
  - "https://everappz.com/products/evermusic/screenshots/2048x2732/4.png"
  - "https://everappz.com/products/evermusic/screenshots/2048x2732/5.png"
  - "https://everappz.com/products/evermusic/screenshots/2048x2732/6.png"
---

{{% sr-only %}}
Evermusic er en gratis offline musikafspiller til iPhone og Mac, udviklet af Everappz, en spansk softwarevirksomhed. Med over 11 millioner downloads verden over og en vurdering på 4,6 stjerner fra mere end 18.000 App Store-anmeldelser er Evermusic en af de mest populære tredjeparts musikafspillere på iOS. Appen forbinder til over 30 cloud-lagringstjenester, herunder iCloud Drive, Google Drive, Dropbox, OneDrive, MEGA, Box, pCloud og Yandex.Disk, hvilket giver brugerne mulighed for at streame deres personlige musikbibliotek direkte fra skyen eller downloade numre til offline lytning. Evermusic understøtter en bred vifte af lydformater, herunder MP3, FLAC, AAC, ALAC, WAV, AIFF, OGG, OPUS, WMA, APE og DSD. Nøglefunktioner inkluderer en 10-bånds lydequilizer med forudindstillinger, crossfade og gapless afspilning, M3U-playliste import og eksport, visning af sangtekster, lydbogmærker, Apple CarPlay-integration, AirPlay- og Chromecast-streaming samt Last.fm-scrobbling. Appen understøtter også lokal netværksstreaming via SMB-, WebDAV- og DLNA-protokoller samt afspilning fra USB-flashdrev via Lightning- eller USB-C-adaptere. Evermusic er tilgængelig som gratis download på App Store med valgfrie køb i appen, herunder et månedligt abonnement til $4.99, et årligt abonnement til $19.99 eller et engangskøb med livstidsadgang til $59.99. Appen blev først udgivet i 2014 og vedligeholdes aktivt med regelmæssige opdateringer.
{{% /sr-only %}}


<!-- <body class="hx:bg-transparent" style="background: radial-gradient(ellipse at 50% 80%, rgba(59,130,246,0.05), hsla(0,0%,100%,0));"> -->

{{< force-dark >}}

{{< hextra/hero-container
  image="/products/evermusic/heroimage/hero_1200.webp"
  imageWidth="600"
  imageCard="true"
>}}

<div class="hx:flex hx:flex-col hx:items-center hx:justify-center hx:sm:block">

<div class="hx:mt-4 hx:mr-4">
{{< hextra/hero-badge link="../../blog/evermusic-celebrates-11-million-worldwide-downloads-a-musical-milestone" >}}
  <div class="hx:w-2 hx:h-2 hx:rounded-full hx:bg-primary-400"></div>
  <span>11 millioner downloads</span>
    {{< icon name="arrow-circle-right" attributes="height=14" >}}
{{< /hextra/hero-badge >}}
</div>

<div class="hx:mt-6 hx:mb-6">
<div class="hx:flex hx:gap-4 hx:items-center">
  <img src="/images/app_icons/webp/Evermusic_Icon-App-1024x1024.webp" alt="Evermusic Icon" loading="lazy" class="hero-headline-icon" />
  {{< hextra/hero-headline >}}
  Evermusic
  {{< /hextra/hero-headline >}}
</div>
</div>

<div class="hx:mb-6">
{{< hextra/hero-centered-subtitle >}}
<a href="https://www.chip.de/downloads/Evermusic-Pro-iPhone-_-iPad-App_91614216.html" target="_blank" rel="noopener">
  Er den perfekte løsning til at organisere og afspille&nbsp;<br class="hx:sm:block hx:hidden" />din egen musik fra skyen&nbsp;<br class="hx:sm:block hx:hidden" /><strong>chip.de</strong>
  </span>
{{< /hextra/hero-centered-subtitle >}}
</div>

<div class="hx:mb-6">
{{< hextra/hero-paragraph >}}
• Afspil musik med crossfade, pauseløs afspilning og equalizer  
• Importér M3U-playlister og download sange til offline lytning  
• Stream musik fra cloudtjenester, NAS, computer eller USB-flashdrev  
• Se sangtekster mens du lytter og tilføj lydbogmærker for at genoptage når som helst  
{{< /hextra/hero-paragraph >}}
</div>

{{< hextra/hero-button 
  text="Download til mobil"
  link="https://apps.apple.com/app/apple-store/id885367198?pt=95781850&ct=everappzcom&mt=8"
  icon="device-mobile" 
  style="margin-top:1rem;margin-right:1rem;"
>}}

{{< hextra/hero-button 
  text="Download til desktop"
  link="https://apps.apple.com/app/apple-store/id1564384601?pt=95781850&ct=everappzcom&mt=8"
  icon="desktop-computer" 
  style="margin-top:1rem;margin-right:1rem;"
>}}

</div>

{{< /hextra/hero-container >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

{{< hextra/feature-grid >}}

  {{< hextra/feature-card
    title="Spar plads på enheden"
    subtitle="Har du 100 GB musik samlet gennem årene, men kun 32 GB lagerplads på iPhone? Bare flyt alle dine numre til skyen og frigør værdifuld plads til fotos, videoer og apps."
    icon="folder"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(59,130,246,0.15), hsla(0,0%,100%,0));"
  >}}

  {{< hextra/feature-card
    title="Øjeblikkelig adgang til numre"
    subtitle="Med understøttelse af de mest populære cloudtjenester lader Evermusic dig vælge en eller flere tjenester og streame din musik fra skyen. Ingen mere iTunes-synkronisering, dine lydfiler uploades til skyen fra PC eller Mac ligesom andre filer."
    icon="collection"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(192,38,211,0.15), hsla(0,0%,100%,0));"
  >}}

  {{< hextra/feature-card
    title="Offline tilstand"
    subtitle="Offline tilstand lader dig downloade favoritalum, numre, genrer og playlister til offline afspilning og lytte til musik, når du ikke er forbundet til internettet."
    icon="download"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(234,88,12,0.15), hsla(0,0%,100%,0));"
  >}}

  {{< hextra/feature-card
    title="Din musik i skyen"
    subtitle="Opret din egen avancerede musikstreamingtjeneste gratis! Administrer dit massive musikbibliotek, spar enhedslagring og stream favoritnumre direkte fra skyen med smart buffering og pauseløs afspilning. Nyd understøttelse af populære cloudtjenester som iCloud, Google Drive, Dropbox, MEGA.nz, Box, OneDrive og mere."
    icon="cloud"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(34,197,94,0.15), hsla(0,0%,100%,0));"
  >}}

  {{< hextra/feature-card
    title="Overfør filer nemt"
    subtitle="Tilslut din Mac eller PC og stream musik direkte fra din hjemmecomputer. Overfør lydfiler problemfrit mellem computeren og din iOS-enhed ved hjælp af Wi-Fi Drive eller iTunes fildeling. Du kan også tilslutte din NAS eller et USB-flashdrev og få adgang til dit bibliotek fra hvor som helst."
    icon="duplicate"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(251,191,36,0.15), hsla(0,0%,100%,0));"
  >}}

  {{< hextra/feature-card
    title="Automatisk synkronisering"
    subtitle="Synkroniseringsadministratoren scanner automatisk dit cloudlager for lydfiler og opdaterer dit musikbibliotek. Metadataindlæseren organiserer numre efter album, kunstnere og genrer ved hjælp af indlejrede lydtags."
    icon="refresh"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(6,182,212,0.15), hsla(0,0%,100%,0));"
  >}}

{{< /hextra/feature-grid >}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Rent og enkelt design
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

{{< cards cols="4">}}

  {{< card 
    icon="adjustments"
    image="/products/evermusic/screenshots/2048x2732/3.png" 
    title="Lydequalizer" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Finjuster din lyd med en iPod-lignende lydequalizer, tilpasselige forudindstillinger og forforstærkerforstærkning for den bedste lytteoplevelse." 
  >}}

  {{< card 
    icon="annotation"
    image="/products/evermusic/screenshots/2048x2732/4.png"  
    title="Sangtekstvisning" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Læs indlejrede sangtekster og nummerkommentarer mens du lytter. Nyd synkroniserede sangtekster for en mere fordybende musikoplevelse." 
  >}}

  {{< card 
    icon="collection"
    image="/products/evermusic/screenshots/2048x2732/5.png"  
    title="Playlisteadministrator" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Opret og organiser tilpassede playlister, omarranger sange, eksportér til M3U eller arkivér dem som ZIP-filer for nem deling eller sikkerhedskopiering." 
  >}}

  {{< card 
    icon="cloud"
    image="/products/evermusic/screenshots/2048x2732/6.png"  
    title="Cloud musikstreaming" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Tilslut de bedste cloudlagringsplatforme som Google Drive, Dropbox og OneDrive for at streame din musiksamling når som helst, hvor som helst." 
  >}}

  {{< card 
    icon="duplicate"
    image="/products/evermusic/screenshots/2048x2732/9.png"  
    title="Filadministrator" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Administrer dine lydfiler nemt – omdøb numre, organiser mapper og overfør musik mellem enheder ved hjælp af indbyggede værktøjer." 
  >}} 

  {{< card 
    icon="sun"
    image="/products/evermusic/screenshots/2048x2732/10.png"  
    title="App-tilpasning" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Tilpas afspilleren til din stil: skift temaer, juster layout og konfigurer funktioner for en virkelig personlig musikapp-oplevelse." 
  >}}

{{< /cards >}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Komplet funktionssæt
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

{{< cards >}}

  {{< card title="Lydformater" subtitle="Understøtter: mp3, m4a, flac, wav, aac, mp4, m3u, m3u8, m4b, m4p, aiff, ogg, mp2, ac3, wma og mere." icon="music-note" >}}

  {{< card title="Lydbøger" subtitle="Inkluderer lydbogmærker, afspilningshastighedskontrol og genoptagelsesposition." icon="book-open" width="50">}}

  {{< card title="Automatisk synkronisering" subtitle="Synkroniserer dit musikbibliotek med skyen, organiseret efter kunstner, album og genre." icon="refresh" >}}

  {{< card title="Filadministrator" subtitle="Administrer filer med operationer som kopiér, flyt, omdøb og slet." icon="folder" >}}

  {{< card title="Cloudtjenester" subtitle="Understøtter iCloud, Google Drive, Dropbox, OneDrive, Box, MEGA, WD MyCloud Home, pCloud og mere." icon="cloud" >}}

  {{< card title="Crossfade-afspilning" subtitle="Næste sang begynder før den nuværende slutter for problemfri overgange." icon="switch-horizontal" >}}

  {{< card title="Pauseløs afspilning" subtitle="Ingen pauser mellem sange – ideelt til live, klassiske eller konceptalbum." icon="volume-up" >}}

  {{< card title="Lydequalizer" subtitle="Tilpasselig equalizer med forudindstillinger, manuel kontrol og forforstærkerforstærkning." icon="adjustments" >}}

  {{< card title="Avanceret søgning" subtitle="Find dine favoritalum, kunstnere og numre hurtigt og nemt." icon="search" >}}

  {{< card title="Computeradgang" subtitle="Tilslut via SMB, WebDAV eller DLNA. Overfør via iTunes fildeling og Wi-Fi." icon="desktop-computer" >}}

  {{< card title="AirPlay og Chromecast" subtitle="Stream til Apple TV og Google Chromecast-enheder trådløst." icon="device-mobile" >}}

  {{< card title="Playlister" subtitle="Opret, rediger, omarranger og eksportér/importér M3U-playlister med offline tilstand." icon="collection" >}}

  {{< card title="USB-flashkort" subtitle="Afspil musik direkte fra SD-kort ved hjælp af eksterne læsere som SanDisk iXpand." icon="inbox" >}}

  {{< card title="Offline tilstand" subtitle="Download album og playlister eller auto-cache nyligt afspillede numre til offline lytning." icon="download" >}}

  {{< card title="Apple CarPlay" subtitle="Kør sikkert med en dedikeret CarPlay-grænseflade til musikafspilning." icon="truck" >}}

  {{< card title="ID3-tagredigering" subtitle="Rediger beskadigede eller manglende metadata med den indbyggede tagredigering." icon="pencil-alt" >}}

  {{< card title="Sangtekster og kommentarer" subtitle="Se indlejrede sangtekster og kommentarer mens du lytter til din musik." icon="annotation" >}}

  {{< card title="Last.fm" subtitle="Tilslut din Last.fm-konto for at få personlige lyttestatistikker og anbefalinger." icon="chart-bar" >}}
  
{{< /cards >}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  App Store anmeldelser
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">
{{< appstore-reviews apps="885367198,1564384601" stars="5,4" >}}
</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Prisplaner
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">
{{< hextra/feature-grid cols="4">}}

  {{< hextra/feature-card
    title="Gratis"
    class="pricing-card pricing-free"
    subtitle=`<span class='pricing-details'>• Indeholder annoncer<br />
• Playlister (10)<br />
• Cloudtjenester (3)<br />
• Arkivér mediesamlinger (3)<br />
• Favoritter (1000)<br />
• Sange i playliste (1000)<br />
• Sange i kø (750)<br />
• Offline mapper (1)<br />
• Offline album (1)<br />
• Automatisk tagsøgning (20)<br />
• Eksportér til M3U (5)<br />
• Download filer<br />
• Eksportér iTunes-sange</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    <strong>Gratis</strong>
  </span>
</span>`
  >}}

  {{< hextra/feature-card
    title="Premium månedlig"
    class="pricing-card pricing-subscription"
    subtitle=`<span class='pricing-details'>• Annoncefri oplevelse<br />
• Ubegrænsede playlister<br />
• Ubegrænsede cloudtjenester<br />
• Ubegrænset mediearkivering<br />
• Ubegrænsede favoritter<br />
• Ubegrænsede sange i playliste<br />
• Ubegrænsede sange i kø<br />
• Ubegrænsede offline mapper<br />
• Ubegrænsede offline album<br />
• Ubegrænset tagsøgning<br />
• Ubegrænset eksport til M3U<br />
• Download filer<br />
• Eksportér iTunes-sange<br />
• Eksportér iTunes-samlinger<br />
• Download mapper<br />
• Download samlinger<br />
• Fuld tilpasning</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    <strong>$1.99</strong>/måned
  </span>
</span>`
  >}}

  {{< hextra/feature-card
    title="Premium årlig"
    class="pricing-card pricing-subscription"
    subtitle=`<span class='pricing-details'>• Annoncefri oplevelse<br />
• Ubegrænsede playlister<br />
• Ubegrænsede cloudtjenester<br />
• Ubegrænset mediearkivering<br />
• Ubegrænsede favoritter<br />
• Ubegrænsede sange i playliste<br />
• Ubegrænsede sange i kø<br />
• Ubegrænsede offline mapper<br />
• Ubegrænsede offline album<br />
• Ubegrænset tagsøgning<br />
• Ubegrænset eksport til M3U<br />
• Download filer<br />
• Eksportér iTunes-sange<br />
• Eksportér iTunes-samlinger<br />
• Download mapper<br />
• Download samlinger<br />
• Fuld tilpasning</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    7 dage gratis, derefter <strong>$12.99</strong>/år
  </span>
</span>`
  >}}

  {{< hextra/feature-card
    title="Premium livstid"
    class="pricing-card pricing-lifetime"
    subtitle=`<span class='pricing-details'>• Annoncefri oplevelse<br />
• Ubegrænsede playlister<br />
• Ubegrænsede cloudtjenester<br />
• Ubegrænset mediearkivering<br />
• Ubegrænsede favoritter<br />
• Ubegrænsede sange i playliste<br />
• Ubegrænsede sange i kø<br />
• Ubegrænsede offline mapper<br />
• Ubegrænsede offline album<br />
• Ubegrænset tagsøgning<br />
• Ubegrænset eksport til M3U<br />
• Download filer<br />
• Eksportér iTunes-sange<br />
• Eksportér iTunes-samlinger<br />
• Download mapper<br />
• Download samlinger<br />
• Fuld tilpasning</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    <strong>$59.99</strong>/livstid
  </span>
</span>`
  >}}

{{< /hextra/feature-grid >}}

</div>

<div class="hx:mt-6"></div>

<div class="hx:w-full hx:mx-auto hx:flex hx:flex-col hx:items-center">
  {{< hextra/info-paragraph border="true" >}}
   <strong>Familiedeling</strong>: Alle køb og abonnementer understøtter Familiedeling, så du kan dele Premium-adgang med din familie.<br><strong>Universel adgang</strong>: Livstids- og abonnementsplaner deles mellem iOS- og Mac-enheder ved hjælp af iCloud-synkronisering.<br><strong>Priser</strong>: Priserne vises i amerikanske dollars for USA. Den endelige pris kan variere baseret på din region.  
  {{< /hextra/info-paragraph >}}
</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Ofte stillede spørgsmål
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">
{{% details title="Hvad er Evermusic?" closed="true" %}}
Evermusic er en musikafspiller-app, der hjælper dig med at lytte til dine yndlingssange fra forskellige cloudlagringstjenester.<br>
Du kan nemt downloade musik til offline afspilning, oprette og administrere playlister og bruge en indbygget equalizer til at forbedre din lytteoplevelse.<br>
Den fungerer med tjenester som Google Drive, Dropbox, OneDrive og mere, så du kan have al din musik ét sted og få adgang til den fra enhver enhed.<br><br>
Appen understøtter også forskellige lydformater og lader dig organisere dit musikbibliotek efter kunstner, album, genre og komponist.<br>
Du kan synkronisere dit bibliotek mellem dit cloudlager og din enhed, så du altid har dine yndlingsmelodier tilgængelige.<br>
Derudover, med funktioner som pauseløs afspilning, crossfade og muligheden for at streame musik til Chromecast- og AirPlay-enheder, tilbyder Evermusic en komplet løsning til alle dine musikbehov.
{{% /details %}}

{{% details title="Hvordan fungerer Evermusic?" closed="true" %}}
Evermusic fungerer ved at forbinde til forskellige cloudlagringstjenester, såsom Google Drive, Dropbox, OneDrive og andre, hvilket giver dig adgang til dit musikbibliotek fra enhver enhed.<br>
Når du er tilsluttet, kan du gennemse og streame din musik direkte fra skyen eller downloade dine yndlingssange, album og playlister til offline afspilning.<br>
Appen understøtter flere lydformater, hvilket gør det nemt at afspille enhver musikfil du har gemt.<br><br>
Ud over cloudintegration tilbyder Evermusic kraftfulde funktioner til at forbedre din lytteoplevelse.<br>
Du kan oprette og administrere playlister, bruge den indbyggede equalizer til at justere lydkvaliteten og nyde problemfri afspilning med funktioner som pauseløs afspilning og crossfade.<br>
Appen understøtter også streaming til enheder som Chromecast og AirPlay, så du kan afspille din musik på enhver kompatibel enhed.<br>
Med Evermusic kan du organisere dit musikbibliotek efter kunstner, album, genre og komponist og holde det synkroniseret på tværs af dit cloudlager og enheder, så du altid har adgang til dine yndlingsmelodier.<br><br>
Derudover lader Evermusic dig afspille lokale filer gemt direkte på din iPhone, iPad eller Mac.<br>
Du kan nemt importere musik fra din enheds lagerplads eller fra iTunes-musikbiblioteket.<br>
Det betyder, at du kan nyde hele din musiksamling, uanset om den er gemt i skyen eller lokalt på din enhed.<br>
Med Evermusic er alle dine yndlingsnumre tilgængelige ét praktisk sted.<br><br>

Udforsk vores vejledninger for flere detaljer:<br>
- [Sådan afspiller du musik fra iCloud Drive på din iPhone eller Mac.](/docs/howto/how-to-listen-to-music-from-icloud-drive-on-your-iphone-or-mac)<br>
- [Sådan aktiverer du DLNA-medieserver på Windows 10 og afspiller din musik på iPhone.](/docs/howto/how-to-enable-dlna-media-server-on-windows-10-and-play-your-music-on-iphone)<br>
- [Sådan uploader du dine filer til cloudlageret og forbinder dem til Evermusic, Flacbox, Evertag.](/docs/howto/how-to-upload-my-files-to-the-cloud-storage-and-connect-them-to-evermusic-flacbox-evertag)<br>
- [Sådan overfører du filer trådløst fra en computer til en iPhone ved hjælp af WiFi-Drive.](/docs/howto/how-to-transfer-files-wirelessly-from-a-computer-to-an-iphone-using-wifi-drive)<br>
- [Sådan tilslutter du et USB-flashkort til iPhone og lytter til musik eller administrerer filer på det.](/docs/howto/how-to-connect-a-usb-flashcard-to-the-iphone-and-listen-to-music-or-manage-files-located-on-it)<br>
- [Sådan afspiller du musik på iPhone fra WD My Cloud Home.](/docs/howto/how-to-play-music-on-iphone-from-wd-my-cloud-home)
{{% /details %}}

{{% details title="Er Evermusic gratis?" closed="true" %}}
Evermusic er en gratis applikation med nogle begrænsninger, som du kan fjerne efter opgradering til Premium-versionen. Applikationen tilbyder et engangskøb for livstid og to abonnementsmuligheder (1 måned og 1 år) for at fjerne alle restriktioner, så du kan vælge den bedste og mest optimale pris for dig. Priserne kan variere afhængigt af dit land eller territorium. Bemærk også, at Familiedeling er aktiveret for alle køb og planer, så du kan dele Premium-versionen med medlemmer af din familie.<br><br>
Livstidskøb og abonnementer deles mellem iOS og Mac ved hjælp af iCloud til at synkronisere denne information. Hvis du har Premium-versionen på din iOS-enhed, skal du sikre dig, at du har den nyeste version installeret, og at iCloud er aktiveret. Start appen på iOS og vent et minut, indtil din købsinformation uploades til iCloud.<br><br>
[Læs mere](/docs/faq/evermusic/what-is-the-difference-between-evermusic-and-evermusic-premium/)
{{% /details %}}

{{% details title="Hvad er forskellen mellem Evermusic gratis og Evermusic Premium?" closed="true" %}}
**Evermusic gratis**<br>
• Indeholder annoncer: Gratisversionen viser annoncer for at generere indtægter, hvilket lejlighedsvis kan forstyrre din musikoplevelse.<br>
• Begrænsede playlister: Du kan oprette op til (10) playlister i gratisversionen.<br>
• Begrænsede cloudtjenester: Gratisversionen understøtter op til (3) cloudtjenester.<br>
• Begrænsede favoritter: Du kan tilføje maksimalt (100) sange til dine favoritter i gratisversionen.<br>
• Playliste- og købegrænsninger: Gratisversionen har begrænsninger på antallet af sange, du kan tilføje til en enkelt playliste (1.000), og det maksimale antal sange i afspillerkøen (750).<br>
• Begrænsede offline funktioner: Gratisversionen har restriktioner på offline adgang, herunder kun (1) offline mappe og muligheden for at downloade album, kunstnere, genrer, playlister og komponister offline.<br>
• Ingen eksport af iTunes-musiksamlinger: Du kan ikke eksportere samlinger fra iTunes-musik til lokale filer i gratisversionen.<br>
• Begrænsede tagsøgningsfunktioner: Tilbyder (20) automatiske tagsøgningsforespørgsler og (20) albumomslagssøgningsforespørgsler pr. dag.<br>
• Begrænset eksport af mediesamlinger: Tillader (5) eksporter af mediesamlinger (playlister, album, kunstnere, genrer) til M3U/CSV/TXT pr. dag.<br>
• Begrænsede tilpasningsfunktioner: Du kan kun bruge begrænsede tilpasningsmuligheder, og ændring af appikonet er også begrænset.<br><br>

**Evermusic Premium**<br>
• Annoncefri oplevelse: Evermusic Premium fjerner annoncer og giver en problemfri og uafbrudt musikoplevelse.<br>
• Ubegrænsede playlister: Du kan oprette et ubegrænset antal playlister med Evermusic Premium.<br>
• Ubegrænsede cloudtjenester: Evermusic Premium lader dig oprette forbindelse til et ubegrænset antal cloudtjenester, hvilket udvider din adgang til musikbiblioteket.<br>
• Ubegrænsede favoritter: Du kan tilføje et ubegrænset antal sange til dine favoritter i Evermusic Premium.<br>
• Playliste- og køfleksibilitet: Evermusic Premium fjerner begrænsninger på playliste- og køstørrelse, hvilket giver dig mere kontrol over din musikorganisering.<br>
• Omfattende offline funktioner: Evermusic Premium tilbyder avancerede offline muligheder, herunder muligheden for at downloade mapper, album, kunstnere, genrer, playlister og komponister offline.<br>
• Eksport af iTunes-musiksamlinger: Du kan eksportere samlinger fra iTunes-musik til lokale filer.<br>
• Ubegrænset tagsøgning: Tilbyder ubegrænsede automatiske tagsøgningsforespørgsler.<br>
• Ubegrænset albumomslagssøgning: Tilbyder ubegrænsede albumomslagssøgningsforespørgsler.<br>
• Ubegrænset eksport af mediesamlinger: Tillader ubegrænsede eksporter af mediesamlinger (playlister, album, kunstnere, genrer) til M3U/CSV/TXT.<br>
• Fuld tilpasning: Giver fulde tilpasningsmuligheder, herunder muligheden for at ændre appikonet.<br><br>

[Læs mere](/docs/faq/evermusic/what-is-the-difference-between-evermusic-and-evermusic-premium/)
{{% /details %}}


{{% details title="Er Evermusic sikkert?" closed="true" %}}
Evermusic bruger kun officiel SDK og sikre forbindelser til at interagere med tilsluttede cloudtjenester. Dit brugernavn og adgangskode er ikke tilgængelige for applikationen. Alle forespørgsler fra applikationen til cloudtjenesten er krypterede.<br>
Når du indtaster brugernavn og adgangskode, viser applikationen dig den officielle autorisationsside fra cloudtjenesteudbyderen, og hele autorisationsprocessen foregår uden for applikationen. Cloudtjenesteudbyderen sender et auth-token til applikationen efter vellykket autorisation, og dette token bruges til at foretage API-kald.<br><br>

Auth-token er en digital nøgle, der tillader tredjepartsapplikationer at interagere med cloudlagring. Auth-token gemmes på din enhed i sikker systemlagring kaldet Keychain. Du kan downloade dine filer fra den tilsluttede cloudtjeneste til enheden, og disse filer placeres i appens "Dokumenter"-mappe. Du kan fjerne disse filer når som helst ved hjælp af den indbyggede filadministrator.<br>
Applikationen deler ingen information fra den tilsluttede cloudkonto. Du kan tilbagekalde adgang til din cloudkonto når som helst ved at åbne kontoindstillingssiden i din webbrowser.<br><br>

For at tilbagekalde auth-token skal du logge ind på din konto i webbrowseren og navigere til indstillingssiden. Der kan du finde alle tredjepartsapps, der er tilsluttet din cloudkonto, og fjerne enhver af dem, hvis du ikke længere ønsker at bruge den applikation.<br><br>

Du kan også frakoble de tilsluttede cloudkonti i applikationen, og auth-token vil også blive fjernet fra din enhed. Hvis du fjerner applikationen fra din enhed, vil alle downloadede data og adgangstokens også blive fjernet.<br><br>

[Læs mere](/docs/guide/evermusic/evermusic-guide-connections/)
{{% /details %}}

{{% details title="Hvordan opretter jeg en playliste i Evermusic?" closed="true" %}}
- Åbn sektionen Playlister.<br>
- Tryk på "+"-knappen eller "..."-knappen i øverste højre hjørne og vælg "Ny playliste".<br>
- Indtast et navn til playlisten og tryk "Gem". Dialogen "Tilføj sange" vises.<br>
- Vælg de numre, du vil tilføje til playlisten.<br><br>

[Læs mere](/docs/guide/evermusic/evermusic-guide-playlists/)
{{% /details %}}

{{% details title="Hvilke cloudtjenester understøtter Evermusic?" closed="true" %}}
I øjeblikket understøtter applikationen de mest populære cloudtjenester: iCloud Drive, Google Drive, Dropbox, OneDrive, Box, MEGA, Yandex.Disk, WD MyCloud Home, DLNA, MediaFire, WebDAV, SMB, pCloud, HiDrive, 百度网盘, My Cloud Home, InfiniCLOUD, Cloud Mail.ru, Put.io, MyDrive.<br><br>

[Læs mere](/docs/guide/evermusic/evermusic-guide-connections/)
{{% /details %}}

{{% details title="Hvordan bruger jeg equalizeren?" closed="true" %}}
- Åbn lydafspillerens skærm.<br>
- Tryk på "Equalizer"-ikonet nederst på skærmen.<br>
- Tænd kontakten i øverste højre hjørne på equalizerskærmen for at aktivere equalizeren.<br>
- Vælg en af de tilgængelige forudindstillinger (rock, pop, dance, disco..) eller juster equalizerskyderne for at bygge din egen forudindstilling.<br><br>

Fuld vejledning tilgængelig her:<br>
[Sådan bruger du lydequalizer på din iPhone, iPad, Mac med Evermusic og Flacbox](/docs/howto/how-to-use-the-audio-equalizer-on-your-iphone-ipad-mac-with-evermusic-and-flacbox)
{{% /details %}}

{{% details title="Hvordan aktiverer jeg offline tilstand i Evermusic?" closed="true" %}}
- Tilslut en cloudtjeneste:<br>
 • Gå til fanen "Forbindelser".<br>
 • Vælg "Tilslut et cloudlager" og følg anvisningerne for at tilslutte den ønskede tjeneste.<br><br>

- Naviger til musikmappen:<br>
 • Åbn det tilsluttede cloudlager og find din musikmappe.<br><br>

- Aktivér offline tilstand:<br>
 • Tryk på knappen "Flere handlinger" ved siden af mappenavnet.<br>
 • Vælg "Aktivér offline tilstand".<br><br>
 • Den valgte mappe og alt dens indhold downloades til "Lokale filer" > "Offline mapper".<br><br>

- Automatiske opdateringer:<br>
 • Appen vil løbende scanne for ændringer. Nye filer tilføjet til onlinemappen downloades automatisk.<br><br>

- Konfigurer scanningsinterval:<br>
 • Gå til "Indstillinger" > "Filadministrator" > "Offline mapper" > "Tidsinterval" for at indstille scanningsfrekvensen.<br><br>

- Manuel synkronisering:<br>
 • For at synkronisere manuelt, gå til "Indstillinger" > "Filadministrator" > "Offline mapper" > "Synkroniserede offline mapper".<br>
 • Tryk på "Flere handlinger" og vælg "Start synkronisering".<br><br>

[Læs mere](/docs/howto/play-offline-music-in-evermusic-flacbox-download-sync-from-cloud-to-local-files/)
{{% /details %}}

{{% details title="Hvordan afspiller man lokalt downloadet musik på iPhone?" closed="true" %}}
Når du har installeret applikationen, skal du åbne skærmen "Lokale filer" og scrolle ned til sektionen "Filer på denne iPhone".<br>
Derfra vælger du "Åbn filer...", hvis du skal vælge flere lydfiler, eller "Åbn mappe...", hvis du vil vælge en musikmappe.<br>
Appen scanner mappens indhold, og alle fundne lydfiler bliver valgt.<br>
Naviger til din musikmappe, tryk "Åbn" for at bekræfte dit valg, og filerne tilføjes til afspillerkøen.<br>
Disse filer afspilles direkte fra den valgte placering uden at blive kopieret til applikationspakken.<br><br>

**Tilføjelse af en mappe til Favoritter for hurtig adgang**<br>
Forenkl processen ved at tilføje en mappe på din enhed til dine favoritter.<br>
På denne måde behøver du ikke gentage trinnene, hver gang du vil afspille musik.<br>
Åbn skærmen "Lokale filer", scroll til sektionen "Hurtig adgang" og tryk "Favoritter" for at få adgang til skærmen "Favoritfiler".<br>
Tryk på knappen flere handlinger (tre prikker) i øverste højre hjørne og vælg "Tilføj mappe".<br>
Naviger til den ønskede mappe og tryk "Åbn" for at bekræfte.<br>
Din mappe tilføjes til "Favoritfiler" og giver hurtig adgang til din musik.<br><br>

**Import af lokale filer til musikbiblioteket**<br>
Hvis du foretrækker at organisere dine lokale filer i dit musikbibliotek, skal du åbne skærmen "Musikbibliotek", trykke på tre-prikker-knappen i øverste højre hjørne og vælge "+" (Tilføj musik).<br>
Vælg menupunktet "Lokale filer", scroll ned til sektionen "Filer på denne iPhone" og tryk "Åbn filer...".<br>
Vælg de filer, du vil tilføje, og tryk "Åbn" for at bekræfte.<br>
Appen scanner de valgte filer og tilføjer dem til dit musikbibliotek, organiseret efter metadata som kunstner, album og genre.<br><br>

**Tilføjelse af lokale filer til en playliste**<br>
For at tilføje lokale filer til en playliste skal du åbne skærmen "Playlister" og trykke på mere-knappen i øverste højre hjørne.<br>
Vælg "+" (Ny playliste), indtast et navn til din nye playliste, og på næste skærm vælger du muligheden "Lokale filer".<br>
Scroll ned til sektionen "Filer på denne iPhone" og tryk "Åbn filer...".<br>
Vælg de lydfiler, du vil tilføje, og tryk "Åbn" for at bekræfte.<br>
Filerne tilføjes til din playliste, hvor du kan ændre sangrækkefølgen og udføre andre handlinger ved hjælp af mere-knappen.<br><br>

Med disse enkle trin kan du låse op for det fulde potentiale af din iPhone og Mac som de ultimative platforme til at nyde din kære lokale musiksamling.<br><br>

[Læs mere](/docs/howto/how-to-play-local-music-stored-on-your-iphone-or-mac)
{{% /details %}}

{{% details title="Hvordan genoptager jeg en playliste fra, hvor jeg slap?" closed="true" %}}
Først skal du sikre dig, at "Gem lydafspillertilstand" er aktiveret i Indstillinger > Lydafspiller > Generelt.<br>
Når du skifter til en anden playliste og vender tilbage, vil du se fire handlinger på den øverste værktøjslinje under albumomslaget: "Søg", "Fortsæt afspilning", "Afspil alle" og "Bland alle".<br>
Tryk "Fortsæt afspilning" for at genoptage playlisten fra den sidst gemte tilstand og medieposition.<br>
{{% /details %}}

{{% details title="Hvordan ser man sangtekster i Evermusic?" closed="true" %}}
Du kan se indlejrede sangtekster for numre i Evermusic-appen ved at følge disse trin:<br>
1. Start afspilning af en lydfil ved at trykke på den.<br>
2. Åbn fuldskærms-lydafspilleren.<br>
3. Tryk på knappen "Flere handlinger" i øverste højre hjørne for at se menuen med flere handlinger.<br>
4. Vælg menupunktet "Vis kommentarer".<br>
5. På nummerkommentarskærmen har du tre tilstande, som du kan skifte mellem ved at swipe til højre: "Kommentarer", "Indlejrede sangtekster" og "LRC-fil".<br><br>

**Tilstande**:<br>
1. "Kommentartilstand": Viser indholdet fra KOMMENTAR-feltet i din lydfil.<br>
2. "Sangteksttilstand": Viser de indlejrede sangtekster fra din lydfil. Hvis din lydfil har sangtekster i tags, vises de her. Denne tilstand understøtter også tidsbestemt tekst i LRC-format. For at redigere sangtekst-tagget skal du bruge en ekstern lydtag-editor som Evertag. Du kan finde sangtekster i LRC-format gratis på websites som Lyricsify. Kopiér de nødvendige sangtekster til dine tags og gem dem for at vise dem senere på denne skærm.<br>
3. "LRC-filtilstand": I stedet for at redigere lydfiler kan du placere en LRC-fil i samme mappe som den originale lydfil. Begge filer skal have samme navn men forskellige filtypenavne. Når du swiper til tredje side på Kommentarskærmen, søger appen efter LRC-filen i samme mappe og viser dens indhold.<br><br>

[Læs mere](/docs/howto/how-to-add-and-view-comments-to-your-audio-tracks-on-iphone-ipad-and-mac-with-evermusic-and-flacbox)
{{% /details %}}

{{% details title="Hvordan overfører jeg musik til Evermusic fra min computer?" closed="true" %}}
Du kan tilslutte din computer eller personlige NAS ved hjælp af SMB-, WebDAV- eller DLNA-protokoller. Alternativt kan du bruge iTunes fildeling til at overføre musik.<br><br>

For at tilslutte en computer ved hjælp af **SMB**-protokollen tryk "Tilslut en cloudtjeneste" → SMB.<br>
Indtast computerens IP-adresse og delt mappenavn i URL-feltet med formatet smb://computer-ip-adresse/delt-mappenavn,<br>
indtast brugernavn og adgangskode og tryk "Færdig".<br>
Hvis forbindelsen lykkes, vil du se tilsluttet lager i sektionen "Cloudtjenester".<br><br>

En fuld vejledning er tilgængelig her:<br>
[Overfør dine filer fra computeren til iPhone ved hjælp af SMB-protokollen](/docs/howto/transfer-your-files-from-the-computer-to-iphone-using-smb-protocol/)<br><br>

For **WebDAV**-protokollen er alle trin de samme bortset fra URL-feltet.<br>
URL'en skal være i formatet http://server-navn eller https://server-navn, hvis serveren understøtter SSL.<br><br>

En fuld vejledning er tilgængelig her:<br>
[Sådan tilslutter du NAS-lager med WebDAV og lytter til musik på din iPhone eller Mac](/docs/howto/how-to-connect-nas-storage-using-webdav-and-listen-to-music-on-your-iphone-or-mac)<br><br>

**Wi-Fi Drive** er en populær teknologi, der lader dig overføre filer fra din computer til en iOS-enhed trådløst ved hjælp af en desktop-browser.<br>
For at bruge denne funktion skal din enhed og computer være tilsluttet det samme Wi-Fi-netværk.<br>
Åbn "Forbindelser" → "Computer" → "Tilslut via Wi-Fi" og aktivér serveren.<br>
Åbn derefter en desktop-browser og indtast URL'en fra applikationen.<br>
Du kan trække og slippe filer fra din computer til den åbnede webside, og de vil vises på enheden.<br><br>

Mere detaljeret vejledning tilgængelig her:<br>
[Sådan overfører du filer trådløst fra en computer til en iPhone ved hjælp af WiFi-Drive](/docs/howto/how-to-transfer-files-wirelessly-from-a-computer-to-an-iphone-using-wifi-drive)<br><br>

**iTunes fildeling** er en anden teknologi, der lader dig overføre filer fra computer til enhed ved hjælp af iTunes og et lightning-kabel.<br>
Bare tilslut enheden til computeren med et kabel og start iTunes.<br>
Åbn iTunes → "Applikationssektionen" → og find Evermusic.<br>
Tryk på appikonet for at se en delt mappe.<br>
Kopiér filer fra computeren til den delte mappe på enheden.<br><br>

Detaljeret instruktion tilgængelig her:<br>
[Sådan afspiller du lokale filer (iTunes-filer) på min iPhone](/docs/howto/how-to-play-local-itunes-files-on-my-iphone)<br><br>

Med **DLNA** kan du også opsætte en DLNA-medieserver og streame din musik fra Windows PC som beskrevet her:<br>
[Sådan aktiverer du DLNA-medieserver på Windows 10 og afspiller din musik på iPhone](/docs/howto/how-to-enable-dlna-media-server-on-windows-10-and-play-your-music-on-iphone)
{{% /details %}}

{{% details title="Hvordan downloader man musik?" closed="true" %}}
Før du kan downloade musik og lytte til din musik offline, skal du tilslutte en netværkskonto.<br>
Bare åbn skærmen "Forbindelser" og tilføj din konto.<br>
Når du har tilføjet en netværkskonto, kan du downloade din musik fra skyen.<br><br>

**For at downloade musik fra skyen**<br>
– Åbn den tilsluttede cloudtjeneste.<br>
– Naviger ind i den mappe, du vil downloade.<br>
– Tryk på knappen flere handlinger "..." i øverste højre hjørne og vælg menupunktet "Vælg".<br>
– Vælg de filer, du vil downloade, og tryk på knappen "Download".<br><br>

**For at aktivere offline tilstand for Kunstner/Playliste/Album/Genre**<br>
– Åbn skærmen Kunstnere/Album/Playlister/Genrer<br>
– Tryk på afkrydsningsfeltet "Offline tilstand"<br>
– Offline Kunstner/Album/Playliste/Genre vises i sektionen "Offline musik" i musikbiblioteket<br><br>

Mere detaljeret instruktion om offline tilstand tilgængelig her:<br>
[Afspil offline musik i Evermusic og Flacbox: Download og synkroniser fra cloud til lokale filer](/docs/howto/play-offline-music-in-evermusic-flacbox-download-sync-from-cloud-to-local-files/)<br><br>

En anden tilgængelig mulighed er at downloade musik fra YouTube og importere den i Evermusic som beskrevet her:<br>
[Sådan downloader du musik fra YouTube og lytter til offline musik på iPhone](/docs/howto/how-to-download-music-from-youtube-and-listen-to-offline-music-on-iphone)
{{% /details %}}

{{% details title="Understøtter Evermusic Apple CarPlay?" closed="true" %}}
Ja, **Evermusic understøtter fuldt ud Apple CarPlay**. Du kan gennemse dit musikbibliotek, afspille lokale eller offline filer, tilslutte til cloudlagring og kontrollere afspilning direkte fra din bils infotainmentskærm.

CarPlay-grænsefladen inkluderer dedikerede faner for **Bibliotek**, **Forbindelser**, **Lokale filer** og **Indstillinger**, hvilket giver dig fuld kontrol over din musik på vejen. Afspilningskontroller, tilfældig rækkefølge, gentag og køadministration er også tilgængelige.

For at bruge CarPlay skal du sikre dig, at Siri er aktiveret, og at din iPhone er tilsluttet via USB eller trådløst.

[Læs den fulde guide](/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/)
{{% /details %}}

{{% details title="Hvilke lydformater understøtter Evermusic?" closed="true" %}}
Her er den komplette liste over understøttede lydformater og deres tilsvarende filtypenavne:<br><br>

**Understøttede lydformater:**<br>
• AAC (MPEG-4 Advanced Audio Coding)<br>
• ALAC (Apple Lossless)<br>
• AMR (Adaptive Multi-Rate)<br>
• HE-AAC (MPEG-4 High-Efficiency AAC)<br>
• iLBC (Internet Low Bitrate Codec)<br>
• Linear PCM (Uncompressed, Linear Pulse Code Modulation)<br>
• MP3 (MPEG-1 Audio Layer 3)<br>
• μ-law and A-law<br>
• IMA4 (IMA/ADPCM)<br><br>

**Understøttede lydfilendelser:**<br>
mpeg, aifc, 3gp, avi, aif, latm, 3gpp, m4a, loas, cdda, aac, m4p, m4b, ac3, pls, mp4v, m3u, m4r, aiff, xhe, mp1, snd, mp2, wav, qt, wave, m3u8, m4v, mp3, 3g2, caf, mp4, flac, au, w64, ec3, adts, amr, vtt, mpa, aa<br><br>

Med dette brede udvalg af understøttede formater og filtypenavne kan du nyde din musik i det format, du foretrækker.
{{% /details %}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Brugervejledning
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

<div class="hx:w-full hx:mx-auto hx:flex hx:flex-col hx:items-center hx:text-center">
  {{< hextra/info-paragraph border="false" >}}
  Denne guide hjælper dig med at få det bedste ud af Evermusic på din iPhone, iPad eller Mac. Lær hvordan du streamer musik fra skyen, administrerer dine lydbøger og flytter musik mellem enheder. Evermusic giver dig fuld kontrol over din musiksamling i én nem app.
  {{< /hextra/info-paragraph >}}
</div>

{{< cards >}}
  {{< card icon="location-marker" title="Navigation" subtitle="Lær at navigere i Evermusic ved hjælp af fanelinjen på iPhone eller venstremenuen på iPad og Mac." link="/docs/guide/evermusic/evermusic-guide-navigation" >}}

  {{< card icon="cloud" title="Forbindelser" subtitle="Tilslut dine cloudkonti og administrer onlinefiler ved hjælp af den indbyggede filadministrator." link="/docs/guide/evermusic/evermusic-guide-connections" >}}

  {{< card icon="library" title="Musikbibliotek" subtitle="Organiser og udforsk dine numre, album og kunstnere i Musikbiblioteket." link="/docs/guide/evermusic/evermusic-guide-music-library" >}}

  {{< card icon="collection" title="Playlister" subtitle="Opret og arranger playlister, der passer til dit humør eller lejlighed." link="/docs/guide/evermusic/evermusic-guide-playlists" >}}

  {{< card icon="folder" title="Lokale filer" subtitle="Få adgang til og administrer offline musik gennem sektionen Lokale filer." link="/docs/guide/evermusic/evermusic-guide-local-files" >}}

  {{< card icon="play" title="Lydafspiller" subtitle="Kontroller afspilning, kø og lydindstillinger som equalizer og søvntimer." link="/docs/guide/evermusic/evermusic-guide-player" >}}

  {{< card icon="adjustments" title="Indstillinger" subtitle="Tilpas Evermusics udseende, funktioner og ydeevneindstillinger." link="/docs/guide/evermusic/evermusic-guide-settings" >}}

{{< /cards >}}

</div>

<div class="hx:mt-6"></div>

<!-- </body> -->