---
date: '2025-06-12T17:00:00+00:00'
draft: false
title: 'Evermusic - Offline prehrávač hudby pre iPhone a Mac'
layout: hextra-home
description: "Stiahnite si najlepší bezplatný offline prehrávač hudby pre iPhone a Mac a počúvajte svoju hudbu kdekoľvek."
keywords: ["Evermusic", "offline prehrávač hudby", "audio prehrávač iOS", "aplikácia cloudovej hudby", "sťahovanie hudby iPhone", "prehrávač hudby Mac", "streamovanie hudby offline", "FLAC prehrávač iPhone", "multi-cloud prehrávač hudby", "funkcie aplikácie Evermusic"]
tags: ["Evermusic", "offline prehrávanie", "cloudová hudba", "MP3 prehrávač", "aplikácie iPhone", "streamovanie audia", "správa hudby", "aplikácie Mac", "mediálny prehrávač"]
aliases:
  - /evermusic/
  - /index.php/app/evermusic/
appStoreUrl: "https://apps.apple.com/app/apple-store/id885367198?pt=95781850&ct=everappzcom&mt=8"
appStoreId: "885367198"
features: "Offline prehrávanie hudby, Streamovanie z cloudu (iCloud, Google Drive, Dropbox, OneDrive, MEGA, Box), Audio ekvalizér s predvoľbami, Crossfade a plynulé prehrávanie, Import/export playlistov M3U, Prehliadač textov piesní, Audio záložky, Apple CarPlay, AirPlay a Chromecast, Podpora SMB/WebDAV/DLNA, Prehrávanie z USB flash disku, Scrobbling Last.fm"
screenshots:
  - "https://everappz.com/products/evermusic/screenshots/2048x2732/3.png"
  - "https://everappz.com/products/evermusic/screenshots/2048x2732/4.png"
  - "https://everappz.com/products/evermusic/screenshots/2048x2732/5.png"
  - "https://everappz.com/products/evermusic/screenshots/2048x2732/6.png"
---

<!-- <body class="hx:bg-transparent" style="background: radial-gradient(ellipse at 50% 80%, rgba(59,130,246,0.05), hsla(0,0%,100%,0));"> -->

{{< force-dark >}}

{{< hextra/hero-container
  image="/products/evermusic/heroimage/hero_1200.webp"
  imageWidth="600"
  imageCard="true"
>}}

<div class="hx:flex hx:flex-col hx:items-center hx:justify-center hx:sm:block">

<div class="hx:mt-4 hx:mr-4">
{{< hextra/hero-badge link="../../blog/evermusic-celebrates-11-million-worldwide-downloads-a-musical-milestone" >}}
  <div class="hx:w-2 hx:h-2 hx:rounded-full hx:bg-primary-400"></div>
  <span>11 miliónov stiahnutí</span>
    {{< icon name="arrow-circle-right" attributes="height=14" >}}
{{< /hextra/hero-badge >}}
</div>

<div class="hx:mt-6 hx:mb-6">
<div class="hx:flex hx:gap-4 hx:items-center">
  <img src="/images/app_icons/webp/Evermusic_Icon-App-1024x1024.webp" alt="Evermusic Icon" loading="lazy" class="hero-headline-icon" />
  {{< hextra/hero-headline >}}
  Evermusic
  {{< /hextra/hero-headline >}}
</div>
</div>

<div class="hx:mb-6">
{{< hextra/hero-centered-subtitle >}}
<a href="https://www.chip.de/downloads/Evermusic-Pro-iPhone-_-iPad-App_91614216.html" target="_blank" rel="noopener">
  Je dokonalým riešením na organizáciu a prehrávanie&nbsp;<br class="hx:sm:block hx:hidden" />vlastnej hudby z cloudu&nbsp;<br class="hx:sm:block hx:hidden" /><strong>chip.de</strong>
  </span>
{{< /hextra/hero-centered-subtitle >}}
</div>

<div class="hx:mb-6">
{{< hextra/hero-paragraph >}}
• Prehrávajte hudbu s crossfade, plynulým prehrávaním a ekvalizérom  
• Importujte playlisty M3U a sťahujte piesne na offline počúvanie  
• Streamujte hudbu z cloudových úložísk, NAS, počítača alebo USB flash diskov  
• Zobrazujte texty piesní počas počúvania a pridávajte audio záložky na obnovenie kedykoľvek  
{{< /hextra/hero-paragraph >}}
</div>

{{< hextra/hero-button 
  text="Stiahnuť pre mobil"
  link="https://apps.apple.com/app/apple-store/id885367198?pt=95781850&ct=everappzcom&mt=8"
  icon="device-mobile" 
  style="margin-top:1rem;margin-right:1rem;"
>}}

{{< hextra/hero-button 
  text="Stiahnuť pre desktop"
  link="https://apps.apple.com/app/apple-store/id1564384601?pt=95781850&ct=everappzcom&mt=8"
  icon="desktop-computer" 
  style="margin-top:1rem;margin-right:1rem;"
>}}

</div>

{{< /hextra/hero-container >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

{{< hextra/feature-grid >}}

  {{< hextra/feature-card
    title="Ušetrite miesto na zariadení"
    subtitle="Máte 100 GB hudby nazbieranej za roky, ale len 32 GB úložiska na iPhone? Presuňte všetky skladby do cloudu a uvoľnite cenný priestor pre fotografie, videá a aplikácie."
    icon="folder"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(59,130,246,0.15), hsla(0,0%,100%,0));"
  >}}

  {{< hextra/feature-card
    title="Okamžitý prístup k skladbám"
    subtitle="Vďaka podpore najpopulárnejších cloudových služieb vám Evermusic umožňuje vybrať si jednu alebo viac služieb a streamovať hudbu z cloudu. Koniec so synchronizáciou cez iTunes — audio súbory sa nahrávajú do cloudu z PC alebo Mac rovnako ako iné súbory."
    icon="collection"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(192,38,211,0.15), hsla(0,0%,100%,0));"
  >}}

  {{< hextra/feature-card
    title="Offline režim"
    subtitle="Offline režim vám umožňuje sťahovať obľúbené albumy, skladby, žánre a playlisty na offline prehrávanie a počúvať hudbu bez pripojenia k internetu."
    icon="download"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(234,88,12,0.15), hsla(0,0%,100%,0));"
  >}}

  {{< hextra/feature-card
    title="Vaša hudba v cloude"
    subtitle="Vytvorte si vlastnú pokročilú službu streamovania hudby zadarmo! Spravujte rozsiahlu hudobnú knižnicu, šetrite úložisko zariadenia a streamujte obľúbené skladby priamo z cloudu s inteligentným vyrovnávacím ukladaním a plynulým prehrávaním. Využite podporu populárnych cloudových služieb ako iCloud, Google Drive, Dropbox, MEGA.nz, Box, OneDrive a ďalšie."
    icon="cloud"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(34,197,94,0.15), hsla(0,0%,100%,0));"
  >}}

  {{< hextra/feature-card
    title="Jednoduchý prenos súborov"
    subtitle="Pripojte Mac alebo PC a streamujte hudbu priamo z domáceho počítača. Prenášajte audio súbory bezproblémovo medzi počítačom a iOS zariadením pomocou Wi-Fi Drive alebo iTunes File Sharing. Môžete tiež pripojiť NAS alebo USB flash disk a pristupovať ku knižnici odkiaľkoľvek."
    icon="duplicate"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(251,191,36,0.15), hsla(0,0%,100%,0));"
  >}}

  {{< hextra/feature-card
    title="Automatická synchronizácia"
    subtitle="Správca synchronizácie automaticky skenuje cloudové úložisko na audio súbory a aktualizuje hudobnú knižnicu. Načítač metadát organizuje skladby podľa albumov, interpretov a žánrov pomocou vložených audio tagov."
    icon="refresh"
    style="background: radial-gradient(ellipse at 50% 80%, rgba(6,182,212,0.15), hsla(0,0%,100%,0));"
  >}}

{{< /hextra/feature-grid >}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Čistý a jednoduchý dizajn
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

{{< cards cols="4">}}

  {{< card 
    icon="adjustments"
    image="/products/evermusic/screenshots/2048x2732/3.png" 
    title="Audio ekvalizér" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Dolaďte zvuk pomocou audio ekvalizéra v štýle iPod, prispôsobiteľných predvolieb a zosilnenia predzosilňovača pre najlepší hudobný zážitok." 
  >}}

  {{< card 
    icon="annotation"
    image="/products/evermusic/screenshots/2048x2732/4.png"  
    title="Prehliadač textov" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Čítajte vložené texty piesní a komentáre k skladbám počas počúvania. Užívajte si synchronizované texty pre hlbší hudobný zážitok." 
  >}}

  {{< card 
    icon="collection"
    image="/products/evermusic/screenshots/2048x2732/5.png"  
    title="Správca playlistov" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Vytvárajte a organizujte vlastné playlisty, meniete poradie skladieb, exportujte do M3U alebo archivujte ako ZIP súbory na jednoduché zdieľanie alebo zálohovanie." 
  >}}

  {{< card 
    icon="cloud"
    image="/products/evermusic/screenshots/2048x2732/6.png"  
    title="Streamovanie hudby z cloudu" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Pripojte populárne cloudové platformy ako Google Drive, Dropbox a OneDrive a streamujte svoju hudobnú zbierku kedykoľvek a kdekoľvek." 
  >}}

  {{< card 
    icon="duplicate"
    image="/products/evermusic/screenshots/2048x2732/9.png"  
    title="Správca súborov" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Jednoducho spravujte audio súbory — premenúvajte skladby, organizujte priečinky a prenášajte hudbu medzi zariadeniami pomocou vstavaných nástrojov." 
  >}} 

  {{< card 
    icon="sun"
    image="/products/evermusic/screenshots/2048x2732/10.png"  
    title="Prispôsobenie aplikácie" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Prispôsobte prehrávač svojmu štýlu: zmeňte témy, upravte rozloženie a nakonfigurujte funkcie pre skutočne personalizovanú hudobnú aplikáciu." 
  >}}

{{< /cards >}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Kompletná sada funkcií
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

{{< cards >}}

  {{< card title="Audio formáty" subtitle="Podporuje: mp3, m4a, flac, wav, aac, mp4, m3u, m3u8, m4b, m4p, aiff, ogg, mp2, ac3, wma a ďalšie." icon="music-note" >}}

  {{< card title="Audioknihy" subtitle="Zahŕňa audio záložky, ovládanie rýchlosti prehrávania a obnovenie pozície." icon="book-open" width="50">}}

  {{< card title="Automatická synchronizácia" subtitle="Synchronizuje hudobnú knižnicu s cloudom, organizovanú podľa interpreta, albumu a žánru." icon="refresh" >}}

  {{< card title="Správca súborov" subtitle="Spravujte súbory s operáciami ako kopírovanie, presúvanie, premenovanie a mazanie." icon="folder" >}}

  {{< card title="Cloudové služby" subtitle="Podporuje iCloud, Google Drive, Dropbox, OneDrive, Box, MEGA, WD MyCloud Home, pCloud a ďalšie." icon="cloud" >}}

  {{< card title="Prehrávanie s crossfade" subtitle="Ďalšia pieseň začne pred skončením aktuálnej pre plynulé prechody." icon="switch-horizontal" >}}

  {{< card title="Plynulé prehrávanie" subtitle="Žiadne prestávky medzi piesňami — ideálne pre živé, klasické alebo konceptuálne albumy." icon="volume-up" >}}

  {{< card title="Audio ekvalizér" subtitle="Prispôsobiteľný ekvalizér s predvoľbami, manuálnym ovládaním a zosilnením predzosilňovača." icon="adjustments" >}}

  {{< card title="Pokročilé vyhľadávanie" subtitle="Rýchlo a jednoducho nájdite obľúbené albumy, interpretov a skladby." icon="search" >}}

  {{< card title="Prístup z počítača" subtitle="Pripojte sa cez SMB, WebDAV alebo DLNA. Prenášajte cez iTunes File Sharing a Wi-Fi." icon="desktop-computer" >}}

  {{< card title="AirPlay a Chromecast" subtitle="Bezdrôtovo streamujte na zariadenia Apple TV a Google Chromecast." icon="device-mobile" >}}

  {{< card title="Playlisty" subtitle="Vytvárajte, upravujte, meňte poradie a exportujte/importujte playlisty M3U s offline režimom." icon="collection" >}}

  {{< card title="USB flash disky" subtitle="Prehrávajte hudbu priamo z SD kariet pomocou externých čítačiek ako SanDisk iXpand." icon="inbox" >}}

  {{< card title="Offline režim" subtitle="Sťahujte albumy a playlisty alebo automaticky ukladajte do vyrovnávacej pamäte nedávno prehrávané skladby na offline počúvanie." icon="download" >}}

  {{< card title="Apple CarPlay" subtitle="Jazdite bezpečne s dedikovaným rozhraním CarPlay na prehrávanie hudby." icon="truck" >}}

  {{< card title="Editor ID3 tagov" subtitle="Upravte poškodené alebo chýbajúce metadáta pomocou vstavaného editora tagov." icon="pencil-alt" >}}

  {{< card title="Texty a komentáre" subtitle="Zobrazujte vložené texty piesní a komentáre počas počúvania hudby." icon="annotation" >}}

  {{< card title="Last.fm" subtitle="Pripojte účet Last.fm a získajte personalizované štatistiky počúvania a odporúčania." icon="chart-bar" >}}
  
{{< /cards >}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Recenzie z App Store
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">
{{< appstore-reviews apps="885367198,1564384601" stars="5,4" >}}
</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Cenové plány
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">
{{< hextra/feature-grid cols="4">}}

  {{< hextra/feature-card
    title="Zadarmo"
    class="pricing-card pricing-free"
    subtitle=`<span class='pricing-details'>• Obsahuje reklamy<br />
• Playlisty (10)<br />
• Cloudové služby (3)<br />
• Archivácia mediálnych zbierok (3)<br />
• Obľúbené (1000)<br />
• Skladby v playliste (1000)<br />
• Skladby v rade (750)<br />
• Offline priečinky (1)<br />
• Offline albumy (1)<br />
• Automatické vyhľadávanie tagov (20)<br />
• Export do M3U (5)<br />
• Sťahovanie súborov<br />
• Export skladieb z iTunes</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    <strong>Zadarmo</strong>
  </span>
</span>`
  >}}

  {{< hextra/feature-card
    title="Premium Mesačný"
    class="pricing-card pricing-subscription"
    subtitle=`<span class='pricing-details'>• Bez reklám<br />
• Neobmedzené playlisty<br />
• Neobmedzené cloudové služby<br />
• Neobmedzená archivácia médií<br />
• Neobmedzené obľúbené<br />
• Neobmedzené skladby v playliste<br />
• Neobmedzené skladby v rade<br />
• Neobmedzené offline priečinky<br />
• Neobmedzené offline albumy<br />
• Neobmedzené vyhľadávanie tagov<br />
• Neobmedzený export do M3U<br />
• Sťahovanie súborov<br />
• Export skladieb z iTunes<br />
• Export zbierok z iTunes<br />
• Sťahovanie priečinkov<br />
• Sťahovanie zbierok<br />
• Plná personalizácia</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    <strong>$1.99</strong>/mesiac
  </span>
</span>`
  >}}

  {{< hextra/feature-card
    title="Premium Ročný"
    class="pricing-card pricing-subscription"
    subtitle=`<span class='pricing-details'>• Bez reklám<br />
• Neobmedzené playlisty<br />
• Neobmedzené cloudové služby<br />
• Neobmedzená archivácia médií<br />
• Neobmedzené obľúbené<br />
• Neobmedzené skladby v playliste<br />
• Neobmedzené skladby v rade<br />
• Neobmedzené offline priečinky<br />
• Neobmedzené offline albumy<br />
• Neobmedzené vyhľadávanie tagov<br />
• Neobmedzený export do M3U<br />
• Sťahovanie súborov<br />
• Export skladieb z iTunes<br />
• Export zbierok z iTunes<br />
• Sťahovanie priečinkov<br />
• Sťahovanie zbierok<br />
• Plná personalizácia</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    7 dní zadarmo, potom <strong>$12.99</strong>/rok
  </span>
</span>`
  >}}

  {{< hextra/feature-card
    title="Premium Doživotný"
    class="pricing-card pricing-lifetime"
    subtitle=`<span class='pricing-details'>• Bez reklám<br />
• Neobmedzené playlisty<br />
• Neobmedzené cloudové služby<br />
• Neobmedzená archivácia médií<br />
• Neobmedzené obľúbené<br />
• Neobmedzené skladby v playliste<br />
• Neobmedzené skladby v rade<br />
• Neobmedzené offline priečinky<br />
• Neobmedzené offline albumy<br />
• Neobmedzené vyhľadávanie tagov<br />
• Neobmedzený export do M3U<br />
• Sťahovanie súborov<br />
• Export skladieb z iTunes<br />
• Export zbierok z iTunes<br />
• Sťahovanie priečinkov<br />
• Sťahovanie zbierok<br />
• Plná personalizácia</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    <strong>$59.99</strong>/doživotne
  </span>
</span>`
  >}}

{{< /hextra/feature-grid >}}

</div>

<div class="hx:mt-6"></div>

<div class="hx:w-full hx:mx-auto hx:flex hx:flex-col hx:items-center">
  {{< hextra/info-paragraph border="true" >}}
   <strong>Rodinné zdieľanie</strong>: Všetky nákupy a predplatné podporujú Rodinné zdieľanie, čo vám umožňuje zdieľať Premium prístup s rodinou.<br><strong>Univerzálny prístup</strong>: Doživotné a predplatné plány sú zdieľané medzi zariadeniami iOS a Mac pomocou synchronizácie iCloud.<br><strong>Ceny</strong>: Ceny sú uvedené v amerických dolároch pre Spojené štáty. Konečná cena sa môže líšiť v závislosti od vášho regiónu.  
  {{< /hextra/info-paragraph >}}
</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Často kladené otázky
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">
{{% details title="Čo je Evermusic?" closed="true" %}}
Evermusic je aplikácia prehrávača hudby, ktorá vám pomáha počúvať obľúbené piesne z rôznych služieb cloudového úložiska.<br>
Môžete jednoducho sťahovať hudbu na offline prehrávanie, vytvárať a spravovať playlisty a používať vstavaný ekvalizér na zlepšenie zážitku z počúvania.<br>
Funguje so službami ako Google Drive, Dropbox, OneDrive a ďalšie, takže môžete mať všetku hudbu na jednom mieste a pristupovať k nej z akéhokoľvek zariadenia.<br><br>
Aplikácia tiež podporuje rôzne audio formáty a umožňuje organizovať hudobnú knižnicu podľa interpreta, albumu, žánru a skladateľa.<br>
Môžete synchronizovať knižnicu medzi cloudovým úložiskom a zariadením, čím zabezpečíte, že obľúbené skladby budú vždy dostupné.<br>
Navyše, s funkciami ako plynulé prehrávanie, crossfade a možnosť streamovania hudby na zariadenia Chromecast a AirPlay, Evermusic ponúka kompletné riešenie pre všetky vaše hudobné potreby.
{{% /details %}}

{{% details title="Ako funguje Evermusic?" closed="true" %}}
Evermusic funguje pripojením k rôznym službám cloudového úložiska, ako sú Google Drive, Dropbox, OneDrive a ďalšie, čo vám umožňuje pristupovať k hudobnej knižnici z akéhokoľvek zariadenia.<br>
Po pripojení môžete prehliadať a streamovať hudbu priamo z cloudu alebo sťahovať obľúbené piesne, albumy a playlisty na offline prehrávanie.<br>
Aplikácia podporuje viaceré audio formáty, čo uľahčuje prehrávanie akéhokoľvek hudobného súboru.<br><br>
Okrem integrácie s cloudom ponúka Evermusic výkonné funkcie na zlepšenie zážitku z počúvania.<br>
Môžete vytvárať a spravovať playlisty, používať vstavaný ekvalizér na úpravu kvality zvuku a užívať si plynulé prehrávanie s funkciami ako gapless prehrávanie a crossfade.<br>
Aplikácia tiež podporuje streamovanie na zariadenia Chromecast a AirPlay, čo vám umožňuje prehrávať hudbu na akomkoľvek kompatibilnom zariadení.<br>
S Evermusic môžete organizovať hudobnú knižnicu podľa interpreta, albumu, žánru a skladateľa a udržiavať ju synchronizovanú medzi cloudovým úložiskom a zariadeniami, čím zabezpečíte neustály prístup k obľúbeným skladbám.<br><br>
Evermusic vám tiež umožňuje prehrávať lokálne súbory uložené priamo na iPhone, iPad alebo Mac.<br>
Môžete jednoducho importovať hudbu z úložiska zariadenia alebo z knižnice iTunes.<br>
To znamená, že si môžete vychutnať celú hudobnú zbierku, či už je uložená v cloude alebo lokálne na zariadení.<br>
S Evermusic sú všetky obľúbené skladby dostupné na jednom pohodlnom mieste.<br><br>

Preskúmajte naše praktické návody pre viac detailov:<br>
- [Ako prehrávať hudbu z iCloud Drive na iPhone alebo Mac.](/docs/howto/how-to-listen-to-music-from-icloud-drive-on-your-iphone-or-mac)<br>
- [Ako povoliť DLNA mediálny server na Windows 10 a prehrávať hudbu na iPhone.](/docs/howto/how-to-enable-dlna-media-server-on-windows-10-and-play-your-music-on-iphone)<br>
- [Ako nahrať súbory do cloudového úložiska a pripojiť ich k Evermusic, Flacbox, Evertag.](/docs/howto/how-to-upload-my-files-to-the-cloud-storage-and-connect-them-to-evermusic-flacbox-evertag)<br>
- [Ako bezdrôtovo prenášať súbory z počítača na iPhone pomocou WiFi-Drive.](/docs/howto/how-to-transfer-files-wirelessly-from-a-computer-to-an-iphone-using-wifi-drive)<br>
- [Ako pripojiť USB flash disk k iPhone a počúvať hudbu alebo spravovať súbory.](/docs/howto/how-to-connect-a-usb-flashcard-to-the-iphone-and-listen-to-music-or-manage-files-located-on-it)<br>
- [Ako prehrávať hudbu na iPhone z WD My Cloud Home.](/docs/howto/how-to-play-music-on-iphone-from-wd-my-cloud-home)
{{% /details %}}

{{% details title="Je Evermusic zadarmo?" closed="true" %}}
Evermusic je bezplatná aplikácia s niektorými obmedzeniami, ktoré je možné odstrániť po inovácii na Premium verziu. Aplikácia ponúka jednorazový doživotný nákup a dve možnosti predplatného (1 mesiac a 1 rok) na odstránenie všetkých obmedzení, čo vám umožňuje vybrať si najlepšiu a najoptimálnejšiu cenu. Ceny sa môžu líšiť v závislosti od krajiny alebo územia. Tiež majte na pamäti, že Rodinné zdieľanie je povolené pre všetky nákupy a plány, takže Premium verziu môžete zdieľať s členmi rodiny.<br><br>
Doživotné nákupy a predplatné sú zdieľané medzi iOS a Mac pomocou iCloud na synchronizáciu týchto informácií. Ak máte Premium verziu na iOS zariadení, uistite sa, že máte nainštalovanú najnovšiu verziu a že iCloud je povolený. Spustite aplikáciu na iOS a počkajte minútu, kým sa informácie o nákupe nahrajú do iCloud.<br><br>
[Čítať viac](/docs/faq/evermusic/what-is-the-difference-between-evermusic-and-evermusic-premium/)
{{% /details %}}

{{% details title="Aký je rozdiel medzi bezplatným Evermusic a Evermusic Premium?" closed="true" %}}
**Bezplatný Evermusic**<br>
• Obsahuje reklamy: Bezplatná verzia zobrazuje reklamy na generovanie príjmu, čo môže občas narušiť počúvanie hudby.<br>
• Obmedzené playlisty: V bezplatnej verzii môžete vytvoriť až (10) playlistov.<br>
• Obmedzené cloudové služby: Bezplatná verzia podporuje až (3) cloudové služby.<br>
• Obmedzené obľúbené: V bezplatnej verzii môžete pridať maximálne (100) skladieb do obľúbených.<br>
• Limity playlistov a radu: Bezplatná verzia má limity na počet skladieb v jednom playliste (1 000) a maximálny počet skladieb v rade prehrávača (750).<br>
• Obmedzené offline funkcie: Bezplatná verzia má obmedzenia offline prístupu vrátane len (1) offline priečinka a možnosti sťahovania albumov, interpretov, žánrov, playlistov a skladateľov offline.<br>
• Bez exportu zbierok iTunes: V bezplatnej verzii nemôžete exportovať zbierky z iTunes do Lokálnych súborov.<br>
• Obmedzené funkcie vyhľadávania tagov: Ponúka (20) automatických požiadaviek na vyhľadávanie tagov a (20) požiadaviek na vyhľadávanie obalov albumov denne.<br>
• Obmedzený export mediálnych zbierok: Umožňuje (5) exportov mediálnych zbierok (playlisty, albumy, interpreti, žánre) do M3U/CSV/TXT denne.<br>
• Obmedzené funkcie personalizácie: Môžete používať len obmedzené možnosti personalizácie a zmena ikony aplikácie je tiež obmedzená.<br><br>

**Evermusic Premium**<br>
• Bez reklám: Evermusic Premium odstraňuje reklamy a poskytuje plynulý a neprerušovaný zážitok z počúvania hudby.<br>
• Neobmedzené playlisty: S Evermusic Premium môžete vytvárať neobmedzený počet playlistov.<br>
• Neobmedzené cloudové služby: Evermusic Premium vám umožňuje pripojiť sa k neobmedzenému počtu cloudových služieb, čím rozšírite prístup k hudobnej knižnici.<br>
• Neobmedzené obľúbené: S Evermusic Premium môžete pridávať neobmedzený počet skladieb do obľúbených.<br>
• Flexibilita playlistov a radu: Evermusic Premium odstraňuje obmedzenia veľkosti playlistov a radu, čím vám dáva väčšiu kontrolu nad organizáciou hudby.<br>
• Rozšírené offline funkcie: Evermusic Premium ponúka pokročilé offline schopnosti vrátane sťahovania priečinkov, albumov, interpretov, žánrov, playlistov a skladateľov offline.<br>
• Export zbierok iTunes: Môžete exportovať zbierky z iTunes do Lokálnych súborov.<br>
• Neobmedzené vyhľadávanie tagov: Ponúka neobmedzené automatické požiadavky na vyhľadávanie tagov.<br>
• Neobmedzené vyhľadávanie obalov albumov: Ponúka neobmedzené požiadavky na vyhľadávanie obalov albumov.<br>
• Neobmedzený export mediálnych zbierok: Umožňuje neobmedzený export mediálnych zbierok (playlisty, albumy, interpreti, žánre) do M3U/CSV/TXT.<br>
• Plná personalizácia: Poskytuje plné možnosti personalizácie vrátane zmeny ikony aplikácie.<br><br>

[Čítať viac](/docs/faq/evermusic/what-is-the-difference-between-evermusic-and-evermusic-premium/)
{{% /details %}}


{{% details title="Je Evermusic bezpečný?" closed="true" %}}
Evermusic používa len oficiálne SDK a bezpečné pripojenia na interakciu s pripojenými cloudovými službami. Vaše prihlasovacie meno a heslo nie sú pre aplikáciu dostupné. Všetky požiadavky z aplikácie na cloudovú službu sú šifrované.<br>
Keď zadáte prihlasovacie meno a heslo, aplikácia vám zobrazí oficiálnu autorizačnú stránku poskytnutú poskytovateľom cloudovej služby a celý autorizačný proces prebieha mimo aplikácie. Poskytovateľ cloudovej služby odošle autorizačný token aplikácii po úspešnej autorizácii a tento token sa používa na vykonávanie API volaní.<br><br>

Autorizačný token je digitálny kľúč, ktorý umožňuje aplikáciám tretích strán komunikovať s cloudovým úložiskom. Autorizačný token je uložený na vašom zariadení v bezpečnom systémovom úložisku nazývanom Keychain. Môžete sťahovať súbory z pripojenej cloudovej služby na zariadenie a tieto súbory budú umiestnené v adresári „Documents" aplikácie. Tieto súbory môžete kedykoľvek odstrániť pomocou vstavaného správcu súborov.<br>
Aplikácia nezdieľa žiadne informácie z pripojeného cloudového účtu. Prístup k cloudovému účtu môžete kedykoľvek zrušiť otvorením stránky nastavení účtu vo webovom prehliadači.<br><br>

Na zrušenie autorizačného tokenu sa prihláste do účtu vo webovom prehliadači a prejdite na stránku nastavení. Tam nájdete všetky aplikácie tretích strán pripojené k cloudovému účtu a môžete odstrániť ktorúkoľvek z nich, ak ju už nechcete používať.<br><br>

Pripojené cloudové účty môžete tiež odpojiť v aplikácii a autorizačný token bude tiež odstránený z vášho zariadenia. Ak aplikáciu odstránite zo zariadenia, všetky stiahnuté dáta a prístupové tokeny budú tiež odstránené.<br><br>

[Čítať viac](/docs/guide/evermusic/evermusic-guide-connections/)
{{% /details %}}

{{% details title="Ako vytvorím playlist v Evermusic?" closed="true" %}}
- Otvorte sekciu Playlisty.<br>
- Ťuknite na tlačidlo „+" alebo tlačidlo „..." v pravom hornom rohu a vyberte „Nový playlist".<br>
- Zadajte názov playlistu a ťuknite na „Uložiť". Zobrazí sa dialóg „Pridať skladby".<br>
- Vyberte skladby, ktoré chcete pridať do playlistu.<br><br>

[Čítať viac](/docs/guide/evermusic/evermusic-guide-playlists/)
{{% /details %}}

{{% details title="Aké cloudové služby Evermusic podporuje?" closed="true" %}}
V súčasnosti aplikácia podporuje najpopulárnejšie cloudové služby: iCloud Drive, Google Drive, Dropbox, OneDrive, Box, MEGA, Yandex.Disk, WD MyCloud Home, DLNA, MediaFire, WebDAV, SMB, pCloud, HiDrive, 百度网盘, My Cloud Home, InfiniCLOUD, Cloud Mail.ru, Put.io, MyDrive.<br><br>

[Čítať viac](/docs/guide/evermusic/evermusic-guide-connections/)
{{% /details %}}

{{% details title="Ako môžem používať ekvalizér?" closed="true" %}}
- Otvorte obrazovku audio prehrávača.<br>
- Ťuknite na ikonu „Ekvalizér" v spodnej časti obrazovky.<br>
- Zapnite prepínač v pravom hornom rohu obrazovky ekvalizéra na jeho aktiváciu.<br>
- Vyberte jednu z dostupných predvolieb (rock, pop, dance, disco..) alebo zmeňte posuvníky ekvalizéra na vytvorenie vlastnej predvoľby.<br><br>

Kompletný návod dostupný tu:<br>
[Ako používať audio ekvalizér na iPhone, iPad, Mac s Evermusic a Flacbox](/docs/howto/how-to-use-the-audio-equalizer-on-your-iphone-ipad-mac-with-evermusic-and-flacbox)
{{% /details %}}

{{% details title="Ako povolím offline režim v Evermusic?" closed="true" %}}
- Pripojte cloudovú službu:<br>
 • Prejdite na kartu „Pripojenia".<br>
 • Vyberte „Pripojiť cloudové úložisko" a postupujte podľa pokynov na pripojenie požadovanej služby.<br><br>

- Prejdite do priečinka s hudbou:<br>
 • Otvorte pripojené cloudové úložisko a nájdite priečinok s hudbou.<br><br>

- Povoľte offline režim:<br>
 • Ťuknite na tlačidlo „Viac akcií" vedľa názvu priečinka.<br>
 • Vyberte „Povoliť offline režim".<br><br>
 • Vybraný priečinok a celý jeho obsah sa stiahnu do „Lokálne súbory" > „Offline priečinky".<br><br>

- Automatické aktualizácie:<br>
 • Aplikácia bude nepretržite skenovať zmeny. Nové súbory pridané do online priečinka sa automaticky stiahnu.<br><br>

- Nastavenie intervalu skenovania:<br>
 • Prejdite do „Nastavenia" > „Správca súborov" > „Offline priečinky" > „Časový interval" na nastavenie frekvencie skenovania.<br><br>

- Manuálna synchronizácia:<br>
 • Pre manuálnu synchronizáciu prejdite do „Nastavenia" > „Správca súborov" > „Offline priečinky" > „Synchronizované offline priečinky".<br>
 • Ťuknite na „Viac akcií" a vyberte „Spustiť synchronizáciu".<br><br>

[Čítať viac](/docs/howto/play-offline-music-in-evermusic-flacbox-download-sync-from-cloud-to-local-files/)
{{% /details %}}

{{% details title="Ako prehrávať lokálne stiahnutú hudbu na iPhone?" closed="true" %}}
Po nainštalovaní aplikácie otvorte obrazovku „Lokálne súbory" a posuňte sa nadol do sekcie „Súbory na tomto iPhone".<br>
Odtiaľ vyberte „Otvoriť súbory...", ak potrebujete vybrať niekoľko audio súborov, alebo „Otvoriť priečinok...", ak chcete vybrať priečinok s hudbou.<br>
Aplikácia naskenuje obsah priečinka a všetky nájdené audio súbory sa označia.<br>
Prejdite do priečinka s hudbou, ťuknite na „Otvoriť" na potvrdenie výberu a súbory sa pridajú do radu prehrávača.<br>
Tieto súbory sa budú prehrávať priamo z vybraného umiestnenia bez kopírovania do balíka aplikácie.<br><br>

**Pridanie priečinka do obľúbených pre rýchly prístup**<br>
Zjednodušte proces pridaním priečinka z vášho zariadenia do obľúbených.<br>
Tak nebudete musieť opakovať kroky zakaždým, keď budete chcieť prehrávať hudbu.<br>
Otvorte obrazovku „Lokálne súbory", posuňte sa do sekcie „Rýchly prístup" a ťuknite na „Obľúbené" pre prístup k obrazovke „Obľúbené súbory".<br>
Ťuknite na tlačidlo viac akcií (tri bodky) v pravom hornom rohu a vyberte „Pridať priečinok".<br>
Prejdite do požadovaného priečinka a ťuknite na „Otvoriť" na potvrdenie.<br>
Priečinok sa pridá do „Obľúbených súborov", čím získate rýchly prístup k hudbe.<br><br>

**Import lokálnych súborov do hudobnej knižnice**<br>
Ak uprednostňujete organizáciu lokálnych súborov v hudobnej knižnici, otvorte obrazovku „Hudobná knižnica", ťuknite na tlačidlo s tromi bodkami v pravom hornom rohu a vyberte „+" (Pridať hudbu).<br>
Vyberte položku menu „Lokálne súbory", posuňte sa do sekcie „Súbory na tomto iPhone" a ťuknite na „Otvoriť súbory...".<br>
Vyberte súbory, ktoré chcete pridať, a ťuknite na „Otvoriť" na potvrdenie.<br>
Aplikácia naskenuje vybrané súbory a pridá ich do hudobnej knižnice, organizujúc ich podľa metadát ako interpret, album a žáner.<br><br>

**Pridanie lokálnych súborov do playlistu**<br>
Pre pridanie lokálnych súborov do playlistu otvorte obrazovku „Playlisty" a ťuknite na tlačidlo viac v pravom hornom rohu.<br>
Vyberte „+" (Nový playlist), zadajte názov nového playlistu a na ďalšej obrazovke vyberte možnosť „Lokálne súbory".<br>
Posuňte sa do sekcie „Súbory na tomto iPhone" a ťuknite na „Otvoriť súbory...".<br>
Vyberte audio súbory, ktoré chcete pridať, a ťuknite na „Otvoriť" na potvrdenie.<br>
Súbory sa pridajú do playlistu, kde môžete meniť poradie skladieb a vykonávať ďalšie akcie pomocou tlačidla viac.<br><br>

Pomocou týchto jednoduchých krokov môžete odomknúť plný potenciál iPhone a Mac ako dokonalých platforiem na vychutnávanie si lokálnej hudobnej zbierky.<br><br>

[Čítať viac](/docs/howto/how-to-play-local-music-stored-on-your-iphone-or-mac)
{{% /details %}}

{{% details title="Ako môžem obnoviť playlist od miesta, kde som skončil?" closed="true" %}}
Najprv sa uistite, že je povolená možnosť „Uložiť stav audio prehrávača" v Nastavenia > Audio prehrávač > Všeobecné.<br>
Keď prepnete na iný playlist a vrátite sa, uvidíte štyri akcie na hornej lište nástrojov pod obalom albumu: „Hľadať", „Pokračovať v prehrávaní", „Prehrať všetko" a „Premiešať všetko".<br>
Ťuknite na „Pokračovať v prehrávaní" na obnovenie playlistu z posledného uloženého stavu a pozície média.<br>
{{% /details %}}

{{% details title="Ako zobraziť texty piesní v Evermusic?" closed="true" %}}
Vložené texty piesní v aplikácii Evermusic môžete zobraziť podľa týchto krokov:<br>
1. Začnite prehrávanie audio súboru ťuknutím naň.<br>
2. Otvorte celoobrazovkový audio prehrávač.<br>
3. Ťuknite na tlačidlo „Viac akcií" v pravom hornom rohu pre zobrazenie menu akcií.<br>
4. Vyberte položku menu „Zobraziť komentáre".<br>
5. Na obrazovke komentárov skladby máte tri režimy, ktoré môžete meniť posúvaním doprava: „Komentáre", „Vložené texty" a „LRC súbor".<br><br>

**Režimy**:<br>
1. „Režim komentárov": Zobrazuje obsah poľa COMMENT vášho audio súboru.<br>
2. „Režim textov": Zobrazuje vložené texty z vášho audio súboru. Ak váš audio súbor má texty v tagoch, zobrazia sa tu. Tento režim tiež podporuje synchronizovaný text vo formáte LRC. Na úpravu tagu textov použite externý editor audio tagov ako Evertag. Texty vo formáte LRC nájdete zadarmo na stránkach ako Lyricsify. Skopírujte potrebné texty do tagov a uložte ich na neskoršie zobrazenie na tejto obrazovke.<br>
3. „Režim LRC súboru": Namiesto úpravy audio súborov môžete umiestniť LRC súbor do rovnakého priečinka ako pôvodný audio súbor. Oba súbory by mali mať rovnaký názov, ale rôzne prípony. Keď prejdete na tretiu stránku na obrazovke Komentáre, aplikácia vyhľadá LRC súbor v rovnakom adresári a zobrazí jeho obsah.<br><br>

[Čítať viac](/docs/howto/how-to-add-and-view-comments-to-your-audio-tracks-on-iphone-ipad-and-mac-with-evermusic-and-flacbox)
{{% /details %}}

{{% details title="Ako prenesiem hudbu do Evermusic z počítača?" closed="true" %}}
Môžete pripojiť počítač alebo osobný NAS pomocou protokolov SMB, WebDAV alebo DLNA. Alternatívne použite iTunes File Sharing na prenos hudby.<br><br>

Na pripojenie počítača pomocou protokolu **SMB** ťuknite na „Pripojiť cloudovú službu" → SMB.<br>
Zadajte IP adresu počítača a názov zdieľaného priečinka do poľa URL vo formáte smb://ip-adresa-počítača/názov-zdieľaného-priečinka,<br>
zadajte prihlasovacie meno a heslo a ťuknite na „Hotovo".<br>
Ak je pripojenie úspešné, uvidíte pripojené úložisko v sekcii „Cloudové služby".<br><br>

Kompletný návod dostupný tu:<br>
[Prenos súborov z počítača na iPhone pomocou protokolu SMB](/docs/howto/transfer-your-files-from-the-computer-to-iphone-using-smb-protocol/)<br><br>

Pre protokol **WebDAV** sú všetky kroky rovnaké okrem poľa URL.<br>
URL by mal byť vo formáte http://názov-servera alebo https://názov-servera, ak server podporuje SSL.<br><br>

Kompletný návod dostupný tu:<br>
[Ako pripojiť NAS úložisko pomocou WebDAV a počúvať hudbu na iPhone alebo Mac](/docs/howto/how-to-connect-nas-storage-using-webdav-and-listen-to-music-on-your-iphone-or-mac)<br><br>

**Wi-Fi Drive** je populárna technológia, ktorá umožňuje bezdrôtovo prenášať súbory z počítača na iOS zariadenie pomocou desktopového prehliadača.<br>
Na použitie tejto funkcie musia byť zariadenie a počítač pripojené k rovnakej Wi-Fi sieti.<br>
Otvorte „Pripojenia" → „Počítač" → „Pripojiť cez Wi-Fi" a zapnite server.<br>
Potom otvorte desktopový prehliadač a zadajte URL z aplikácie.<br>
Súbory môžete presunúť z počítača na otvorenú webovú stránku a objavia sa na zariadení.<br><br>

Podrobnejší návod dostupný tu:<br>
[Ako bezdrôtovo prenášať súbory z počítača na iPhone pomocou WiFi-Drive](/docs/howto/how-to-transfer-files-wirelessly-from-a-computer-to-an-iphone-using-wifi-drive)<br><br>

**iTunes File Sharing** je ďalšia technológia na prenos súborov z počítača na zariadenie pomocou iTunes a lightning kábla.<br>
Stačí pripojiť zariadenie k počítaču pomocou kábla a spustiť iTunes.<br>
Otvorte iTunes → „Sekcia Aplikácie" → a nájdite Evermusic.<br>
Ťuknite na ikonu aplikácie pre zobrazenie zdieľaného priečinka.<br>
Skopírujte súbory z počítača do zdieľaného priečinka na zariadení.<br><br>

Podrobný návod dostupný tu:<br>
[Ako prehrávať lokálne súbory (iTunes súbory) na iPhone](/docs/howto/how-to-play-local-itunes-files-on-my-iphone)<br><br>

S **DLNA** môžete tiež nastaviť DLNA mediálny server a streamovať hudbu z Windows PC, ako je popísané tu:<br>
[Ako povoliť DLNA mediálny server na Windows 10 a prehrávať hudbu na iPhone](/docs/howto/how-to-enable-dlna-media-server-on-windows-10-and-play-your-music-on-iphone)
{{% /details %}}

{{% details title="Ako stiahnuť hudbu?" closed="true" %}}
Pred sťahovaním hudby a offline počúvaním musíte pripojiť sieťový účet.<br>
Stačí otvoriť obrazovku „Pripojenia" a pridať účet.<br>
Po pridaní sieťového účtu môžete sťahovať hudbu z cloudu.<br><br>

**Na stiahnutie hudby z cloudu**<br>
– Otvorte pripojenú cloudovú službu.<br>
– Prejdite do priečinka, ktorý chcete stiahnuť.<br>
– Ťuknite na tlačidlo viac akcií „..." v pravom hornom rohu a vyberte položku menu „Vybrať".<br>
– Vyberte súbory, ktoré chcete stiahnuť, a ťuknite na tlačidlo „Stiahnuť".<br><br>

**Na povolenie offline režimu pre Interpreta/Playlist/Album/Žáner**<br>
– Otvorte obrazovku Interpreti/Albumy/Playlisty/Žánre<br>
– Ťuknite na zaškrtávacie políčko „Offline režim"<br>
– Offline Interpret/Album/Playlist/Žáner sa zobrazí v sekcii „Offline hudba" hudobnej knižnice<br><br>

Podrobnejší návod o offline režime dostupný tu:<br>
[Prehrávanie offline hudby v Evermusic a Flacbox: Sťahovanie a synchronizácia z cloudu do lokálnych súborov](/docs/howto/play-offline-music-in-evermusic-flacbox-download-sync-from-cloud-to-local-files/)<br><br>

Ďalšou dostupnou možnosťou je stiahnutie hudby z YouTube a jej import do Evermusic, ako je popísané tu:<br>
[Ako stiahnuť hudbu z YouTube a počúvať offline hudbu na iPhone](/docs/howto/how-to-download-music-from-youtube-and-listen-to-offline-music-on-iphone)
{{% /details %}}

{{% details title="Podporuje Evermusic Apple CarPlay?" closed="true" %}}
Áno, **Evermusic plne podporuje Apple CarPlay**. Môžete prehliadať hudobnú knižnicu, prehrávať lokálne alebo offline súbory, pripájať sa ku cloudovému úložisku a ovládať prehrávanie priamo z obrazovky infotainmentu vášho auta.

Rozhranie CarPlay obsahuje dedikované karty pre **Knižnicu**, **Pripojenia**, **Lokálne súbory** a **Nastavenia**, čo vám dáva plnú kontrolu nad hudbou na ceste. K dispozícii sú tiež ovládanie prehrávania, premiešanie, opakovanie a správa radu.

Na použitie CarPlay sa uistite, že Siri je povolená a iPhone je pripojený cez USB alebo bezdrôtovo.

[Prečítajte si kompletný návod](/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/)
{{% /details %}}

{{% details title="Aké audio formáty Evermusic podporuje?" closed="true" %}}
Tu je kompletný zoznam podporovaných audio formátov a ich zodpovedajúcich prípon súborov:<br><br>

**Podporované audio formáty:**<br>
• AAC (MPEG-4 Advanced Audio Coding)<br>
• ALAC (Apple Lossless)<br>
• AMR (Adaptive Multi-Rate)<br>
• HE-AAC (MPEG-4 High-Efficiency AAC)<br>
• iLBC (Internet Low Bitrate Codec)<br>
• Linear PCM (Uncompressed, Linear Pulse Code Modulation)<br>
• MP3 (MPEG-1 Audio Layer 3)<br>
• μ-law and A-law<br>
• IMA4 (IMA/ADPCM)<br><br>

**Podporované prípony audio súborov:**<br>
mpeg, aifc, 3gp, avi, aif, latm, 3gpp, m4a, loas, cdda, aac, m4p, m4b, ac3, pls, mp4v, m3u, m4r, aiff, xhe, mp1, snd, mp2, wav, qt, wave, m3u8, m4v, mp3, 3g2, caf, mp4, flac, au, w64, ec3, adts, amr, vtt, mpa, aa<br><br>

S touto širokou škálou podporovaných formátov a prípon si môžete vychutnať hudbu vo formáte podľa vlastného výberu.
{{% /details %}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Používateľská príručka
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

<div class="hx:w-full hx:mx-auto hx:flex hx:flex-col hx:items-center hx:text-center">
  {{< hextra/info-paragraph border="false" >}}
  Táto príručka vám pomôže naplno využiť Evermusic na iPhone, iPad alebo Mac. Naučte sa, ako streamovať hudbu z cloudu, spravovať audioknihy a prenášať hudbu medzi zariadeniami. Evermusic vám dáva plnú kontrolu nad hudobnou zbierkou v jednej jednoduchej aplikácii.
  {{< /hextra/info-paragraph >}}
</div>

{{< cards >}}
  {{< card icon="location-marker" title="Navigácia" subtitle="Naučte sa navigovať v Evermusic pomocou lišty kariet na iPhone alebo ľavého menu na iPad a Mac." link="/docs/guide/evermusic/evermusic-guide-navigation" >}}

  {{< card icon="cloud" title="Pripojenia" subtitle="Pripojte cloudové účty a spravujte online súbory pomocou vstavaného správcu súborov." link="/docs/guide/evermusic/evermusic-guide-connections" >}}

  {{< card icon="library" title="Hudobná knižnica" subtitle="Organizujte a objavujte skladby, albumy a interpretov v Hudobnej knižnici." link="/docs/guide/evermusic/evermusic-guide-music-library" >}}

  {{< card icon="collection" title="Playlisty" subtitle="Vytvárajte a usporiadajte playlisty podľa nálady alebo príležitosti." link="/docs/guide/evermusic/evermusic-guide-playlists" >}}

  {{< card icon="folder" title="Lokálne súbory" subtitle="Pristupujte k offline hudbe a spravujte ju prostredníctvom sekcie Lokálne súbory." link="/docs/guide/evermusic/evermusic-guide-local-files" >}}

  {{< card icon="play" title="Audio prehrávač" subtitle="Ovládajte prehrávanie, rad a nastavenia zvuku ako ekvalizér a časovač spánku." link="/docs/guide/evermusic/evermusic-guide-player" >}}

  {{< card icon="adjustments" title="Nastavenia" subtitle="Prispôsobte vzhľad, funkcie a nastavenia výkonu Evermusic." link="/docs/guide/evermusic/evermusic-guide-settings" >}}

{{< /cards >}}

</div>

<div class="hx:mt-6"></div>

<!-- </body> -->
