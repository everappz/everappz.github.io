---
date: '2025-06-12T17:00:00+00:00'
draft: false
title: 'Flacbox - Pemain Audio Hi-Res untuk iPhone dan Mac'
layout: hextra-home
description: "Main & muat turun fail FLAC, DSD, APE, ALAC pada iPhone anda. Dengar muzik anda terus dari storan awan atau komputer dengan penyama."
keywords: ["pemain FLAC iPhone", "pemain muzik lossless", "pemain audio hi-res", "pemain DSD iOS", "pemain ALAC iPhone", "pemain muzik luar talian iOS", "pemain muzik awan", "strim muzik dari NAS", "main FLAC pada iPhone", "pemain muzik iPhone dengan tag", "pemain muzik Mac", "muat turun muzik iPhone", "penyama audio iOS", "pemain muzik USB iPhone", "pemain muzik berbilang awan", "pemain SMB WebDAV DLNA", "aplikasi iTunes File Sharing", "sokongan AirPlay dan Chromecast"]
tags: ["pemain FLAC", "audio Hi-Res", "main balik luar talian", "muzik awan", "audio DSD", "aplikasi Mac", "aplikasi iPhone", "audio USB iOS", "pengurus fail muzik", "penyama audio", "penyunting metadata", "pemain MP3", "pemain SMB", "muzik Chromecast", "pemain muzik CarPlay", "sokongan pelbagai format", "penstriman muzik", "pengurus perpustakaan audio"]
aliases:
  - /flacbox/
appStoreUrl: "https://apps.apple.com/app/apple-store/id1097564256?pt=95781850&ct=everappzcom&mt=8"
appStoreId: "1097564256"
features: "Main balik Hi-Res FLAC/DSD/APE/ALAC, Penstriman awan (iCloud, Google Drive, Dropbox, OneDrive, MEGA, Box), Penyama audio dengan pratetap, Crossfade dan main balik tanpa jeda, Import/eksport senarai main M3U, Pemapar lirik, Penanda buku audio, Apple CarPlay, AirPlay dan Chromecast, Sokongan SMB/WebDAV/DLNA, Main balik pemacu kilat USB, Scrobbling Last.fm, Penyunting tag terbina dalam"
screenshots:
  - "https://everappz.com/products/flacbox/screenshots/2048x2732/3.png"
  - "https://everappz.com/products/flacbox/screenshots/2048x2732/4.png"
  - "https://everappz.com/products/flacbox/screenshots/2048x2732/5.png"
  - "https://everappz.com/products/flacbox/screenshots/2048x2732/6.png"
---

<!-- force dark theme for this page -->
{{< force-dark >}}

{{< hextra/hero-container
  image="/products/flacbox/heroimage/hero_1600.webp"
  imageWidth="800"
  imageCard="true"
>}}

<div class="hx:flex hx:flex-col hx:items-center hx:justify-center hx:sm:block">

{{< hextra/hero-badge link="../../blog/flacbox-celebrates-1-million-worldwide-downloads-your-gateway-to-hi-res-music/" >}}
  <div class="hx:w-2 hx:h-2 hx:rounded-full hx:bg-primary-400"></div>
  <span>1 Juta Muat Turun</span>
  {{< icon name="arrow-circle-right" attributes="height=14" >}}
{{< /hextra/hero-badge >}}

<div class="hx:mt-6 hx:mb-6">
<div class="hx:flex hx:gap-4 hx:items-center">
<img src="/images/app_icons/webp/Flacbox_Icon-App-1024x1024.webp" alt="Flacbox Icon" loading="lazy" class="hero-headline-icon" />
{{< hextra/hero-headline >}}
Flacbox
{{< /hextra/hero-headline >}}
</div>
</div>

<div class="hx:mb-12">
{{< hextra/hero-centered-subtitle >}}
<strong>Pemain dan Penstrim Audio Hi-Res&nbsp;<br class="hx:sm:block hx:hidden" />untuk iPhone dan MAC</strong>
{{< /hextra/hero-centered-subtitle >}}
</div>

<div class="hx:mb-6">
{{< hextra/hero-paragraph >}}
• Main FLAC, ALAC, APE, DSD dan lagi dengan kualiti lossless  
• Muat turun muzik dan dengar luar talian dengan kawalan penuh  
• Strim dari Google Drive, Dropbox, NAS atau komputer   
{{< /hextra/hero-paragraph >}}
</div>

{{< hextra/hero-button 
    text="Muat turun untuk mudah alih"
    link="https://apps.apple.com/app/apple-store/id1097564256?pt=95781850&ct=everappzcom&mt=8"
    icon="device-mobile"
    style="margin-top:1rem;margin-right:1rem;"
>}}

{{< hextra/hero-button 
    text="Muat turun untuk desktop"
    link="https://apps.apple.com/app/apple-store/id1594027432?pt=95781850&ct=everappzcom&mt=8"
    icon="desktop-computer" 
    style="margin-top:1rem;margin-right:1rem;"
>}}

</div>

{{< /hextra/hero-container >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

{{< hextra/feature-grid >}}

  {{< hextra/feature-card
    title="Strim Muzik Lossless"
    subtitle=`Nikmati muzik lossless pada iPhone, iPad dan Mac tanpa langganan.<br><br>
Sambungkan storan awan anda untuk menstrim FLAC, ALAC, MKA dan lagi secara percuma. Cast dengan mudah ke peranti Chromecast dan AirPlay.<br><br>
Bina perpustakaan muzik anda, susun trek mengikut album, artis dan genre. Tingkatkan audio dengan penyama terbina dalam, kawalan pic/kelajuan dan peningkatan bes.`
    icon="status-online"
    style="background: radial-gradient(circle at 50% 80%, rgba(99,102,241,0.15), transparent);"
  >}}

  {{< hextra/feature-card
    title="Main Audio Hi-Res"
    subtitle=`Nikmati bunyi berkualiti studio dengan sokongan untuk lebih 120 format audio, termasuk FLAC, ALAC, WAV, AIFF dan DSD.<br><br>
Flacbox juga memainkan MP3, AAC, OGG, APE, MOD, MKA dan bekas lanjutan seperti MKV, MP4 dan MOV.<br><br>
Dengan keserasian codec yang luas, seluruh koleksi anda boleh diakses tanpa penukaran diperlukan.`
    icon="volume-up"
    style="background: radial-gradient(circle at 50% 80%, rgba(236,72,153,0.15), transparent);"
  >}}

  {{< hextra/feature-card
    title="Muat Turun & Dengar Luar Talian"
    subtitle=`Kekal berhubung dengan muzik anda walaupun luar talian.<br><br>
Muat turun keseluruhan album, genre, senarai main dan trek ke peranti anda. Gunakan Wi-Fi Drive atau iTunes File Sharing untuk memindahkan audio dari Mac atau PC.<br><br>
Strim dari pemacu kilat USB atau storan rangkaian (NAS) dan nikmati perpustakaan muzik penuh anda tanpa akses Internet.`
    icon="download"
    style="background: radial-gradient(circle at 50% 80%, rgba(16,185,129,0.15), transparent);"
  >}}

{{< /hextra/feature-grid >}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
Ciri-ciri Lengkap
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

{{< cards >}}

  {{< card title="Sokongan Audio Lossless" subtitle="Main FLAC, ALAC, WAV, APE, DFF, WV, AIFF dan banyak format berkualiti tinggi lain." icon="music-note" >}}
  {{< card title="Mod Luar Talian" subtitle="Muat turun album, senarai main dan trek untuk main balik luar talian pada bila-bila masa." icon="download" >}}
  {{< card title="Pengurusan Fail" subtitle="Salin, alih, namakan semula dan padam fail dengan mudah dalam aplikasi." icon="folder" >}}

  {{< card title="Senarai Main Tersuai" subtitle="Cipta, sunting, susun semula senarai main dan eksport atau import fail M3U." icon="collection" >}}
  {{< card title="Perpustakaan Media" subtitle="Tambah muzik dan susun mengikut album, artis dan genre menggunakan tag." icon="library" >}}
  {{< card title="Carian Lanjutan" subtitle="Cari lagu, artis atau album dengan cepat menggunakan alat carian pintar." icon="search" >}}

  {{< card title="Penstriman Awan" subtitle="Strim dari Google Drive, Dropbox, OneDrive, iCloud, MEGA dan lagi." icon="cloud" >}}
  {{< card title="Akses NAS & Komputer" subtitle="Sambung melalui SMB, DLNA atau WebDAV. Pindah dengan iTunes atau Wi-Fi." icon="desktop-computer" >}}
  {{< card title="Kad Kilat USB" subtitle="Main muzik terus dari kad SD menggunakan penyesuai USB seperti iXpand." icon="inbox" >}}

  {{< card title="Kawalan Main Balik" subtitle="Tukar kelajuan dan pic main balik untuk kawalan audio yang sempurna." icon="fast-forward" >}}
  {{< card title="Lirik & Ulasan" subtitle="Lihat lirik dan ulasan terbenam semasa memainkan lagu." icon="annotation" >}}
  {{< card title="Penyunting Tag ID3" subtitle="Sunting dan betulkan metadata seperti tajuk, artis atau album dengan alat terbina dalam." icon="pencil-alt" >}}

  {{< card title="Penyama & Peningkatan" subtitle="Gunakan pratetap atau laras secara manual dengan gandaan praamplifier untuk bunyi yang kaya." icon="adjustments" >}}
  {{< card title="Apple CarPlay" subtitle="Pandu dengan selamat dengan UI CarPlay khusus untuk main balik muzik." icon="truck" >}}
  {{< card title="AirPlay & Chromecast" subtitle="Strim ke Apple TV dan peranti Google Chromecast secara wayarles." icon="device-mobile" >}}

  {{< card title="Integrasi Last.fm" subtitle="Jejak statistik muzik dan dapatkan cadangan lagu yang diperibadikan." icon="chart-bar" >}}
  {{< card title="Penyegerakan Automatik" subtitle="Segerak perpustakaan muzik anda secara automatik dengan folder awan dan tempatan." icon="refresh" >}}
  {{< card title="Sokongan Buku Audio" subtitle="Dengar buku audio dengan penanda buku, pemasa tidur dan sambung main balik dari tempat anda berhenti." icon="book-open" >}}

{{< /cards >}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
Reka Bentuk Intuitif
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

{{< cards cols="4">}}

  {{< card 
    image="/products/flacbox/screenshots/2048x2732/1.png" 
    title="Pemain Audio" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Pemain muzik lanjutan dengan ulang, rawak, kawalan kelajuan dan pelarasan pic." 
    icon="play"
  >}}

  {{< card 
    image="/products/flacbox/screenshots/2048x2732/2.png"  
    title="Penyama Audio" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Penyama tersuai dengan pratetap, peningkatan bes dan praamplifier untuk bunyi yang lebih baik." 
    icon="adjustments"
  >}}

  {{< card 
    image="/products/flacbox/screenshots/2048x2732/3.png"  
    title="Pengurus Senarai Main" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Buat senarai main, tukar susunan lagu, eksport ke M3U atau ZIP untuk sandaran atau perkongsian." 
    icon="collection"
  >}}

  {{< card 
    image="/products/flacbox/screenshots/2048x2732/5.png"  
    title="Perpustakaan Media" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Susun lagu mengikut album, artis atau genre menggunakan tag audio dan metadata." 
    icon="library"
  >}}

  {{< card 
    image="/products/flacbox/screenshots/2048x2732/7.png"  
    title="Storan Awan" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Strim muzik dari Google Drive, Dropbox, OneDrive dan perkhidmatan awan lain." 
    icon="cloud"
  >}}

  {{< card 
    image="/products/flacbox/screenshots/2048x2732/8.png"  
    title="iCloud Drive" 
    method="Fill"
    options="515x200 q90 webp"
    subtitle="Simpan muzik lossless di iCloud dan strim terus tanpa memuat turun." 
    icon="wifi"
  >}}

{{< /cards >}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
Ulasan App Store
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">
{{< appstore-reviews apps="1097564256,1594027432" stars="5,4" >}}
</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
   Pelan Harga
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

{{< hextra/feature-grid cols="4">}}

  {{< hextra/feature-card
    title="Percuma"
    class="pricing-card pricing-free"
    subtitle=`<span class='pricing-details'>• Mengandungi Iklan<br />
• Senarai main (10)<br />
• Perkhidmatan awan (3)<br />
• Koleksi media arkib (3)<br />
• Kegemaran (1000)<br />
• Lagu dalam senarai main (1000)<br />
• Lagu dalam baris gilir (750)<br />
• Folder luar talian (1)<br />
• Album luar talian (1)<br />
• Carian tag automatik (20)<br />
• Eksport ke M3U (5)<br />
• Muat turun fail</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    <strong>Percuma</strong>
  </span>
</span>`
  >}}

  {{< hextra/feature-card
    title="Premium Bulanan"
    class="pricing-card pricing-subscription"
    subtitle=`<span class='pricing-details'>• Pengalaman tanpa iklan<br />
• Senarai main tanpa had<br />
• Perkhidmatan awan tanpa had<br />
• Pengarkiban media tanpa had<br />
• Kegemaran tanpa had<br />
• Lagu dalam senarai main tanpa had<br />
• Lagu dalam baris gilir tanpa had<br />
• Folder luar talian tanpa had<br />
• Album luar talian tanpa had<br />
• Carian tag tanpa had<br />
• Eksport ke M3U tanpa had<br />
• Muat turun fail<br />
• Muat turun folder<br />
• Muat turun koleksi<br />
• Peribadian penuh</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    <strong>$2.99</strong>/bulan
  </span>
</span>`
  >}}

  {{< hextra/feature-card
    title="Premium Tahunan"
    class="pricing-card pricing-subscription"
    subtitle=`<span class='pricing-details'>• Pengalaman tanpa iklan<br />
• Senarai main tanpa had<br />
• Perkhidmatan awan tanpa had<br />
• Pengarkiban media tanpa had<br />
• Kegemaran tanpa had<br />
• Lagu dalam senarai main tanpa had<br />
• Lagu dalam baris gilir tanpa had<br />
• Folder luar talian tanpa had<br />
• Album luar talian tanpa had<br />
• Carian tag tanpa had<br />
• Eksport ke M3U tanpa had<br />
• Muat turun fail<br />
• Muat turun folder<br />
• Muat turun koleksi<br />
• Peribadian penuh</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    <strong>$19.99</strong>/tahun
  </span>
</span>`
  >}}

  {{< hextra/feature-card
    title="Premium Seumur Hidup"
    class="pricing-card pricing-lifetime"
    subtitle=`<span class='pricing-details'>• Pengalaman tanpa iklan<br />
• Senarai main tanpa had<br />
• Perkhidmatan awan tanpa had<br />
• Pengarkiban media tanpa had<br />
• Kegemaran tanpa had<br />
• Lagu dalam senarai main tanpa had<br />
• Lagu dalam baris gilir tanpa had<br />
• Folder luar talian tanpa had<br />
• Album luar talian tanpa had<br />
• Carian tag tanpa had<br />
• Eksport ke M3U tanpa had<br />
• Muat turun fail<br />
• Muat turun folder<br />
• Muat turun koleksi<br />
• Peribadian penuh</span><br /><br />
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    <strong>$69.99</strong>/seumur hidup
  </span>
</span>`
  >}}

{{< /hextra/feature-grid >}}

</div>

<div class="hx:mt-6"></div>

<div class="hx:w-full hx:mx-auto hx:flex hx:flex-col hx:items-center">
  {{< hextra/info-paragraph border="true" >}}
   <strong>Perkongsian Keluarga</strong>: Semua pembelian dan langganan menyokong Perkongsian Keluarga, membolehkan anda berkongsi akses Premium dengan keluarga anda.<br><strong>Akses Universal</strong>: Pelan seumur hidup dan langganan dikongsi antara peranti iOS dan Mac menggunakan penyegerakan iCloud.<br><strong>Harga</strong>: Harga ditunjukkan dalam dolar AS untuk Amerika Syarikat. Harga akhir mungkin berbeza berdasarkan kawasan anda.  
  {{< /hextra/info-paragraph >}}
</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
   Soalan Lazim
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

{{% details title="Bagaimana Flacbox berfungsi?" closed="true" %}}
Flacbox ialah pemain muzik hi-res yang membolehkan anda mengurus trek audio seperti fail biasa.<br>
Anda boleh memuat naik seluruh koleksi muzik anda ke perkhidmatan awan seperti Dropbox, Google Drive atau NAS peribadi dan memainkan muzik terus dari awan dengan kawalan penuh.<br><br>
Tiada penyegerakan iTunes diperlukan—hanya muat naik dari PC atau Mac anda seperti yang anda lakukan dengan mana-mana fail.<br>
Untuk mendengar luar talian, gunakan pengurus muat turun terbina dalam untuk menyimpan muzik ke peranti anda.<br><br>

Terokai panduan cara kami untuk maklumat lanjut:<br>
- [Panduan Flacbox](/docs/guide/flacbox/flacbox-guide)<br>
- [Cara memainkan muzik dari iCloud Drive pada iPhone atau Mac saya.](/docs/howto/how-to-listen-to-music-from-icloud-drive-on-your-iphone-or-mac)<br>
- [Cara Mengaktifkan Pelayan Media DLNA pada Windows 10 dan Memainkan Muzik Anda pada iPhone.](/docs/howto/how-to-enable-dlna-media-server-on-windows-10-and-play-your-music-on-iphone)<br>
- [Cara memuat naik fail saya ke storan awan dan menyambungkannya ke Evermusic, Flacbox, Evertag.](/docs/howto/how-to-upload-my-files-to-the-cloud-storage-and-connect-them-to-evermusic-flacbox-evertag)<br>
- [Cara memindahkan fail secara wayarles dari komputer ke iPhone menggunakan WiFi-Drive.](/docs/howto/how-to-transfer-files-wirelessly-from-a-computer-to-an-iphone-using-wifi-drive)<br>
- [Cara menyambungkan kad kilat USB ke iPhone dan mendengar muzik atau mengurus fail.](/docs/howto/how-to-connect-a-usb-flashcard-to-the-iphone-and-listen-to-music-or-manage-files-located-on-it)<br>
- [Cara memainkan muzik pada iPhone dari WD My Cloud Home.](/docs/howto/how-to-play-music-on-iphone-from-wd-my-cloud-home)
{{% /details %}}

{{% details title="Adakah Flacbox Percuma?" closed="true" %}}
Flacbox percuma untuk digunakan dengan beberapa had, yang boleh dialih keluar dengan menaik taraf ke versi Premium.<br>
Anda boleh memilih antara pembelian seumur hidup sekali sahaja atau dua pilihan langganan (bulanan atau tahunan). Harga mungkin berbeza berdasarkan kawasan anda.<br><br>

Perkongsian Keluarga diaktifkan untuk semua pelan, jadi anda boleh berkongsi versi Premium dengan ahli keluarga anda.<br><br>

Pembelian dan langganan Premium dikongsi antara iOS dan Mac melalui iCloud. Untuk menyegerakkan pembelian anda, pastikan iCloud diaktifkan, buka aplikasi pada peranti iOS anda dan tunggu seminit untuk penyegerakan selesai.<br><br>

[Baca lebih lanjut tentang perbezaan antara Flacbox dan Flacbox Premium](/docs/faq/flacbox/what-is-the-difference-between-flacbox-and-flacbox-premium/)
{{% /details %}}

{{% details title="Apakah perbezaan antara Flacbox dan Evermusic?" closed="true" %}}
**Flacbox** dibina untuk menyokong semua format audio iOS lalai bersama banyak format tambahan yang tidak disokong secara asli pada iPhone, seperti WMA, OGG, M4A, DSD dan lagi.<br>
Ia menggunakan enjin audio tersuai untuk mengendalikan hampir semua format dan menawarkan ciri seperti kadar sampel output audio boleh laras dan pembetulan pic.<br><br>

**Evermusic**, sebaliknya, menyokong format audio lalai dan termasuk ciri lanjutan seperti main balik crossfade, main balik tanpa jeda, penyama audio, kawalan kelajuan main balik dan lagi.<br><br>

Jika anda kebanyakannya menggunakan MP3, ALAC atau FLAC, **Evermusic** mungkin pilihan yang lebih baik untuk ciri main baliknya.<br>
Jika anda memerlukan keserasian luas dengan pelbagai jenis fail audio, **Flacbox** ialah pilihan yang tepat.<br><br>

[Ketahui lebih lanjut tentang perbezaan antara Flacbox dan Evermusic](/docs/faq/evermusic/what-is-the-difference-between-evermusic-and-flacbox/)
{{% /details %}}

{{% details title="Bagaimana saya menyegerakkan Flacbox?" closed="true" %}}

**Penyegerakan Metadata**  
Apabila anda menambah trek ke perpustakaan anda, pembaca metadata latar belakang mengimbas fail anda dan menyusunnya mengikut Artis, Album, Genre dan Komposer.<br>
Anda boleh melaraskan kelajuan imbasan untuk pemuatan lebih cepat, walaupun ia mungkin meningkatkan penggunaan tenaga. Anda juga boleh melumpuhkannya sepenuhnya untuk menunjukkan hanya nama fail dan bukannya tag.<br><br>

Pembaca metadata **tidak mengubah suai** fail audio asal anda dalam awan atau storan tempatan. Jika anda ingin menyunting tag, gunakan penyunting tag terbina dalam dari menu pilihan fail.<br><br>

Jika penyegerakan latar belakang diaktifkan, pembaca metadata akan terus berjalan walaupun aplikasi berada di latar belakang, tetapi perlu diingat bahawa iOS mungkin menangguhkannya jika ia menggunakan terlalu banyak tenaga.<br>
Untuk perpustakaan besar, disyorkan untuk menyegerakkan metadata pada versi desktop dan menggunakan ciri **sandaran & pemulihan data** untuk memindahkannya ke peranti mudah alih anda.<br><br>

Gunakan tindakan **'Muat semula metadata'** untuk memaksa penyegaran metadata penuh merentasi perpustakaan anda.<br><br>

**Penyegerakan Muzik Dalam Talian**<br>
Penyegerakan dalam talian membolehkan anda menambah muzik dari storan awan ke perpustakaan anda secara automatik.<br>
Pergi ke **Tetapan Perpustakaan Muzik → Folder penyegerakan**, pilih folder untuk diimbas dan aplikasi akan mencari dan menambah fail audio yang disokong.<br><br>

Penyegerakan dalam talian berfungsi **hanya apabila aplikasi aktif**, jadi sebaiknya biarkan aplikasi terbuka, disambungkan ke kuasa dan aktifkan **"Skrin sentiasa aktif"** dalam tetapan untuk penyegerakan lebih cepat.<br>
Anda juga boleh melakukan ini pada desktop dan kemudian memindahkan perpustakaan ke mudah alih menggunakan sandaran & pemulihan.<br>
Kekerapan penyegerakan boleh dikonfigurasi (cth. "serta-merta" memulakan penyegerakan setiap kali aplikasi dibuka).

**Penyegerakan Muzik Luar Talian**<br>  
Ini mengimbas semua folder dalam direktori **Documents** aplikasi dan menambah fail tempatan yang disokong ke perpustakaan muzik.<br>
Fail di luar kotak pasir aplikasi (disebabkan keselamatan iOS/macOS) mesti ditambah secara manual.<br>
Jalankan penyegerakan luar talian secara berkala untuk memastikan perpustakaan sentiasa terkini dengan fail tempatan anda.<br><br>

**Folder Luar Talian**<br>  
Mengaktifkan mod luar talian untuk mana-mana album, artis, genre, senarai main atau folder jauh memuat turun semua fail dalam kumpulan itu untuk akses luar talian.<br>
Ia disimpan di bawah bahagian **Folder Luar Talian**.<br><br>

Fail baharu yang ditambah ke folder jauh yang sama akan dimuat turun secara automatik.<br>
Untuk **menyegerak semula secara manual**, ketik menu tiga titik dan pilih **"Segerakkan"**.<br><br>

Anda juga boleh menyesuaikan **selang masa tamat penyegerakan** dalam tetapan untuk lebih kawalan terhadap tingkah laku penyegerakan dan memastikan muzik luar talian anda kekal terkini.<br><br>

[Baca lebih lanjut](/docs/guide/flacbox/flacbox-guide-music-library)

{{% /details %}}

{{% details title="Bagaimana saya menggunakan Flacbox?" closed="true" %}}

**Pasang Flacbox**<br>
Muat turun dan pasang aplikasi Flacbox dari kedai aplikasi peranti anda. Ia tersedia untuk kedua-dua peranti iOS dan Mac.<br><br>

**Buka Aplikasi**<br>
Lancarkan Flacbox dengan mengetik ikonnya pada skrin utama peranti anda.<br><br>

**Sambungkan Perkhidmatan Awan Anda**<br>
• Buka tab **Sambungan**.<br>
• Pilih **Sambung perkhidmatan awan** dari menu.<br>
• Pilih perkhidmatan awan yang dikehendaki dari senarai.<br>
• Masukkan kelayakan anda dan ketik **Selesai**.<br><br>

**Import Fail Pada Peranti Anda**<br>
Anda boleh mengimport fail dari peranti anda, pemacu kilat USB luaran, komputer peribadi atau NAS.<br>
• Buka tab **Fail tempatan**.<br>
• Ketik **Buka fail...** atau **Buka folder...** dari menu **Fail pada peranti ini**.<br>
• Pilih fail atau folder pada peranti anda.<br>
• Ketik **Buka** untuk mengimport fail ke aplikasi.<br><br>

**Tambah Muzik Anda**<br>
Anda mempunyai dua pilihan untuk menambah muzik ke Flacbox: penambahan manual atau penyegerakan automatik.<br><br>

**Penambahan Manual**<br>
• Ketik pada item menu **Tambah muzik** atau serupa.<br>
• Pilih folder/fail dari perkhidmatan awan yang disambungkan atau fail tempatan.<br>
• Flacbox mencipta pautan ke trek anda, mengekalkan lokasi fail asal.<br>
• Untuk menjadikan trek tersedia luar talian, gunakan pengurus muat turun terbina dalam atau aktifkan mod luar talian.<br><br>

**Penyegerakan Automatik**<br>
• Pergi ke **Tetapan Perpustakaan Muzik**.<br>
• Pilih **Folder penyegerakan** untuk mengaktifkan penyegerakan automatik.<br>
• Aplikasi mengimbas folder yang dipilih, mengenal pasti fail audio dan mengintegrasikannya ke dalam perpustakaan anda.<br><br>

**Terokai Perpustakaan Muzik Anda**<br>
• Disusun mengikut jenis sumber dan tag muzik.<br>
• Bahagian: Muzik dalam talian, Muzik luar talian, Fail tempatan, dll.<br>
• Kategori: Lagu, Album, Artis, Genre, Komposer.<br>
• Gunakan **Carian** untuk mencari trek dengan cepat.<br><br>

**Sesuaikan Tetapan Anda**<br>
• Ketik butang tiga titik untuk mengakses **Lebih tindakan** dan **Tetapan**.<br>
• Laraskan penyegerakan metadata, peribadian, pilihan kulit album, senarai main, terkini dan kegemaran.<br><br>

**Nikmati Muzik Anda**<br>
Setelah muzik anda disusun, gunakan bar alat atas untuk tindakan pantas seperti **Carian**, **Main Semua**, **Rawak** dan **Sambung Main Balik**.<br><br>

**Ketahui Lebih Lanjut**<br>
Terokai tutorial dalam aplikasi atau lawati panduan ini:<br><br>

• [Panduan Flacbox](/docs/guide/flacbox/flacbox-guide)<br>
• [Muat naik fail ke awan dan sambungkan ke Flacbox](/docs/howto/how-to-upload-my-files-to-the-cloud-storage-and-connect-them-to-evermusic-flacbox-evertag)<br>
• [Dengar Buku Audio](/docs/howto/how-to-listen-to-audiobooks-on-iphone-ipad-mac-using-evermusic)<br>
• [Main Muzik Tempatan pada iPhone atau Mac](/docs/howto/how-to-play-local-music-stored-on-your-iphone-or-mac)<br>
• [Main dari iCloud Drive](/docs/howto/how-to-listen-to-music-from-icloud-drive-on-your-iphone-or-mac)<br>
• [Gunakan Penyama Audio](/docs/howto/how-to-use-the-audio-equalizer-on-your-iphone-ipad-mac-with-evermusic-and-flacbox)<br>
• [Sambungkan Kad Kilat USB](/docs/howto/how-to-connect-a-usb-flashcard-to-the-iphone-and-listen-to-music-or-manage-files-located-on-it)<br>
• [Pemindahan Wayarles WiFi-Drive](/docs/howto/how-to-transfer-files-wirelessly-from-a-computer-to-an-iphone-using-wifi-drive)

{{% /details %}}

{{% details title="Adakah Flacbox Selamat?" closed="true" %}}
Flacbox hanya menggunakan SDK rasmi dan sambungan selamat untuk berinteraksi dengan perkhidmatan awan yang disambungkan. Log masuk dan kata laluan anda tidak tersedia untuk aplikasi. Semua permintaan dari aplikasi ke perkhidmatan awan dienkripsi.<br>
Apabila anda memasukkan log masuk dan kata laluan, aplikasi menunjukkan halaman kebenaran rasmi yang disediakan oleh penyedia perkhidmatan awan dan semua proses kebenaran dilakukan di luar aplikasi. Penyedia perkhidmatan awan menghantar token pengesahan ke aplikasi selepas kebenaran berjaya dan token itu digunakan untuk membuat panggilan API.<br><br>

Token pengesahan ialah kunci digital yang membenarkan aplikasi pihak ketiga berinteraksi dengan storan awan. Token pengesahan disimpan pada peranti anda dalam storan sistem selamat dipanggil Keychain. Anda boleh memuat turun fail anda dari perkhidmatan awan yang disambungkan ke peranti dan fail tersebut akan diletakkan dalam direktori "Documents" aplikasi. Anda boleh membuang fail tersebut pada bila-bila masa menggunakan pengurus fail terbina dalam.<br>
Aplikasi tidak berkongsi sebarang maklumat dari akaun awan yang disambungkan. Anda boleh membatalkan akses ke akaun awan anda pada bila-bila masa dengan membuka halaman tetapan akaun pada pelayar web anda.<br><br>

Untuk menolak token pengesahan, log masuk ke akaun anda pada pelayar web dan navigasi ke halaman tetapan. Di sana anda boleh menemui semua aplikasi pihak ketiga yang disambungkan ke akaun awan anda dan membuang mana-mana daripadanya jika anda tidak mahu menggunakan aplikasi itu lagi.<br><br>

Anda juga boleh memutuskan sambungan akaun awan yang disambungkan dalam aplikasi dan token pengesahan juga akan dibuang dari peranti anda. Jika anda membuang aplikasi dari peranti anda, semua data yang dimuat turun dan token akses juga akan dibuang.<br><br>

[Baca lebih lanjut](/docs/guide/flacbox/flacbox-guide-connections)
{{% /details %}}

{{% details title="Bagaimana saya membuat senarai main pada Flacbox?" closed="true" %}}
- Buka bahagian Senarai Main.<br>
- Ketik butang "+" atau butang "..." di penjuru kanan atas dan pilih "Senarai Main Baharu."<br>
- Masukkan nama untuk senarai main dan ketik "Simpan." Dialog "Tambah Lagu" akan muncul.<br>
- Pilih trek yang ingin anda tambahkan ke senarai main.<br><br>

[Baca lebih lanjut](/docs/guide/flacbox/flacbox-guide-playlists)
{{% /details %}}

{{% details title="Apakah perkhidmatan awan yang disokong Flacbox?" closed="true" %}}
Pada masa ini, aplikasi menyokong perkhidmatan awan paling popular: iCloud Drive, Google Drive, Dropbox, OneDrive, Box, MEGA, Yandex.Disk, WD MyCloud Home, DLNA, MediaFire, WebDAV, SMB, pCloud, HiDrive, My Cloud Home, InfiniCLOUD, Cloud Mail.ru, Put.io, MyDrive.<br><br>

[Baca lebih lanjut](/docs/guide/flacbox/flacbox-guide-connections)
{{% /details %}}

{{% details title="Bagaimana saya boleh menggunakan penyama?" closed="true" %}}
- Buka Skrin Pemain Audio.<br>
- Ketik ikon "Penyama" di bahagian bawah skrin.<br>
- Hidupkan kawalan suis togol di penjuru kanan atas pada skrin penyama untuk mengaktifkan penyama.<br>
- Pilih salah satu pratetap yang tersedia (rock, pop, dance, disco..) atau tukar peluncur penyama untuk membina pratetap anda sendiri.<br><br>

Tutorial penuh tersedia di sini:<br>
[Cara menggunakan penyama audio pada iPhone, iPad, Mac anda dengan Evermusic dan Flacbox](/docs/howto/how-to-use-the-audio-equalizer-on-your-iphone-ipad-mac-with-evermusic-and-flacbox)
{{% /details %}}

{{% details title="Bagaimana saya mengaktifkan mod luar talian dalam Flacbox?" closed="true" %}}
- Sambungkan Perkhidmatan Awan:<br>
 • Pergi ke tab "Sambungan".<br>
 • Pilih "Sambung perkhidmatan awan" dan ikuti arahan untuk menyambungkan perkhidmatan yang dikehendaki.<br><br>

- Navigasi ke Folder Muzik:<br>
 • Buka perkhidmatan awan yang disambungkan dan cari folder muzik anda.<br><br>

- Aktifkan Mod Luar Talian:<br>
 • Ketik butang "Lebih tindakan" di sebelah nama folder.<br>
 • Pilih "Aktifkan mod luar talian."<br><br>

- Muat Turun Kandungan:<br>
 • Folder yang dipilih dan semua kandungannya akan dimuat turun ke "Fail tempatan" > "Folder luar talian."<br><br>

- Kemas Kini Automatik:<br>
 • Aplikasi akan terus mengimbas perubahan. Fail baharu yang ditambah ke folder dalam talian akan dimuat turun secara automatik.<br><br>

- Konfigurasikan Tamat Masa Imbasan:<br>
 • Pergi ke "Tetapan" > "Pengurus fail" > "Folder luar talian" > "Selang masa" untuk menetapkan kekerapan imbasan.<br><br>

- Penyegerakan Manual:<br>
 • Untuk menyegerak secara manual, pergi ke "Tetapan" > "Pengurus fail" > "Folder luar talian" > "Folder luar talian yang disegerakkan."<br>
 • Ketik "Lebih tindakan" dan pilih "Mulakan penyegerakan."<br><br>

[Baca lebih lanjut](/docs/howto/play-offline-music-in-evermusic-flacbox-download-sync-from-cloud-to-local-files/)
{{% /details %}}

{{% details title="Bagaimana untuk memainkan muzik yang dimuat turun secara tempatan pada iPhone?" closed="true" %}}
Setelah anda memasang aplikasi, buka skrin "Fail Tempatan" dan tatal ke bahagian "Fail pada iPhone ini". Dari situ, pilih "Buka fail..." jika anda perlu memilih beberapa fail audio atau "Buka folder..." jika anda ingin memilih folder muzik. Aplikasi akan mengimbas kandungan folder dan semua fail audio yang ditemui akan dipilih. Navigasi ke folder muzik anda, ketik "Buka" untuk mengesahkan pilihan anda dan fail akan ditambah ke baris gilir pemain. Fail ini akan dimainkan terus dari lokasi yang dipilih tanpa disalin ke bundle aplikasi.<br><br>

**Menambah Folder ke Kegemaran untuk Akses Pantas**<br>
Mudahkan proses dengan menambah folder pada peranti anda ke kegemaran. Dengan cara ini, anda tidak perlu mengulangi langkah setiap kali ingin memainkan muzik. Buka skrin "Fail Tempatan", tatal ke bahagian "Akses Pantas" dan ketik "Kegemaran" untuk mengakses skrin "Fail Kegemaran". Ketik butang lebih tindakan (tiga titik) di penjuru kanan atas dan pilih "Tambah Folder". Navigasi ke folder yang dikehendaki dan ketik "Buka" untuk mengesahkan. Folder anda akan ditambah ke "Fail Kegemaran", memberikan akses pantas ke muzik anda.<br><br>

**Mengimport Fail Tempatan ke Perpustakaan Muzik**<br>
Jika anda lebih suka menyusun fail tempatan dalam perpustakaan muzik anda, buka skrin "Perpustakaan Muzik", ketik butang tiga titik di penjuru kanan atas dan pilih "+ Tambah Muzik". Pilih item menu "Fail Tempatan", tatal ke bahagian "Fail pada iPhone ini" dan ketik "Buka Fail...". Pilih fail yang ingin anda tambah dan ketik "Buka" untuk mengesahkan. Aplikasi akan mengimbas fail yang dipilih dan menambahnya ke perpustakaan muzik anda, menyusunnya mengikut metadata seperti artis, album dan genre.<br><br>

**Menambah Fail Tempatan ke Senarai Main**<br>
Untuk menambah fail tempatan ke senarai main, buka skrin "Senarai Main" dan ketik butang lebih di penjuru kanan atas. Pilih "+ Senarai Main Baharu", masukkan nama untuk senarai main baharu anda dan pada skrin seterusnya, pilih pilihan "Fail Tempatan". Tatal ke bahagian "Fail pada iPhone ini" dan ketik "Buka Fail...". Pilih fail audio yang ingin anda tambah dan ketik "Buka" untuk mengesahkan. Fail akan ditambah ke senarai main anda, di mana anda boleh menukar susunan lagu dan melakukan tindakan lain menggunakan butang lebih.<br><br>

Dengan langkah mudah ini, anda boleh membuka potensi penuh iPhone dan Mac anda sebagai platform terbaik untuk menikmati koleksi muzik tempatan anda.<br><br>

[Baca lebih lanjut](/docs/howto/how-to-play-local-music-stored-on-your-iphone-or-mac)
{{% /details %}}

{{% details title="Bagaimana saya boleh menyambung semula senarai main dari tempat saya berhenti?" closed="true" %}}
Pertama, pastikan "Simpan Keadaan Pemain Audio" diaktifkan dalam Tetapan > Pemain Audio > Umum. Apabila anda beralih ke senarai main lain dan kembali, anda akan melihat empat tindakan pada bar alat atas di bawah seni karya album: "Carian", "Sambung Main Balik", "Main Semua" dan "Rawak Semua". Ketik "Sambung Main Balik" untuk menyambung semula senarai main dari keadaan dan kedudukan media terakhir yang disimpan.
{{% /details %}}

{{% details title="Bagaimana untuk melihat lirik lagu dalam Flacbox?" closed="true" %}}
Anda boleh melihat lirik terbenam untuk trek dalam aplikasi dengan **mengikuti langkah berikut**:<br>
1. Mulakan memainkan fail audio dengan mengetiknya.<br>
2. Buka pemain audio skrin penuh.<br>
3. Ketik butang "Lebih Tindakan" di penjuru kanan atas untuk melihat menu tindakan tambahan.<br>
4. Pilih item menu "Lihat Ulasan".<br>
5. Pada skrin ulasan trek, anda mempunyai tiga mod yang boleh anda tukar dengan menatal ke kanan: Ulasan, Lirik Terbenam dan Fail LRC.<br><br>

**Mod**:<br>
1. Mod Ulasan: Memaparkan kandungan dari medan COMMENT fail audio anda.<br>
2. Mod Lirik: Menunjukkan lirik terbenam dari fail audio anda. Jika fail audio anda mempunyai lirik dalam tag, ia akan muncul di sini. Mod ini juga menyokong teks berjadual dalam format LRC. Untuk menyunting tag lirik, gunakan penyunting tag audio luaran seperti Evertag. Anda boleh menemui lirik format LRC secara percuma di laman web seperti Lyricsify. Salin lirik yang diperlukan ke tag anda dan simpannya untuk dipaparkan kemudian pada skrin ini.<br>
3. Mod Fail LRC: Daripada menyunting fail audio, anda boleh meletakkan fail LRC dalam folder yang sama dengan fail audio asal. Kedua-dua fail sepatutnya mempunyai nama yang sama tetapi sambungan berbeza. Apabila anda meleret ke halaman ketiga pada skrin Ulasan, aplikasi akan mencari fail LRC dalam direktori yang sama dan memaparkan kandungannya.<br><br>

[Baca lebih lanjut](/docs/howto/how-to-add-and-view-comments-to-your-audio-tracks-on-iphone-ipad-and-mac-with-evermusic-and-flacbox)
{{% /details %}}

{{% details title="Bagaimana saya memindahkan muzik ke Flacbox dari komputer saya?" closed="true" %}}
Anda boleh menyambungkan komputer atau NAS peribadi anda menggunakan protokol SMB, WebDAV atau DLNA. Sebagai alternatif, gunakan iTunes File Sharing untuk memindahkan muzik.<br><br>

Untuk menyambungkan komputer menggunakan protokol SMB ketik "Sambung perkhidmatan awan" → SMB. Masukkan alamat IP komputer dan nama folder kongsi dalam medan URL menggunakan format smb://alamat-ip-komputer/nama-folder-kongsi, masukkan log masuk dan kata laluan dan ketik "Selesai". Jika sambungan anda berjaya anda akan melihat storan yang disambungkan dalam bahagian "Perkhidmatan awan".<br><br>

Tutorial penuh tersedia di sini:<br>
[Pindahkan fail anda dari komputer ke iPhone menggunakan protokol SMB](/docs/howto/transfer-your-files-from-the-computer-to-iphone-using-smb-protocol/)<br><br>

Untuk protokol WebDAV, semua langkah sama kecuali medan URL. URL sepatutnya dalam format http://nama-pelayan atau https://nama-pelayan jika pelayan menyokong SSL.<br><br>

Wi-Fi Drive ialah teknologi popular yang membolehkan anda memindahkan fail dari komputer ke peranti iOS secara wayarles menggunakan pelayar desktop. Untuk menggunakan ciri ini peranti dan komputer anda perlu disambungkan ke rangkaian Wi-Fi yang sama. Buka "Sambungan" → "Komputer" → "Sambung menggunakan Wi-Fi" dan aktifkan pelayan. Selepas ini buka pelayar desktop dan masukkan URL dari aplikasi. Anda boleh seret dan lepas fail dari komputer ke halaman web yang dibuka dan ia akan muncul pada peranti.<br>
Tutorial yang lebih terperinci tersedia di sini:<br>
[Cara memindahkan fail secara wayarles dari komputer ke iPhone menggunakan WiFi-Drive](/docs/howto/how-to-transfer-files-wirelessly-from-a-computer-to-an-iphone-using-wifi-drive)<br><br>

iTunes File Sharing ialah teknologi lain yang membolehkan anda memindahkan fail dari komputer ke peranti menggunakan iTunes dan kabel lightning. Hanya sambungkan peranti ke komputer menggunakan kabel dan jalankan iTunes. Buka iTunes → "Bahagian Aplikasi" → dan cari Evermusic. Ketik ikon aplikasi untuk melihat folder kongsi. Salin fail dari komputer ke folder kongsi pada peranti.<br><br>

Arahan terperinci tersedia di sini:<br>
[Cara memainkan fail tempatan (fail iTunes) pada iPhone saya](/docs/howto/how-to-play-local-itunes-files-on-my-iphone)<br><br>

DLNA Anda juga boleh menyediakan pelayan media DLNA dan menstrim muzik anda dari PC Windows seperti yang diterangkan di sini:<br>
[Cara Mengaktifkan Pelayan Media DLNA pada Windows 10 dan Memainkan Muzik Anda pada iPhone](/docs/howto/how-to-enable-dlna-media-server-on-windows-10-and-play-your-music-on-iphone)
{{% /details %}}

{{% details title="Bagaimana untuk memuat turun muzik?" closed="true" %}}
Sebelum anda boleh memuat turun muzik dan mendengar muzik anda luar talian anda perlu menyambungkan akaun rangkaian.<br>
Hanya buka skrin "Sambungan" dan tambah akaun anda.<br>
Setelah anda menambah akaun rangkaian anda boleh memuat turun muzik anda dari awan.<br><br>

**Untuk memuat turun muzik dari awan**<br>
– Buka perkhidmatan awan yang disambungkan.<br>
– Navigasi ke dalam folder yang ingin anda muat turun.<br>
– Ketik butang lebih tindakan "..." di penjuru kanan atas dan pilih item menu "Pilih".<br>
– Pilih fail yang ingin anda muat turun dan ketik butang "Muat turun".<br><br>

**Untuk mengaktifkan mod luar talian untuk Artis/Senarai Main/Album/Genre**<br>
– Buka Skrin Artis/Album/Senarai Main/Genre<br>
– Ketik kotak semak "Mod luar talian"<br>
– Artis/Album/Senarai Main/Genre Luar Talian akan muncul dalam bahagian "Muzik luar talian" perpustakaan muzik<br><br>

Arahan lebih terperinci mengenai mod luar talian tersedia di sini:<br>
[Main Muzik Luar Talian dalam Evermusic & Flacbox: Muat Turun & Segerak dari Awan ke Fail Tempatan](/docs/howto/play-offline-music-in-evermusic-flacbox-download-sync-from-cloud-to-local-files/)<br><br>

Pilihan lain yang tersedia ialah memuat turun muzik dari perkhidmatan awan dan mengimportnya dalam Evermusic seperti yang diterangkan di sini:<br>
[Cara Memuat Turun Muzik Dari YouTube dan Mendengar Muzik Luar Talian pada iPhone](/docs/howto/how-to-download-music-from-youtube-and-listen-to-offline-music-on-iphone)
{{% /details %}}

{{% details title="Apakah format audio yang disokong Flacbox?" closed="true" %}}
Aplikasi ini menyokong **codec audio sistem** lalai dan **codec perisian ffmpeg** tambahan:<br><br>
3g2, 3gp, 3gp2, 3gpp, 8svx, aa, aac, aax, ac3, act, adt, adts, aif, aifc, aiff, alac, amr, amv, ape, asf, au, avi, awb, caf, cavs, cdda, cue, dct, dff, drc, dsf, dss, dvf, "dvr-ms", ec3, f4a, f4b, f4p, f4v, flac, flv, gif, gifv, gsm, gxf, h261, h263, h264, ifv, iklax, ivf, ivs, l16, latm, loas, m2t, m2ts, m2v, m3u, m3u8, m4a, m4b, m4p, m4r, m4v, mka, mkv, mmf, mng, mod, mogg, mov, mp1, mp2, mp3, mp4, mp4v, mpa, mpc, mpe, mpeg, mpg, mpv, msv, mts, mxf, nsf, nsv, nut, oga, ogg, ogv, opus, pcm, pls, qt, ra, raw, rm, rmvb, roq, rv, sln, snd, svi, tod, tta, vob, voc, vox, vtt, w64, wav, wave, webm, wma, wmv, wv, xhe, xmv, y4m, yuv.
{{% /details %}}

{{% details title="Bolehkah saya menggunakan Flacbox untuk memainkan buku audio?" closed="true" %}}

Ya, Flacbox boleh digunakan sebagai pemain buku audio yang berkuasa.<br><br>

**Cara Menambah Buku Audio**<br>
• Untuk **fail tempatan**, sambungkan iPhone atau iPad anda ke komputer, buka iTunes (atau Finder pada macOS Catalina atau lebih baharu) dan seret fail buku audio anda ke bahagian "Flacbox".<br>
• Untuk **fail awan**, sambungkan perkhidmatan seperti iCloud Drive, Google Drive, Dropbox dan lain-lain dalam aplikasi Flacbox untuk mengakses buku audio anda dari jauh.<br><br>

**Ciri Main Balik**<br>
• Navigasi ke buku audio anda melalui **Perpustakaan Muzik** atau **Fail Tempatan**.<br>
• Flacbox menyokong **main, jeda, langkau** dan **kelajuan main balik boleh laras**.<br>
• Anda boleh mengaktifkan butang **Masa Langkau** dalam tetapan untuk navigasi pantas.<br>
• Cipta **Penanda buku Audio** untuk menyimpan tempat anda.<br>
• Gunakan **Sambung Main Balik** untuk menyambung semula dari tempat anda berhenti.<br><br>

**Penyusunan dan Akses Luar Talian**<br>
• Susun buku audio ke dalam senarai main mengikut **genre**, **pengarang** atau **siri**.<br>
• Muat turun buku audio untuk **pendengaran luar talian**, sempurna untuk perjalanan atau sambungan terhad.<br><br>

Flacbox menyediakan penyelesaian lengkap untuk peminat buku audio pada iPhone, iPad dan Mac.<br><br>

[Baca lebih lanjut](/docs/howto/how-to-listen-to-audiobooks-on-iphone-ipad-mac-using-evermusic)

{{% /details %}}

{{% details title="Adakah Flacbox berfungsi dengan peranti NAS?" closed="true" %}}

Ya, Flacbox menyokong sambungan NAS menggunakan protokol **SMB**, **WebDAV** dan **DLNA**.<br><br>

**Untuk menyambungkan NAS menggunakan protokol SMB:**<br>
• Ketik **"Sambung perkhidmatan awan" → SMB**.<br>
• Masukkan alamat IP pelayan NAS dan nama folder kongsi dalam format ini: `smb://ip-anda/folder-kongsi`<br>
• Pilih versi protokol: **Auto**, **SMB1** atau **SMB2**.<br>
• Masukkan log masuk dan kata laluan anda (jika diperlukan).<br>
• Ketik **"Selesai"**.<br><br>

Jika sambungan berjaya, anda akan melihat NAS anda dalam bahagian **Perkhidmatan awan**.<br>
[Baca tutorial penuh](/docs/howto/stream-your-music-from-mac-or-pc-to-iphone-using-smb/)<br><br>

**Untuk menyambung melalui protokol WebDAV:**<br>
• Langkah sama seperti SMB, tetapi gunakan URL WebDAV seperti `http://nama-pelayan-anda` atau `https://nama-pelayan-anda` jika menggunakan SSL.<br><br>

**Untuk menstrim melalui DLNA:**<br>
• Anda boleh menstrim dari PC Windows dengan mengaktifkan pelayan media DLNA.<br>
[Baca panduan persediaan DLNA](/docs/howto/how-to-enable-dlna-media-server-on-windows-10-and-play-your-music-on-iphone)<br><br>

**Bahagian Peranti Tersedia:**<br>
• Menunjukkan semua peranti NAS yang boleh ditemui pada rangkaian tempatan anda.<br>
• Ketik nama peranti untuk menyambung, kemudian masukkan kelayakan log masuk jika diperlukan.<br><br>

**Pautan Berguna:**<br>
• [Cara memainkan muzik pada iPhone dari WD My Cloud Home](/docs/howto/how-to-play-music-on-iphone-from-wd-my-cloud-home)<br>
• [Strim muzik anda dari MAC atau PC ke iPhone menggunakan SMB](/docs/howto/stream-your-music-from-mac-or-pc-to-iphone-using-smb/)<br>
• [Sambungkan storan Bluesound Vault](/docs/howto/how-to-connect-bluesound-vault-s-internal-storage-from-the-evermusic-flacbox-evertag)<br>
• [Sambungkan Storan NAS Menggunakan WebDAV](/docs/howto/how-to-connect-nas-storage-using-webdav-and-listen-to-music-on-your-iphone-or-mac)

{{% /details %}}

{{% details title="Bagaimana saya mengimport muzik ke Flacbox?" closed="true" %}}

**Sambungkan Perkhidmatan Awan Anda**<br>
• Buka tab **Sambungan**.<br>
• Pilih **Sambung perkhidmatan awan** dari menu.<br>
• Pilih perkhidmatan awan yang dikehendaki dari senarai.<br>
• Masukkan kelayakan anda dan ketik **Selesai**.<br><br>

**Import Fail Pada Peranti Anda**<br>
Anda boleh mengimport fail dari peranti anda, pemacu kilat USB, komputer atau NAS.<br>
• Buka tab **Fail tempatan**.<br>
• Ketik **Buka fail...** atau **Buka folder...** dari menu **Fail pada peranti ini**.<br>
• Pilih fail atau folder pada peranti anda.<br>
• Ketik **Buka** untuk mengimport fail ke aplikasi.<br><br>

**Tambah Muzik Anda**<br>
Anda boleh menambah muzik secara manual atau automatik menggunakan folder penyegerakan.<br><br>

**Penambahan Manual**<br>
• Ketik item menu **Tambah muzik**.<br>
• Pilih folder atau fail dari perkhidmatan awan yang disambungkan atau peranti anda.<br>
• Flacbox mencipta pautan ke trek anda tanpa menyalinnya.<br>
• Untuk menjadikan trek tersedia luar talian, gunakan pengurus muat turun atau aktifkan **Mod luar talian**.<br><br>

**Penyegerakan Automatik**<br>
• Buka **Tetapan Perpustakaan Muzik**.<br>
• Pilih **Folder penyegerakan** untuk menghidupkan penyegerakan automatik.<br>
• Aplikasi mengimbas folder yang dipilih dan menambah fail audio yang disokong ke perpustakaan anda.<br><br>

**Terokai Perpustakaan Muzik Anda**<br>
• Perpustakaan anda disusun mengikut sumber (dalam talian, luar talian, tempatan).<br>
• Layari mengikut lagu, album, artis, genre dan komposer.<br>
• Gunakan bar carian untuk mencari mana-mana lagu dengan cepat.<br><br>

**Ketahui Lebih Lanjut**<br>
Terokai tutorial ini untuk bantuan lanjut:<br><br>

• [Muat naik fail ke awan dan sambungkannya ke Flacbox](/docs/howto/how-to-upload-my-files-to-the-cloud-storage-and-connect-them-to-evermusic-flacbox-evertag)<br>
• [Dengar Buku Audio pada iPhone, iPad atau Mac](/docs/howto/how-to-listen-to-audiobooks-on-iphone-ipad-mac-using-evermusic)<br>
• [Main Muzik Tempatan Yang Disimpan pada iPhone atau Mac Anda](/docs/howto/how-to-play-local-music-stored-on-your-iphone-or-mac)<br>
• [Main Muzik dari iCloud Drive](/docs/howto/how-to-listen-to-music-from-icloud-drive-on-your-iphone-or-mac)<br>
• [Gunakan Penyama Audio](/docs/howto/how-to-use-the-audio-equalizer-on-your-iphone-ipad-mac-with-evermusic-and-flacbox)<br>
• [Sambungkan Kad Kilat USB ke iPhone](/docs/howto/how-to-connect-a-usb-flashcard-to-the-iphone-and-listen-to-music-or-manage-files-located-on-it)<br>
• [Pindahkan Fail Secara Wayarles Menggunakan WiFi-Drive](/docs/howto/how-to-transfer-files-wirelessly-from-a-computer-to-an-iphone-using-wifi-drive)<br>
• [Pindahkan Fail Menggunakan Protokol SMB](/docs/howto/transfer-your-files-from-the-computer-to-iphone-using-smb-protocol/)

{{% /details %}}

{{% details title="Bagaimana saya menggunakan ciri Wi-Fi Drive dalam Flacbox?" closed="true" %}}

**Pemindahan wayarles menggunakan pelayar desktop**<br>
1. Lancarkan aplikasi: Buka Flacbox.<br>
2. Sambung melalui Wi-Fi: Pergi ke "Sambungan" → "Komputer" → "Sambung menggunakan Wi-Fi".<br>
3. (Pilihan) Tambah keselamatan: Masukkan nama pengguna dan kata laluan jika diperlukan.<br>
4. Mulakan Wi-Fi Drive: Ketik "Mulakan Wi-Fi Drive" dan salin URL yang disediakan.<br>
5. Buka pelayar (Safari, Chrome, Opera, Firefox, dll.) dan masukkan URL.<br>
6. Gunakan pengurus fail terbina dalam untuk memuat naik, memuat turun, menamakan semula atau memadam fail. Anda boleh seret dan lepas fail terus ke iPhone anda.<br>
7. Apabila selesai, ketik "Hentikan Wi-Fi Drive" pada iPhone anda.<br><br>

Nota: Pastikan JavaScript diaktifkan dan anda menggunakan versi pelayar terkini untuk prestasi terbaik.<br><br>

**Pindahkan fail menggunakan Mac Finder**<br>
1. Dalam Finder, klik "Pergi" → "Sambung ke Pelayan…".<br>
2. Masukkan URL pelayan yang ditunjukkan dalam aplikasi Flacbox.<br>
3. Klik "Sambung" dan urus fail pada iPhone anda seperti mana-mana pemacu rangkaian.<br><br>

**Pindahkan fail menggunakan Windows File Explorer**<br>
1. Klik kanan "This PC" → "Petakan Pemacu Rangkaian…".<br>
2. Masukkan URL pelayan dari aplikasi Flacbox dalam medan "Folder".<br>
3. Pilih huruf pemacu, klik "Selesai" dan layari fail iPhone anda secara terus.<br><br>

[Baca lebih lanjut](/docs/howto/how-to-transfer-files-wirelessly-from-a-computer-to-an-iphone-using-wifi-drive)

{{% /details %}}

{{% details title="Adakah Flacbox menyokong Apple CarPlay?" closed="true" %}}
Ya, **Flacbox menyokong sepenuhnya Apple CarPlay**. Anda boleh melayari perpustakaan muzik anda, memainkan fail tempatan atau luar talian, menyambung ke storan awan dan mengawal main balik terus dari skrin infotainment kereta anda.

Antara muka CarPlay termasuk tab khusus untuk **Perpustakaan**, **Sambungan**, **Fail Tempatan** dan **Tetapan**, memberikan anda kawalan penuh terhadap muzik anda di jalan raya. Kawalan main balik, rawak, ulang dan pengurusan baris gilir juga tersedia.

Untuk menggunakan CarPlay, pastikan Siri diaktifkan dan iPhone anda disambungkan melalui USB atau wayarles.

[Baca panduan penuh](/docs/howto/how-to-play-your-own-music-on-iphone-using-carplay/)
{{% /details %}}

</div>


<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
  Panduan Pengguna
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

<div class="hx:w-full hx:mx-auto hx:flex hx:flex-col hx:items-center hx:text-center">
  {{< hextra/info-paragraph border="false" >}}
  Panduan ini akan membantu anda memanfaatkan sepenuhnya Flacbox pada iPhone, iPad atau Mac anda. Ketahui cara menstrim muzik resolusi tinggi dari awan, menyusun perpustakaan anda, mengurus buku audio dan memindahkan muzik antara peranti.
  {{< /hextra/info-paragraph >}}
</div>

{{< cards >}}

{{< card icon="map" link="/docs/guide/flacbox/flacbox-guide-navigation" title="Navigasi" subtitle="Gunakan Bar Tab pada iPhone atau Menu Kiri pada iPad & Mac." >}}

{{< card icon="cloud" link="/docs/guide/flacbox/flacbox-guide-connections" title="Sambungan" subtitle="Sambungkan Dropbox, Google Drive, iCloud atau NAS anda." >}}

{{< card icon="collection" link="/docs/guide/flacbox/flacbox-guide-music-library" title="Perpustakaan Muzik" subtitle="Urus dan cari trek mengikut artis, album atau genre." >}}

{{< card icon="music-note" link="/docs/guide/flacbox/flacbox-guide-playlists" title="Senarai Main" subtitle="Cipta dan susun senarai main untuk sebarang suasana atau acara." >}}

{{< card icon="folder" link="/docs/guide/flacbox/flacbox-guide-local-files" title="Fail Tempatan" subtitle="Sunting dan mainkan muzik luar talian dengan pengurus fail terbina dalam." >}}

{{< card icon="play" link="/docs/guide/flacbox/flacbox-guide-player" title="Pemain Audio" subtitle="Kawal main balik, laras kelajuan, tetapkan penanda buku dan lagi." >}}

{{< card icon="adjustments" link="/docs/guide/flacbox/flacbox-guide-settings" title="Tetapan" subtitle="Sesuaikan penyama, penampilan dan tingkah laku aplikasi." >}}

{{< /cards >}}

</div>

<div class="hx:mt-6"></div>
