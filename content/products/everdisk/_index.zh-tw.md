...
layout: hextra-home
description: "把你的 iPhone 或 iPad 變成無線隨身碟，透過 DLNA、HTTP、WebDAV、SMB 和 FTP 和任何裝置分享檔案、相片、影片和音樂。可掛載成網路磁碟並選用 SMB 加密。不需要雲端。"
keywords: ["Everdisk", "iPhone 無線隨身碟", "iPhone 分享檔案", "iOS WebDAV 伺服器", "iPhone SMB 伺服器", "iOS SMB 加密", "iPhone DLNA 伺服器", "iOS FTP 伺服器", "從 iPhone 串流到電視", "iPhone 網路磁碟", "iPhone USB 檔案傳輸", "iOS 檔案管理", "掃描文件成 PDF", "透過網路分享相片"]
tags: ["Everdisk", "無線隨身碟", "檔案分享", "DLNA", "WebDAV", "SMB", "FTP", "SFTP", "網路磁碟", "iPhone 應用程式", "iPad 應用程式", "Mac 應用程式", "檔案管理", "媒體伺服器"]
appStoreUrl: "https://apps.apple.com/app/apple-store/id6751851132?pt=95781850&ct=everappzcom&mt=8"
appStoreId: "6751851132"
features: "透過 Wi-Fi 和 USB 的無線隨身碟, 同時開五種伺服器 (DLNA、HTTP、WebDAV、SMB、FTP), 透過 DLNA 串流到智慧電視, 在任何瀏覽器開啟, 在 Finder/Windows/Linux 掛載成網路磁碟, SMB 網路磁碟並選用 SMB3 加密 (AES), 連接 DLNA/WebDAV/FTP/SFTP/SMB 伺服器, 內建檔案管理，可壓縮與解壓縮, 掃描文件成 PDF, 為 PDF 和相片加註與簽名, PDF 工具 (壓縮、OCR 文字辨識、密碼保護、權限、中繼資料、扁平化), 相片圖庫與媒體播放器, 分享完整相片與音樂庫, 密碼保護與封鎖裝置"
screenshots:
...
{{% sr-only %}}
Everdisk 是 Everappz 開發的 iPhone 和 iPad 無線隨身碟 App。它把你的裝置變成一個中樞，和區域網路上的任何裝置分享檔案、相片、影片和音樂，不需要帳號、不需要雲端，對方也不必額外安裝任何 App。這個 App 同時執行五種伺服器 - 給智慧電視和媒體播放器的 DLNA、給任何瀏覽器的 HTTP、給 Finder、Windows 和 Linux 的 WebDAV、給 Mac、Windows 和 Linux 上網路磁碟的 SMB (可選用 SMB3 加密)，以及給檔案 App 和進階使用者的 FTP - 讓每台裝置都能用自己習慣的方式連上。你可以透過 DLNA 把相片、影片和音樂直接串流到智慧電視，還會附上預覽縮圖；分享一個簡單的連結，在任何瀏覽器打開就能瀏覽和下載你的檔案；或是把裝置掛載成網路磁碟，雙向拖放檔案。你也可以透過 SMB 掛載，它是給 Mac、Windows 和 Linux 的網路磁碟，在 Mac 上會自己出現在 Finder 側邊欄，並可開啟 SMB3 加密 (AES)，讓同一個網路上的其他人都無法讀取你的傳輸內容。沒有 Wi-Fi 時，你可以用 USB 傳輸線連接 Mac，傳檔比 Wi-Fi 更快，即使在飛機上或在受限的網路裡也沒問題。Everdisk 也能連上你網路裡的 DLNA、WebDAV、FTP、SFTP 和 SMB 伺服器 - 包括 Mac、Windows PC、Linux 電腦和 NAS 磁碟 - 瀏覽、串流和下載它們的檔案。內建的檔案管理可以用清單或格狀瀏覽，建立資料夾、重新命名、移動、複製、壓縮、解壓縮和刪除。你可以把紙本文件掃描成乾淨的 PDF，打開 PDF 或相片用內建編輯器加註 - 繪圖、螢光標記、加文字和簽名，變更會存回原檔 - 還能用一整套 PDF 工具來壓縮檔案、辨識文字 (OCR) 成可搜尋的 PDF、加上或移除開啟密碼、檢視權限、編輯文件中繼資料，以及扁平化註解和表單欄位。你也可以在迷你播放器裡播放音訊、全螢幕觀看影片，並把整個相片和音樂庫以原始品質或更相容的格式分享到網路上。所有內容都留在你的區域網路裡，永遠不會接觸網際網路，你還可以用帳號和密碼保護存取權限，一鍵封鎖任何裝置。Everdisk 在 App Store 免費下載，並提供選購的一次性 Premium 終身版。
{{% /sr-only %}}
...
• 點一下開始，五種方式同時分享：DLNA、HTTP、WebDAV、SMB 和 FTP  
• 透過 DLNA 把相片、影片和音樂直接串流到智慧電視  
• 在 Finder、Windows 或 Linux 把裝置掛載成網路磁碟 - 並可選用 SMB 加密  
...
{{< hextra/feature-grid >}}
  ...
  {{< hextra/feature-card
    title="你的裝置變成隨身碟"
    subtitle="點一下開始，你的 iPhone 或 iPad 就變成無線隨身碟。Everdisk 同時執行五種伺服器 - DLNA、HTTP、WebDAV、SMB 和 FTP - 讓智慧電視、電腦、瀏覽器和檔案 App 各自用習慣的方式連上。不需要帳號、不需要雲端，對方也不必額外安裝任何東西。"
    icon="server"
  ...
  {{< hextra/feature-card
    title="連接其他裝置"
    subtitle="尋找並連接你網路上的 DLNA、WebDAV、FTP、SFTP 和 SMB 伺服器 - Mac、Windows PC、Linux 電腦和 NAS 磁碟。瀏覽它們的資料夾、串流音訊和影片、下載檔案，在伺服器允許時還能建立資料夾、上傳、重新命名、移動或刪除。"
    icon="cloud-download"
...
{{< cards cols="4">}}
  ...
  {{< feature-card 
    ...
    method="Fill"
    options="1030x400 q82 webp Center"
    subtitle="探索並連接你網路上的 DLNA、WebDAV、FTP、SFTP 和 SMB 伺服器，瀏覽、串流和下載它們的檔案。" 
  >}}
...
{{< cards >}}
  ...
  {{< feature-card title="同時開五種伺服器" subtitle="點一下開始，五種方式同時分享你的檔案：給電視和媒體播放器的 DLNA、給任何瀏覽器的 HTTP、給 Finder、Windows 和 Linux 的 WebDAV、給 Mac、Windows 和 Linux 上網路磁碟的 SMB，以及給檔案 App 和進階使用者的 FTP。" icon="wifi" >}}
  ...
  {{< feature-card title="加密的 SMB 磁碟" subtitle="透過 SMB 掛載你的裝置，它是給 Mac、Windows 和 Linux 的網路磁碟 - 在 Mac 上會自己出現在 Finder 側邊欄。它是唯一可以加密的連線：用帳號和密碼開啟 SMB3 加密 (AES)，讓每一次傳輸都保持私密。" icon="lock-closed" >}}

  {{< feature-card title="透過 USB 傳輸" subtitle="用充電線接上 Mac，檔案直接透過傳輸線傳送，比 Wi-Fi 更快。在飛機上、飯店裡，或任何受限網路都能持續運作。" icon="lightning-bolt" >}}

  {{< feature-card title="連接 DLNA、WebDAV、FTP、SFTP 和 SMB" subtitle="尋找並連接你網路上的伺服器與 NAS 磁碟，瀏覽它們的資料夾、串流音訊和影片、下載檔案，在允許時還能建立、上傳、重新命名、移動或刪除。" icon="server" >}}
...
這個 App 同時執行五種伺服器，讓每台裝置都能用自己習慣的方式連上：智慧電視用 DLNA、電腦和瀏覽器用 HTTP、Finder、Windows 和 Linux 用 WebDAV、網路磁碟用 SMB (可選用加密)，檔案 App 則用 FTP。
{{% /details %}}

{{% details title="Everdisk 如何運作？" closed="true" %}}
當你點一下開始，Everdisk 就會開始分享你選取的內容，並同時執行五種伺服器，讓不同裝置用各自偏好的方式連上。<br><br>
...
- <strong>SMB</strong> 讓你的裝置成為 Mac、Windows 和 Linux 上的網路磁碟 - 在 Mac 上會自己出現在 Finder 側邊欄 - 而且它是唯一可以用 SMB3 加密 (AES) 加密的連線。<br>
- <strong>FTP</strong> 適用於檔案 App 和進階使用者。<br><br>

Everdisk 也能反過來連接你網路上的 DLNA、WebDAV、FTP、SFTP 和 SMB 伺服器 - 包括 Mac、Windows PC、Linux 電腦和 NAS 磁碟 - 讓你瀏覽它們的資料夾、串流音訊和影片，並在伺服器允許時下載或管理檔案。所有內容都留在你的區域網路裡，永遠不會接觸網際網路。
{{% /details %}}

{{% details title="Everdisk 是免費的嗎？" closed="true" %}}
是的。Everdisk 免費下載，你可以免費把裝置變成無線隨身碟、用五種方式分享檔案、連接其他伺服器，並使用內建檔案管理。<br><br>

另有選購的一次性 <strong>Premium 終身版</strong> - 單次付款、沒有訂閱 - 可解鎖無限共享資料夾和已儲存連線、相片與影片轉換、SMB3 加密 (AES)、自訂連接埠、自動開始分享，以及裝置個人化。它綁定你的 Apple ID，所以你可以用「回復購買項目」在你其他的裝置上解鎖。價格可能因你所在的國家或地區而有所不同。
{{% /details %}}
...
{{% details title="我可以使用 SMB 並加密連線嗎？" closed="true" %}}
可以。除了 WebDAV，Everdisk 也能透過 <strong>SMB</strong> 分享 - 這是給 Mac、Windows 和 Linux 的網路磁碟 (在 App 裡顯示為 <strong>電腦（進階）</strong> 連線)。在 Mac 上，它會在「位置」底下自己出現在 Finder 側邊欄；在 Windows 上，用 App 裡顯示的 <code>smb://</code> 位址在檔案總管打開它。<br><br>

SMB 也是唯一可以<strong>加密</strong>的連線。設定帳號和密碼，然後在「設定」裡開啟 <strong>要求 SMB 加密</strong>，用 <strong>SMB3 加密 (AES)</strong> 保護每一次傳輸，讓同一個網路上的其他人都無法讀取你的檔案。加密的連線不能匿名，所以必須設定帳號和密碼，而且你需要支援 SMB3 的用戶端 - 新型 Mac 上的 Finder，或 Windows 10 以上版本。
{{% /details %}}
...
Everdisk 也是你網路上其他裝置的用戶端。你可以尋找並連接 <strong>DLNA</strong>、<strong>WebDAV</strong>、<strong>FTP</strong>、<strong>SFTP</strong> 和 <strong>SMB</strong> 伺服器，包括 Mac、Windows PC、Linux 電腦、NAS 裝置和媒體伺服器。<br><br>
...
音訊可以從你的本機檔案、連接的資料夾、你的音樂庫，以及你在「可用裝置」分頁連接的伺服器播放。
{{% /details %}}
...
影片可以從你的本機檔案和連接的資料夾播放，或直接從你在「可用裝置」分頁連接的裝置串流，這樣你就不用先下載整個檔案。
{{% /details %}}
...
{{< cards >}}
  {{< feature-card icon="play" title="分享" subtitle="點一下開始，選擇要分享什麼，並同時執行五種伺服器。" link="/docs/guide/everdisk/everdisk-guide-sharing" >}}
  ...
  {{< feature-card icon="server" title="連接伺服器" subtitle="連上 DLNA、WebDAV、FTP、SFTP 和 SMB 伺服器與 NAS 磁碟，瀏覽、串流和下載。" link="/docs/guide/everdisk/everdisk-guide-devices" >}}
