...
<span class='hx:block hx:mt-2 hx:text-center'>
  ...
  {{< hextra/feature-card
    ...
    subtitle=`<span class='pricing-details'>• 無廣告<br />
• 編輯音訊標籤<br>
• 編輯專輯封面<br>
• 同時編輯檔案<br>
• 修復字元編碼<br>
...
<span class='hx:block hx:mt-2 hx:text-center'>
  <span class='pricing-btn hx:inline-block hx:rounded-full hx:border hx:border-gray-200 hx:dark:border-neutral-800 hx:px-3 hx:py-1 hx:transition-all hx:duration-200'>
    免費試用7天，之後<strong>$12.99</strong>/年
  </span>
</span>`
  >}}
...
此版本非常適合想要在無需訂閱的情況下試用Evertag核心功能的使用者。您可以編輯120多個音訊標籤、直接從雲端管理檔案，並使用內建檔案管理器整理音樂庫。<br><br>

**Evertag高級版（月度、年度或終身）**  <br>
升級到高級版可移除廣告並解鎖Evertag的全部潛力。您將獲得所有功能的無限存取權限：編輯120多個標籤、批次編輯多個檔案、修復編碼以及管理無限收藏。您可以連接無限雲端儲存服務、執行無限自動標籤搜尋和搜尋無限專輯封面。<br><br>

高級版使用者還可以存取完整的個人化設定，包括變更應用程式圖示和主題。此方案非常適合音樂收藏家、DJ或任何在不同裝置和雲端平台上管理大型音樂庫的人。<br><br>

**定價選項**  <br>
• 高級月度版 — $1.99/月  <br>
• 高級年度版 — $12.99/年  <br>
• 高級終身版 — $24.99（一次性購買）  <br><br>
...
授權權杖是一種數位金鑰，允許第三方應用程式與雲端儲存互動。授權權杖儲存在您裝置上名為Keychain的安全系統儲存中。您可以將檔案從已連接的雲端服務下載到裝置，這些檔案將被放置在應用程式的「Documents」目錄中。您可以隨時使用內建檔案管理器刪除這些檔案。<br>
應用程式不會分享已連接雲端帳戶中的任何資訊。您可以隨時透過在網頁瀏覽器中開啟帳戶設定頁面來撤銷對雲端帳戶的存取權限。<br><br>

要拒絕授權權杖，請在網頁瀏覽器中登入您的帳戶並導覽到設定頁面。在那裡您可以找到所有連接到您雲端帳戶的第三方應用程式，如果您不再想使用某個應用程式，可以將其移除。<br><br>
...
導覽到您的音樂資料夾，點擊「開啟」確認選擇，檔案將被新增到標籤編輯器。<br>
這些檔案將直接從所選位置編輯，不會被複製到應用程式套件中。<br><br>

**新增裝置資料夾以快速存取**<br>
透過將裝置上的資料夾新增到快速存取部分來簡化流程。<br>
這樣，每次想要編輯音樂資料夾中檔案的標籤時，您就不需要重複這些步驟了。<br>
開啟「本機檔案」畫面。<br>
...
* 向下捲動到「非同步歌詞」部分，然後點擊文字欄位。<br>
* 點擊「新增頁面」選單項目。<br>
* 在「語言」部分，選擇歌詞語言，或留空以預設使用英語。<br>
* 在「文字」部分，以純文字或帶時間碼的LRC格式輸入歌詞。畫面上顯示了一個範例。您也可以在 [lyricsify.com](https://www.lyricsify.com/) 找到LRC格式的歌詞<br>
* 點擊「完成」確認目前畫面和歌詞編輯畫面上的變更。<br>
* 關閉歌詞編輯畫面，然後點擊「儲存」將變更套用到目前檔案。<br><br>
...
在URL欄位中輸入電腦IP位址和共用資料夾名稱，格式為 smb://電腦IP位址/共用資料夾名稱，<br>
輸入使用者名稱和密碼，然後點擊「完成」。<br>
如果連線成功，您將在「雲端儲存」部分看到已連接的儲存。<br><br>

完整教學在此：<br>
[使用SMB通訊協定將檔案從電腦傳輸到iPhone](/docs/howto/transfer-your-files-from-the-computer-to-iphone-using-smb-protocol/)<br><br>

對於**WebDAV**通訊協定，除了URL欄位外，所有步驟相同。<br>
URL應為 http://伺服器名稱 格式，如果伺服器支援SSL，則為 https://伺服器名稱。<br><br>

完整教學在此：<br>
[如何使用WebDAV連接NAS儲存並在iPhone或Mac上聆聽音樂](/docs/howto/how-to-connect-nas-storage-using-webdav-and-listen-to-music-on-your-iphone-or-mac)<br><br>

**Wi-Fi Drive**是一種熱門的技術，允許您使用桌面瀏覽器透過無線方式將檔案從電腦傳輸到iOS裝置。<br>
要使用此功能，您的裝置和電腦需要連接到同一個Wi-Fi網路。<br>
開啟「連線」→「電腦」→「透過Wi-Fi連接」並啟用伺服器。<br>
然後開啟桌面瀏覽器並輸入應用程式中的URL。<br>
您可以將檔案從電腦拖放到開啟的網頁上，它們將出現在裝置上。<br><br>

更詳細的教學在此：<br>
[如何使用WiFi-Drive從電腦無線傳輸檔案到iPhone](/docs/howto/how-to-transfer-files-wirelessly-from-a-computer-to-an-iphone-using-wifi-drive)<br><br>

**iTunes檔案共享**是另一種技術，允許您使用iTunes和lightning傳輸線將檔案從電腦傳輸到裝置。<br>
只需使用傳輸線將裝置連接到電腦並執行iTunes。<br>
開啟iTunes →「應用程式部分」→ 找到Evertag。<br>
點擊應用程式圖示檢視共用資料夾。<br>
將檔案從電腦複製到裝置上的共用資料夾。<br><br>
...
您可以使用內建檔案管理器瀏覽和管理檔案。該應用程式支援所有基本檔案操作，如重新命名、刪除、移動等。<br><br>
...
• **顯示確認訊息** – 啟用後，應用程式將在更新雲端檔案之前提示確認訊息。<br>
• **自動更新檔案中繼資料** – 啟用後，應用程式將在儲存後立即將變更套用到雲端檔案。<br>
...
所選檔案的可用動作：<br>
• <strong>下載</strong> – 將雲端儲存中的檔案或資料夾下載到「本機檔案」部分。<br>
• <strong>移動</strong> – 將檔案或資料夾移動到雲端儲存中的其他資料夾。<br>
• <strong>重新命名</strong> – 在遠端儲存上重新命名檔案或資料夾。<br>
• <strong>刪除</strong> – 永久刪除雲端儲存中的檔案或資料夾。<br>
• <strong>排序</strong> – 按名稱、大小或編輯日期對檔案排序。<br>
