---
date: '2025-06-12T17:00:00+00:00'
draft: false
title: 'Hjem'
layout: hextra-home
description: "Udforsk Everappz-apps til iPhone og Mac: afspil FLAC, DSD, APE og ALAC, stream fra cloud-lagring eller NAS, download musik og videoer, rediger tags og tilpas afspilning med equalizer- og playlisteværktøjer."
keywords: [
  "FLAC-afspiller iPhone", "DSD-lydafspiller iOS", "ALAC-afspiller Mac", "offline musikafspiller iPhone",
  "hi-res lyd-app iOS", "tabsfri musikafspiller", "stream musik fra NAS", "cloud-musikafspiller",
  "videoafspiller til iPhone", "MP4 MKV-afspiller Mac", "lydequalizer iOS", "metadata-tag-editor iPhone",
  "multi-cloud filafspiller", "iTunes-fildeling-app", "CarPlay-musik-app",
  "AirPlay- og Chromecast-understøttelse", "Mac-musikorganisering", "playlistehåndtering iOS"
]
tags: [
  "lydafspiller", "videoafspiller", "FLAC-understøttelse", "offline afspilning", "cloud-streaming",
  "hi-res lyd", "DSD-lyd", "iPhone-apps", "Mac-apps", "lyd-tag-editor",
  "mediebiblioteksorganisering", "equalizer", "filhåndtering", "NAS-streaming",
  "Chromecast", "AirPlay", "CarPlay", "oprettelse af playliste"
]
headless: false
aliases:
  - /home/
  - /index.php/
---

<!-- force dark theme for this page -->
{{< force-dark >}}

{{< dark-gradient-bg >}}

{{< home-hero-banner >}}

<div class="hx:mt-12"></div>

<div class="hx:w-full">
{{< hero-slideshow >}}
icon="/images/app_icons/webp/Evermusic_Icon-App-1024x1024.webp" title="Evermusic" appStoreUrl="https://apps.apple.com/app/apple-store/id885367198?pt=95781850&ct=everappzcom&mt=8" productPageUrl="/products/evermusic/" imageDesktop="products/evermusic/heroimage/hero_desktop.jpg" imageMobile="products/evermusic/heroimage/hero_mobile.jpg"
icon="/images/app_icons/webp/Flacbox_Icon-App-1024x1024.webp" title="Flacbox" appStoreUrl="https://apps.apple.com/app/apple-store/id1097564256?pt=95781850&ct=everappzcom&mt=8" productPageUrl="/products/flacbox/" imageDesktop="products/flacbox/heroimage/hero_desktop.jpg" imageMobile="products/flacbox/heroimage/hero_mobile.jpg"
icon="/images/app_icons/webp/Evervideo_Icon-App-1024x1024.webp" title="Evervideo" appStoreUrl="https://apps.apple.com/us/app/evervideo-hd-video-player/id6602897336?ct=everappzcom&mt=8" productPageUrl="/products/evervideo/" imageDesktop="products/evervideo/heroimage/hero_desktop.jpg" imageMobile="products/evervideo/heroimage/hero_mobile.jpg"
icon="/images/app_icons/webp/Evertag_Icon-App-1024x1024.webp" title="Evertag" appStoreUrl="https://apps.apple.com/app/apple-store/id1450763230?pt=95781850&ct=everappzcom&mt=8" productPageUrl="/products/evertag/" imageDesktop="products/evertag/heroimage/hero_desktop.jpg" imageMobile="products/evertag/heroimage/hero_mobile.jpg"
{{< /hero-slideshow >}}
</div>

<div class="hx:mt-16"></div>

<div class="hx:w-full">

{{< hextra/feature-grid >}}

  {{< hextra/feature-card
    title="Bygget til dig. Forbedret af dig."
    subtitle=`Vi læser alle anmeldelser og bruger din feedback til at forbedre hver opdatering.`
    icon="code"
    style="background: radial-gradient(circle at 50% 80%, rgba(99,102,241,0.15), transparent);"
  >}}

  {{< hextra/feature-card
    title="Ydeevne møder formål."
    subtitle=`Ingen oppustethed. Kun rene, stabile apps med funktioner, der betyder noget.`
    icon="presentation-chart-line"
    style="background: radial-gradient(circle at 50% 80%, rgba(236,72,153,0.15), transparent);"
  >}}

  {{< hextra/feature-card
    title="Privatliv. Tilgængelighed. Enkelhed."
    subtitle=`Nemme at bruge, fuldt tilgængelige og bygget med dit privatliv i tankerne.`
    icon="shield-check"
    style="background: radial-gradient(circle at 50% 80%, rgba(16,185,129,0.15), transparent);"
  >}}

{{< /hextra/feature-grid >}}

</div>

<div class="hx:mt-12"></div>

<div class="hx:w-full">
{{< press-carousel >}}
</div>

<div class="hx:mt-12"></div>

{{< hextra/section-headline >}}
Vores produkter
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">

{{< cards cols="8">}}

  {{< card 
    link="/products/evervideo" 
    title="Evervideo" 
    tag="Ny"
    subtitle="Afspil 360° videoer, se med undertekster, brug en video-equalizer, organiser dine medier med playlister, download videoer til offline brug og stream fra iCloud." 
    image="/images/app_icons/webp/Evervideo_Icon-App-1024x1024.webp"
    method="Resize"
    options="200x q80 webp"
    imageStyle="width: 72px; height: auto; margin-left: 1rem; margin-top: 1rem; border-radius: 12px; align-self: start; flex-shrink: 0;"
  >}}

  {{< card 
    link="/products/evermusic"
    title="Evermusic" 
    tag="11 millioner downloads"
    subtitle="Cloud-musikafspiller med offlinetilstand, audio-equalizer, crossfade, gapless afspilning, playlister, musikbibliotek, filhåndtering." 
    image="/images/app_icons/webp/Evermusic_Icon-App-1024x1024.webp"
    method="Resize"
    options="200x q80 webp"
    imageStyle="width: 72px; height: auto; margin-left: 1rem; margin-top: 1rem; border-radius: 12px; align-self: start; flex-shrink: 0;"
  >}}

  {{< card 
    link="/products/flacbox"
    title="Flacbox" 
    tag="1 million downloads"
    subtitle="Hi-Res lydafspiller til iPhone og Mac. Lyt til din musik i tabsfrie lydformater: flac, alac, ape, wv, dsd og mere. Aktivér avancerede lydudgangsindstillinger.​" 
    image="/images/app_icons/webp/Flacbox_Icon-App-1024x1024.webp"
    method="Resize"
    options="200x q80 webp"
    imageStyle="width: 72px; height: auto; margin-left: 1rem; margin-top: 1rem; border-radius: 12px; align-self: start; flex-shrink: 0;"
  >}}

  {{< card 
    link="/products/evertag"
    title="Evertag" 
    subtitle="Musiktageditor med automatisk korrektion og batchtilstand. Find manglende metadata, rediger albumcovers. Rediger ID3 / FLAC / APE. Mere end 120 tags understøttet." 
    image="/images/app_icons/webp/Evertag_Icon-App-1024x1024.webp"
    method="Resize"
    options="200x q80 webp"
    imageStyle="width: 72px; height: auto; margin-left: 1rem; margin-top: 1rem; border-radius: 12px; align-self: start; flex-shrink: 0;"
  >}}

{{< /cards >}}

</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
App Store-anmeldelser
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full">
{{< appstore-reviews apps="1097564256,1594027432,885367198,1564384601,1450763230,1594027661" stars="5,4" >}}
</div>

<div class="hx:mt-6"></div>

{{< hextra/section-headline >}}
Abonner
{{< /hextra/section-headline >}}

<div class="hx:mt-6"></div>

<div class="hx:w-full hx:text-center">

{{< hextra/hero-paragraph >}}
Bliv en del af dem, der får de seneste nyheder og eksklusive tilbud fra Everappz-teamet.  
Glem ikke at følge os på sociale medier for de seneste nyheder og opdateringer om appen.  
Ved at abonnere accepterer du vores [Privacy Policy](/legal/privacy-policy) og [Terms and Conditions](/legal/terms-and-conditions/).
{{< /hextra/hero-paragraph >}}

</div>

<div class="hx:mt-6"></div>

<div class="hx:w-full hx:text-center">

{{< rawhtml >}}

<form action="https://everappz.us10.list-manage.com/subscribe/post?u=f758cdf6a38df2a75513ac5f1&amp;id=2373740226" 
method="post" 
id="mc-embedded-subscribe-form" 
name="mc-embedded-subscribe-form" 
class="validate" 
target="_blank" 
novalidate
>

<!-- style from shortcodes/card -->
<input
  type="email"
  placeholder="Indtast e-mail"
  value=""
  name="EMAIL"
  id="mce-EMAIL"
  required
  class="email
    hx:rounded-full
    hx:border hx:border-gray-200 hx:dark:border-neutral-800
    hx:hover:border-gray-300 hx:dark:hover:border-neutral-700
    hx:bg-transparent
    hx:font-normal
    hx:text-gray-700 hx:dark:text-neutral-200
    hx:px-4 hx:py-3"
style="height: 50px; width: 300px; margin-bottom:1rem; margin-right:1rem; outline: none; box-shadow: none;"
id="mce-EMAIL" 
required
>

<div 
style="position: absolute; width: 0; height: 0; overflow: hidden;" aria-hidden="true"><input type="text" name="b_f758cdf6a38df2a75513ac5f1_2373740226" tabindex="-1" value=""></div>

<!-- style from hextra/hero-button -->
<button type="submit"
class="not-prose hx:font-bold hx:cursor-pointer hx:px-6 hx:py-3 hx:rounded-full hx:text-center hx:text-white hx:inline-flex hx:items-center hx:gap-2 hx:bg-primary-600 hx:hover:bg-primary-700 hx:focus:outline-hidden hx:focus:ring-4 hx:focus:ring-primary-300 hx:dark:bg-primary-600 hx:dark:hover:bg-primary-700 hx:dark:focus:ring-primary-800 hx:transition-all hx:ease-in hx:duration-200" 
style="outline: none; box-shadow: none;">Abonner</button>

{{< /rawhtml >}}

</div>

<div class="hx:mt-6"></div>
